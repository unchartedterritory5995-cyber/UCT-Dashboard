...
- **On the chart (Catalysts tab):** catalyst candles + labels render **white** (not gold) — `StockChart` gained a `highlightColor` prop (default gold; Model Book passes white via `chartHighlightColor`). The markers are **text-only labels with no shape** — `size: 0` zeroes the marker shape (`Math.max(marker.size, 0)` in lightweight-charts) while keeping the white title text, so there's no circle.

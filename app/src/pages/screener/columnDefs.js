...
export const COLUMN_DEFS = {
  ...
  enterprise_value: { label: 'Enterprise Value', fmt: cap,
    desc: 'Market cap plus net debt — what it would cost to buy the whole business, including taking on its borrowings and keeping its cash. The denominator to prefer over market cap when comparing companies with very different debt loads.' },
  ev_sales: { label: 'EV / Sales', fmt: num(1),
    desc: 'Enterprise value over trailing revenue. The valuation multiple that still works when a company has no earnings to divide by — which is why it is the standard for unprofitable growth.' },
  ev_ebitda: { label: 'EV / EBITDA', fmt: num(1),
    desc: 'Enterprise value over earnings before interest, tax, depreciation and amortisation. The multiple acquirers actually use, because it is neutral to how a company is financed and how aggressively it depreciates.' },
  ev_fcf: { label: 'EV / FCF', fmt: num(1),
    desc: 'Enterprise value over free cash flow — the same idea as EV/EBITDA but against cash that actually arrived, after capital spending.' },
  earnings_yield: { label: 'Earnings Yield', fmt: pctPlain(2),
    desc: 'Trailing earnings as a percentage of price — P/E turned upside down. ⚠️ Unlike P/E it stays readable through zero, so a loss-maker reads as a negative yield rather than as a meaningless multiple.' },
  fcf_yield: { label: 'FCF Yield', fmt: pctPlain(2),
    desc: 'Free cash flow as a percentage of price. What the business would pay you if it handed over every spare dollar — and the cash-based answer to a P/E that accounting choices can flatter.' },
  eps_ttm: { label: 'EPS (TTM)', fmt: usd,
    desc: 'Trailing-twelve-month earnings per share, in dollars. Negative for a loss-maker, and published as such.' },
  revenue_ps: { label: 'Revenue / Share', fmt: usd,
    desc: 'Trailing-twelve-month revenue per share. Comparable across a company\'s own history in a way total revenue is not, because it accounts for shares issued.' },
  fcf_ps: { label: 'FCF / Share', fmt: usd,
    desc: 'Free cash flow per share — operating cash flow after capital spending, per share. Goes negative for a company investing more than it earns.' },
  book_value_ps: { label: 'Book Value / Share', fmt: usd,
    desc: 'Shareholders\' equity per share. The denominator under P/B; negative when a company has burned through its equity.' },
  cash_ps: { label: 'Cash / Share', fmt: usd,
    desc: 'Cash and equivalents per share. Read beside the price: a name trading near its cash per share is being valued at little more than its balance sheet.' },
  working_capital: { label: 'Working Capital', fmt: netUsd,
    desc: 'Current assets minus current liabilities, in dollars. ⚠️ Deeply negative is normal for banks and for businesses paid before they pay suppliers — compare within an industry, not across.' },
  ebitda_margin: { label: 'EBITDA Margin', fmt: pctPlain(1),
    desc: 'EBITDA as a percentage of revenue — profitability before interest, tax and the depreciation schedule, so it is the margin least affected by how a company financed and wrote down its assets.' },
  ebit_margin: { label: 'EBIT Margin', fmt: pctPlain(1),
    desc: 'Operating profit as a percentage of revenue, after depreciation but before interest and tax. Sits between gross and net margin and is the cleanest read on the operating business itself.' },
  roce: { label: 'ROCE', fmt: pctPlain(1),
    desc: 'Return on capital employed — operating profit against the debt AND equity put to work. ⭐ The profitability measure least distorted by capital structure, which is why it compares across companies where ROE cannot.' },
  income_quality: { label: 'Income Quality', fmt: num(2),
    desc: 'Operating cash flow divided by net income. Above 1.0 means reported profit is backed by cash arriving; ⚠️ persistently below 1.0 means earnings are being recognised faster than they are collected.' },
  tax_rate: { label: 'Effective Tax Rate', fmt: pctPlain(1),
    desc: 'Tax actually paid as a percentage of pre-tax profit. A rate far below the statutory one is usually credits or foreign structure, and is worth knowing before assuming today\'s net margin persists.' },
  interest_coverage: { label: 'Interest Coverage', fmt: num(1),
    desc: 'Operating profit divided by interest expense — how many times over the company covers its interest bill. ⚠️ Blank rather than 0 for a company with no interest expense to cover: a 0 here would read as the exact opposite, a company that cannot pay its interest at all.' },
  net_debt_to_ebitda: { label: 'Net Debt / EBITDA', fmt: num(2),
    desc: 'Borrowings minus cash, against EBITDA — roughly how many years of earnings it would take to clear the debt. ⭐ NEGATIVE means net cash: the company holds more cash than debt.' },
  cash_ratio: { label: 'Cash Ratio', fmt: num(2),
    desc: 'Cash and equivalents against current liabilities — the strictest liquidity test there is, counting only money already in the bank.' },
  asset_turnover: { label: 'Asset Turnover', fmt: num(2),
    desc: 'Revenue divided by total assets — how much sales each dollar of assets produces. Structurally low for banks and utilities, high for retail; compare within an industry.' },
  capex_to_revenue: { label: 'Capex / Revenue', fmt: pctPlain(1),
    desc: 'Capital spending as a percentage of revenue — how asset-hungry the business is. A rising number means growth is being bought rather than earned.' },
  rnd_to_revenue: { label: 'R&D / Revenue', fmt: pctPlain(1),
    desc: 'Research and development as a percentage of revenue. ⚠️ Blank for most of the market rather than 0, because the provider writes 0 both for a company that reports no R&D line and for one it could not compute — and those are different facts. Populated for roughly a third of the universe.' },
  sbc_to_revenue: { label: 'SBC / Revenue', fmt: pctPlain(2),
    desc: 'Stock-based compensation as a percentage of revenue — the cost of paying staff in shares, which never appears in cash flow but dilutes you all the same. High single digits is normal in software, near zero in industrials.' },
  cash_conversion_cycle: { label: 'Cash Conversion Cycle', fmt: num(0),
    desc: 'Days between paying for inventory and collecting from customers. ⭐ NEGATIVE is a competitive advantage, not an error: it means customers pay before suppliers are paid, so growth funds itself.' },

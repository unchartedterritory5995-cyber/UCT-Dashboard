...
describe('⭐ the transition, and only the transition', () => {
  ...
  describe('THE DENOMINATOR COUNTS THE ACTIVE LAYER, NOT THE LITERAL KEY', () => {
    // ROLLED BACK to flag-false semantics. The SOURCE condition is unchanged -
    // it still asks `offlineEnabled()` rather than `key === '1'`. What changed
    // is what an unset key resolves TO, so the layer is reached here by an
    // explicit '1' instead of by the default.
    it('an unset key is INACTIVE while the default is off ⇒ never reports', async () => {
      const s = store({})
      const post = vi.fn(async () => {})
      expect(shouldReportOptIn(s)).toBe(false)
      expect(await reportOptIn({ storage: s, post })).toBeNull()
    ...
    it('an ACTIVE browser reports ONCE, and a second and third load do not re-report', async () => {
      // The dedupe property is flag-independent and is pinned either way: the
      // marker stores the RESOLVED state, so it cannot be cleared by the key
      // being absent.
      const s = store({ [OFFLINE_FLAG_KEY]: '1' })
      const post = vi.fn(async () => {})
      const props = await reportOptIn({ storage: s, post })
...
describe('⭐⭐ THE WIRE — the Notebook actually reports it', () => {
  ...
  it('a DEFAULT browser (key unset) reports nothing — the layer is OFF again', async () => {
    // ROLLED BACK. With OFFLINE_DEFAULT_ON false an unset key resolves to
    // INACTIVE, so the denominator correctly counts nobody: there is nobody to
    // count. The condition in the source is unchanged - it asks the layer, and
    // the layer is off.
    await mountTab()

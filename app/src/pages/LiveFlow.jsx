...
// ─── WebSocket real-time streaming ────────────────────────────────
  ...
  useEffect(() => {
    ...
    fetch(csvFile)
        .then(res => {
          console.log(`[perf] Live fetch: ${(performance.now()-t0).toFixed(0)}ms`);
          if (!res.ok) throw new Error(`UW API returned ${res.status}`);
          return res.text();
        })
        .then(text => {
          if (cancelled) return;
          ...
          if (!trimmed.includes(",") || trimmed.length < 50) {
            throw new Error("No live flow data — market may be closed (9:30 AM – 4:00 PM ET).");
          }
          setTimeout(() => {
            if (cancelled) return;
            try {
              ...
              console.log(`[perf] Parsed: ${(performance.now()-t1).toFixed(0)}ms (${rows.length} rows)`);
              if (!rows || rows.length === 0) throw new Error("No live flow data — market may be closed.");
              ...
              if (!cancelled) { setD(data); setCsvLoading(false); setLastRefresh(new Date()); }
            } catch(err) {
  ...
  if (csvError) return (
    <div style={{background:"#06090f",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'JetBrains Mono',monospace"}}>
      <div style={{textAlign:"center",maxWidth:400}}>
        ...
        <div style={{color:"#4a5c73",fontSize:11,marginBottom:16}}>Market hours: 9:30 AM – 4:00 PM ET. Flow alerts may be limited outside market hours.</div>
        <div style={{display:"flex",gap:8,justifyContent:"center"}}>
          <button onClick={()=>setRefreshKey(k=>k+1)} style={{background:"#1a2540",color:"#00e676",border:"1px solid #243352",borderRadius:6,padding:"8px 20px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>⟳ Retry</button>
          <a href="/options-flow" style={{background:"transparent",color:"#4a5c73",border:"1px solid #243352",borderRadius:6,padding:"8px 20px",fontSize:12,textDecoration:"none",fontFamily:"inherit"}}>← CSV Dashboard</a>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ maxWidth:1280, margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:4, padding:"10px 16px", background:P.cd, borderRadius:10, border:"1px solid "+P.bd }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <span style={{ fontSize:10, color:P.dm, background:P.al, padding:"3px 10px", borderRadius:4 }}>
              Unusual Whales API · {D.confirmedCount} confirmed of {D.totalTrades} trades
            </span>
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            <a href="/options-flow" style={{ padding:"5px 14px", borderRadius:5, border:"1px solid "+P.bd,
              fontSize:10, fontWeight:700, fontFamily:"inherit", background:"transparent", color:P.mt, textDecoration:"none" }}>
              ← CSV Dashboard
            </a>
          </div>
        </div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:4 }}>
          ...
          <h1 style={{ fontSize:16, fontWeight:800, margin:0, color:P.wh }}>MARKET READ</h1>
        ...
        <div style={{ marginTop:16, padding:"10px 0", borderTop:"1px solid "+P.bd, display:"flex", justifyContent:"space-between" }}>
          <span style={{ fontSize:9, color:P.mt }}>⚡ Live Flow Dashboard · Unusual Whales API · {D.dateRange}</span>

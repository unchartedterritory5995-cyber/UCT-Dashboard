...
export default function OptionsFlowDashboard() {
  ...
  // hydrate and re-run the entire aggregation just to refresh an ER badge.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // other — fetch them CONCURRENTLY. Serially this was 3 round trips
        // (~403ms) which raced the CSV pipeline; whenever erSoonSet landed after
        // parsedRows it re-ran the ENTIRE 96k-row processFlowData (~1,351ms) to
        // refresh an ER badge. (Still null on failure: the row-flag fallback is
        // the documented behaviour, and settling to an empty Set would itself
...
// Fetch DB version on mount + when tab regains focus (so a fresh upload in
  ...
  useEffect(() => {
    ...
    // A version-driven refresh MUST carry the version in the URL. Once
    // Cloudflare caches this endpoint, cache:"no-store" is not enough — it is
    // only a REQUEST directive and CF deliberately ignores client no-cache, so
    // an unversioned refetch is answered from the edge with the very bytes it

...
import { shortOf, drillKeyOf, WEEKLY_METRICS } from './chartMetrics'
...
/**
 * A tile's NAME and DRILL KEY come from the registry (`chartMetrics.METRIC_META`); the
 * tier and format functions below — the behaviour that is genuinely this widget's — stay
 * here (R1, D-011).
 *
 * ⛔ Do not type a `label:` or a `drillKey:` into a tile again. Two registries is how the
 * chart lost "(MA Stack)" from Stage 2 while the heatmap kept it (audit A-34). The rail in
 * `heatmapRegistry.golden.test.js` fails the moment this file carries a name of its own.
 */
const named = (t) => {
  if (t.isHeader) return t                       // section captions are this file's own
  const drillKey = drillKeyOf(t.key)
  return { ...t, label: shortOf(t.key), ...(drillKey ? { drillKey } : {}) }
}

const TILE_DEFS = [
  { key: '__h_score',   label: 'SCORE',           isHeader: true, group: 'Score' },
  { key: 'breadth_score',         group: 'Score',
  ...
  { key: 'mcclellan_osc',       group: 'Regime',
  ...
  { key: 'cnn_fear_greed',        group: 'Sentiment',
...
export const FFILL_KEYS = [...WEEKLY_METRICS]

...
// ─── TOP 10 FLOW PICKS — candidate construction ─────────────────────────────
// Lifted VERBATIM out of OptionsFlow.jsx (the "TOP 10 FLOW PICKS" IIFE, source
// lines 5424-5611 as of 2026-09-07). Nothing about the algorithm changed in the
// move: same passes, same gates, same ordering, same tie-breaks. Only the four
// closed-over values became parameters.
//
// WHY: this is the ONLY first-paint reader of `all_directional` (5.01 MB /
// 12,893 rows) and `all_trades` (11.43 MB / 29,514 rows). Those two arrays are
// shipped to every member on every load to produce, in the end, ten rows. See
// flowBootstrap.js for the consumption audit.
//
// ⛔⛔ FILTER-THEN-AGGREGATE IS LOAD-BEARING — DO NOT "SIMPLIFY" IT.
// `ad` filters ROWS by capBand(t.mktcap) BEFORE tkMap accumulates, while tkMap
// separately UPGRADES a ticker's mktcap when a real value shows up ("prevents
// gap-fill rows (mktcap=0 -> Unknown) from locking a ticker's cap band"). A
// ticker with a mix of gap-fill and real rows therefore lands in a DIFFERENT
// band depending on which happens first. Aggregating once and filtering the
// result afterwards would be smaller, cleaner, and would silently change which
// tickers appear under a cap filter. It is not equivalent and it is not an
// optimisation.
//
// ⛔ `isEtfFn` is a PREDICATE PARAMETER, not a lookup table, because the page's
// version closes over `remoteETFSet` — fetched at runtime from
// /api/ticker-types/etf-index-symbols — plus two local override lists. Any
// server-side caller must reproduce that classification exactly or the two
// sides will disagree about which tickers belong to the Stocks tab.
//
// `includeStandout` preserves the page's laziness: the Standout view rebuilds
// candidates at CONTRACT level and is built only when that filter is selected.
//

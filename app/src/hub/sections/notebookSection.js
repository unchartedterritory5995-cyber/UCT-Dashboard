...
export default function useNotebookSection() {
  ...
  const config = useMemo(() => {
    ...
    return {
       ...
       * narrate is invisible" — and this controller shipped a scrub with no readout since §3.7.
       * It never went red because this section's own suite mounted the hook with NO `HubProvider`,
       * so `useHubMode` reached `HubContext`'s no-op default and `registerHubMode` — the single
       * place that validates — was never called. The check ran for the first time when R-17's rail
       * mounted the provider. The failure in production is silent by design: `report()` only
       * THROWS in dev, so a member scrubbing the note grid saw a chip that said nothing.
       *
       * ⭐ TITLE WHEN THE CARD GIVES ONE, POSITION ALWAYS — and the fallback is `notebookReadout`
       * rather than a second inline copy of "n of m". Two streams wrote a readout for this
       * controller on separate branches: one named the note, one counted position and argued
       * against scraping a card. Both are right about different halves, so the title is used for
       * DISPLAY only (it identifies nothing and acts on nothing — identity is `data-note-card-id`,
       * per R-18) and the count is the floor underneath it.
       */
      readout: () => {
        const at = index
        if (count <= 0 || at < 0) return null
        const title = noteLabelFromCard(noteCardNodes()[at])
        return title ? { label: title, value: `${at + 1} of ${count}` } : notebookReadout({ index, count })
      },
      fan: mode.fan.flatMap((action) => {
        ...
        if (action.id === 'notebook.voiceNote') {
          // D-17. The label carries the state, for the same reason `calendar.macro`'s does.
  ...
  return {
    ...
    voiceMsg,
    recording,
    // ⛔ ONE HOST, TWO WRITERS. D-17's voice messages and R-17/R-19's write refusals both land
    // here. `msg ?? voiceMsg` gives the write channel precedence, and the two are mutually
    // exclusive gestures in practice. ⛔ NEVER gate this on "is there a message" — `JournalToast`
    // is a permanent `role="status"` whose TEXT toggles, and a chip mounted only when there is
    // something to say is silent to a screen reader. The gate is the ROUTE, which is a different
    // question.
    hubMount: onRoute
      ? createElement(JournalToast, { key: 'notebook-toast', msg: msg ?? voiceMsg, style: TOAST_STYLE })
      : null,
  }
}

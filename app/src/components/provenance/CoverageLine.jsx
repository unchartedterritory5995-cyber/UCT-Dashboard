 ...
 * change: every current consumer (`ScanResults.jsx`, `EvidenceTab.jsx`)
 * already renders this the same way (below a result set), and PRD-S8 §9.1
 * requires the receipt stay "always visible, never collapsed behind a
 * toggle" regardless of density. Threaded through as a data attribute so a
 * future dense-grid caller can style off it; no rendering branch keyed on it
 * exists yet, and none is invented here ahead of a real dense-grid consumer.
 */
export default function CoverageLine({ coverage = null, max = 12, maxCauses = 4, density = 'page' }) {

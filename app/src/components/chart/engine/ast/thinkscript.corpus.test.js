...
// ——— ⚰️⚰️ THE REGENERATOR THIS FILE HAS BEEN PROMISING —————————————————
//
// This file's header said the fixture is "regenerated by a `TS_CORPUS_WRITE=1`
// run and reviewed in the diff", and the missing-entry failure told a reader to
// "regenerate with TS_CORPUS_WRITE=1 and read the diff". **Nothing in this file
// read that variable and nothing wrote the fixture.** The instruction was
// printed at the exact moment somebody needed it and could not be followed
// ⇒ `lesson_a_comment_naming_a_mechanism_is_a_claim_about_a_run`.
//
// ⭐ THE PINE LANE ALREADY HAD ONE, and it had already paid for two lessons that
// are inherited here rather than rediscovered:
//   1. THE WRITE GOES ABOVE THE ASSERTIONS. Below them it could only ever run
//      when the fixture ALREADY matched — a regenerator that works exactly when
//      you do not need it and throws in the only case where you do.
//   2. OPTIONAL KEYS ARE OMITTED, NOT WRITTEN AS null/0, because that is what the
//      committed fixture does. A writer that "helpfully" spells them out rewrites
//      every entry on its first run while changing no measurement at all.
//
// ⛔ AND THE CONTROL IS THE POINT: this regenerates IN MEMORY on every ordinary
// run and asserts a clean no-op, so the day the writer and the committed shape
// diverge THIS fails — rather than the next regeneration silently reshaping the
// file.
...
const measure = (f) => {
  ...
  const out = {
    ...
    ignoredLines: got.ignored.map((n) => n.line),
    folded: got.folded.map((x) => `${x.name}=${x.folded}`),
    perOutputRefusals: perOutput,
  }
  // ⚠️ `downstream` is the SAVE-GATE verdict and is NOT re-derived here: this
  // block owns the SHAPE of the fixture, not the meaning of a field the cases
  // above already own. A new entry therefore arrives as `null` and has to be
  // walked through the real doors by hand — deliberately.
  // ⛔ SPELLED OUT AS `null`, NOT OMITTED — which is the OPPOSITE of what the
  // Pine fixture does, and the reason the two writers are not shared. Each one
  // owns the shape of ITS committed file; guessing that both lanes made the same
  // call would have rewritten every entry here on the first run.
  out.refusal = got.refusal ? {
    guard: got.refusal.guard,
    line: got.refusal.line,
    column: got.refusal.column,
    token: got.refusal.token,
  } : null
  if (want && want.downstream !== undefined) out.downstream = want.downstream
  return out
}
...
describe('the snapshot can be REGENERATED, and regenerating changes nothing', () => {
  it('⭐⭐ a regeneration is a NO-OP against the committed fixture', () => {
    ...
    // ⛔ `_translating` AND `_columns` ARE SUMS OF THE ENTRIES ABOVE, so the
    // writer DERIVES them. Copying them through would let a regeneration leave a
    // roll-up describing the previous run — which is the exact "hand-edited
    // snapshot" this fixture's roll-ups exist to detect, reintroduced by the tool
    // meant to prevent it.
    // ⭐ THE OTHER THREE ARE COPIED, ON PURPOSE. `_saveable` and `_blocked` are
    // save-gate verdicts and `_atLineOne` is compared against a live run; this
    // block owns the SHAPE of the fixture, not fields the cases above own.
    for (const k of Object.keys(SNAPSHOT)) {
      if (k.startsWith('_')) regenerated[k] = SNAPSHOT[k]
    }
    regenerated._translating = FILES.filter((f) => regenerated[f].translates).length
    regenerated._columns = FILES.reduce((n, f) => n + regenerated[f].usable, 0)
    if (process.env.TS_CORPUS_WRITE) {
      // ⛔ THE COMMITTED FILE'S OWN ORDER AND INDENT, both of which this writer
      // got wrong on its first run — files first, rollup keys last, two spaces.
      // The control caught it as a 655-line rewrite that changed no measurement,
      // which is precisely the motion it exists to prevent.
      const ordered = {}
      for (const f of FILES) ordered[f] = regenerated[f]
      for (const k of Object.keys(SNAPSHOT)) if (k.startsWith('_')) ordered[k] = regenerated[k]
      fs.writeFileSync(path.resolve(__dirname, '__fixtures__/thinkscriptCorpus.json'),
        `${JSON.stringify(ordered, null, 2)}
`)
    }
    for (const f of FILES) {
      expect(regenerated[f], `${f} regenerates differently than it is committed`)
        .toEqual(SNAPSHOT[f])
    }
  })
})
...
describe('the whole corpus, in one number', () => {
  it('reports what fraction of real thinkScript this engine can run', () => {
    ...
    // ⚰️⚰️ 34 → 23 ON 2026-08-30, AND ELEVEN OF THE THIRTY-FOUR NEVER EXISTED.
    // This module computed `readsTheBar` TWICE — in `chooseOutput` and beside
    // `usable` — and hardcoded `hidden: false` onto every row in between. So the
    // fact was known here and never left the file: every consumer that reads
    // `hidden` (this corpus, the door scorecard, and `PineBox`, which is the
    // thing a member actually looks at) was told `false` about a horizontal
    // guide line.
    //
    // MEASURED, the eleven: `30`/`70`/`50` on 04's RSI, `0`/`2`/`-2` on 08's
    // z-score bands, `74`/`26` on 11's MFI, `0` on 20, and one apiece on 17 and
    // 24 — both of which REFUSE, so those two were orphan columns no member could
    // ever reach anyway. Each is the same number on every bar and every symbol.
    // This file's own comment beside `usable` already said counting them "made
    // two corpus scripts report as working on a zero line".
    // ⭐ `pine.js` stamps the identical flag for the identical reason and has
    // since it learned it. The predicate did not change here; the ANSWER moved
    // onto the row, where every reader already looks
    // (`lesson_rail_the_mirror_not_just_the_lane`).
    expect(columns, 'columns this engine computes').toBe(23)

...
describe('a4 — a retired loop form refuses AT ITS OWN LINE', () => {
  ...
  //   multi-timeframe-supply-demand-zones__a98a…   :264   pine:objects-only
  ...
  // ⚠⚠ THESE FOUR PINS ARE A MERGE MEASUREMENT, NOT EITHER BRANCH'S.
  // Two waves landed the same day and each re-pinned this table from ITS OWN
  // tree, so both were right in isolation and NEITHER set was right together:
  //   · postfix member access removed `pine:character` entirely, which moved
  //     `ict-killzones` and `volume-footprint` onto later refusals;
  //   · the object reader learning to collect a create written inside a
  //     collection call moved `multi-timeframe-supply-demand-zones` the OTHER
  //     way, from `pine:no-output` to `pine:objects-only`.
  // Taking either branch wholesale would have shipped a wrong pin, and the
  // first resolver I wrote dropped BOTH sides of the hunk — which made this
  // case pass while silently checking one script instead of four. Re-measured
  // on the merged tree and set from that run.
  //

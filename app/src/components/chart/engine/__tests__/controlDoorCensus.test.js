...
describe('the control-door census — how many doors, and whether an eighth exists', () => {
  ...
  it('⭐ door EIGHT — the per-INSTANCE door has exactly the callers this census names', () => {
    ...
    const LEDGER = [
      ...
      ['app/src/testing/scan/scanHarness.jsx',
       '⭐ THE ATOMIC-HANDOFF LIVE HARNESS — the FOURTH dev-only probe, ledgered '
       + 'for exactly the reason its three siblings above are: this census asks who '
       + 'can say "which copy", not who ships. ⚰️ IT WAS A CALLER FOR TWO DAYS '
       + 'BEFORE IT WAS LEDGERED — `scanHarness.jsx` landed 2026-09-21 and the next '
       + 'edit of this file (2026-09-23) ledgered the FUNDAMENTALS harness and walked '
       + 'past this one, so the case was red on its own branch. That is the census '
       + 'working: an unledgered caller is exactly what it is built to notice, and '
       + 'the remedy it asks for is this row, not a shorter list. '
       + '⭐ IT USES DOOR EIGHT UNDER `?studies=1` ONLY, to seed this widget with its own '
       + '`opts.settings` with an own-pane RSI and a multi-output MACD (`addInstance` '
       + '× 2) so every legend surface — Bar Info, the study stack, the volume strip '
       + 'and the pane readouts — is on screen at once while the symbol changes '
       + 'underneath them. ⛔ THE SEEDED STUDIES HAVE TO BE MINTED INSTANCES, not a '
       + 'per-definition flip, because the surfaces under measurement render one row '
       + 'per LIVE INSTANCE — seeding them through doors 1-7 would measure a legend '
       + 'the product does not draw. ⛔ IT WRITES NOTHING: ChartWidget holds its own '
       + '`opts`/`onOptsChange` in page state, the workspace is spread from the '
       + 'canonical `WORKSPACE_FALLBACK` rather than a hand-copied shape, and a '
       + '`window.fetch` wrapper REFUSES any non-GET to /api/auth/preferences '

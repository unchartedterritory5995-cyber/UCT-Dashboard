...
describe('watermark settings', () => {
  ...
  it('merges partial user watermark over defaults (back-compat with {visible,opacity}-only)', () => {
    ...
    expect(cs.watermark.lines).toEqual({ ticker: true, company: true, sector: true, industry: true })
  ...
  it('deep-merges partial lines (user disables only ticker)', () => {
    ...
    expect(cs.watermark.lines).toEqual({ ticker: false, company: true, sector: true, industry: true })
  })
})

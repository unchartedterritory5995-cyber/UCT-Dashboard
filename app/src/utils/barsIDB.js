...
//
// Deliberately NOT paired with a CACHE_LOGIC_VERSION bump: a bump
// invalidates every browser's IDB simultaneously, and the resulting
// thundering herd of full-fetches (5000 bars × N mounted charts × M users)
// can saturate SQLite's 2s busy_timeout on the web pod and trigger the
// route's try/except → empty payload → user-visible "every chart broken"
// (verified 2026-05-24 incident, reverted in commit dd5ce85). Without the
// bump, this gap detector heals affected users gradually on each chart
// open instead of all at once. Acceptable trade.

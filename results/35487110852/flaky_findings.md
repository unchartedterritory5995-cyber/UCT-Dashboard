...
| #172 → #173 | no | .gitignore is referenced by 10 source file(s); api/services/indicator_compute.py is referenced by 70 source file(s); app/src/components/StockChart.jsx is referenced by 365 source file(s) |
| #173 → #18 | **UNREADABLE** | .github/workflows/clock-parity-fixture.yml is UNREADABLE at one of the two commits |
...
⛔ **12 of 115 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.

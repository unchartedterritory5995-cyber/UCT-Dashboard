...
| #177 → #178 | no | app/src/components/chart/engine/ast/bothLanesAreTwoLanes.test.js is referenced by 2 source file(s); app/src/components/chart/engine/ast/pine.js is referenced by 350 source file(s); app/src/components/chart/engine/ast/pineObjectOnlyHostAccept.test.js is referenced by 1 source file(s) |
| #178 → #18 | **UNREADABLE** | .github/workflows/clock-parity-fixture.yml is UNREADABLE at one of the two commits |
...
⛔ **12 of 119 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.

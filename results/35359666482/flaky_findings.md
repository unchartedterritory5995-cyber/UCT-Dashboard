...
⛔ **8 of 34 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.
...
## `vitest` · `src/components/research/sections/CatalystsSection.test.jsx`

**CatalystsSection > shows the finding state while catalysts generate and lands on its own (polls)**

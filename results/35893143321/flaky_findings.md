...
| #328 → #33 | **UNREADABLE** | the diff c7bddc046..026a6693f is UNREADABLE (shallow checkout?) |
| #33 → #330 | **UNREADABLE** | the diff 026a6693f..057c085eb is UNREADABLE (shallow checkout?) |
| #330 → #34 | no | api/darkpool_aggregator.py is referenced by 10 source file(s); api/data/canonical_address_book.json is referenced by 7 source file(s); api/data/cap_universe.json is referenced by 129 source file(s) |
...
⛔ **10 of 232 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.

...
| #204 → #205 | no | api/services/indicator_compute.py is referenced by 70 source file(s); app/src/components/chart/engine/ast/clockParity.test.js is referenced by 2 source file(s); app/src/components/chart/engine/ast/closedTable.json is referenced by 164 source file(s) |
| #205 → #21 | **UNREADABLE** | .github/workflows/clock-parity-fixture.yml is UNREADABLE at one of the two commits |
...
⛔ **11 of 141 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.

...
| #222 → #224 | no | .gitignore is referenced by 10 source file(s); api/services/bars_fetch.py is referenced by 147 source file(s); api/services/screener/saved_screens.py is referenced by 13 source file(s) |
| #224 → #23 | **UNREADABLE** | .github/workflows/clock-parity-fixture.yml is UNREADABLE at one of the two commits |
...
⛔ **9 of 158 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.

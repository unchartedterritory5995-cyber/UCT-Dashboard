...
| #280 → #29 | no | .github/workflows/full-suite-report.yml changed OUTSIDE `jobs:` — every job sees that; api/darkpool_aggregator.py is referenced by 10 source file(s); api/data/canonical_address_book.json is referenced by 7 source file(s) |
...
⛔ **9 of 202 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.

...
| #46 → #466 | **UNREADABLE** | .github/workflows/clock-parity-fixture.yml is UNREADABLE at one of the two commits |
| #466 → #467 | **yes** | no changed file can reach a test |
...
⛔ **12 of 295 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.
...
- changed state across: #228→#229, #466→#467, #57→#58, #68→#69
...
- changed state across: #410→#411, #466→#467, #57→#58, #60→#61
- consecutive stable runs since: **1** of 5 needed to leave the set
- ⛔ excluded from NEW while it is here. It is **not fixed**, and it is not re-run: it is a test whose result this suite cannot trust.
...
## `vitest` · `src/pages/journal-2-0/components/notebook/NoteEditorPage.unsentTrash.test.jsx`

**Delete with unsent words (the 404 path) > a note holding unsent words is NOT trashed: the dialog names what is unsent**

...
| #210 → #211 | no | api/services/ast_interpret.py is referenced by 92 source file(s); api/services/ast_lint.py is referenced by 40 source file(s); app/src/components/chart/builder/graphSaveDoor.test.js is referenced by 1 source file(s) |
| #211 → #22 | **UNREADABLE** | .github/workflows/clock-parity-fixture.yml is UNREADABLE at one of the two commits |
...
⛔ **10 of 147 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.

...
| #439 → #440 | **yes** | no changed file can reach a test |
...
⛔ **13 of 283 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.
...
- changed state across: #228→#229, #439→#440, #45→#46, #57→#58, #68→#69
...
## `pytest` · `tests.test_secret_scrub`

**test_no_credential_in_git_history_of_this_branch**
...
## `vitest` · `src/pages/journal-2-0/components/notebook/AskPanel.navNotice.test.jsx`

**AskPanel — the notice is brought into view on touch > on TOUCH, setting a notice scrolls THAT region into view, nearest**

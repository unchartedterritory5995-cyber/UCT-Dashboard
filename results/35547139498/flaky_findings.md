...
| #216 → #217 | no | api/services/indicator_compute.py is referenced by 71 source file(s); app/src/components/chart/engine/ast/__fixtures__/pineCorpus.json is referenced by 6 source file(s); app/src/components/chart/engine/ast/clockParity.test.js is referenced by 2 source file(s) |
| #217 → #22 | **UNREADABLE** | .github/workflows/clock-parity-fixture.yml is UNREADABLE at one of the two commits |
...
⛔ **10 of 152 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.

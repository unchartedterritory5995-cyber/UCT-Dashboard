...
| #192 → #196 | no | api/services/ast_interpret.py is referenced by 89 source file(s); api/services/screener/filters.py is referenced by 367 source file(s); api/services/screener/query.py is referenced by 698 source file(s) |
| #196 → #20 | **UNREADABLE** | .github/workflows/clock-parity-fixture.yml is UNREADABLE at one of the two commits |
...
⛔ **11 of 133 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.

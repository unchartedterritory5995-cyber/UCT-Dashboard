...
| #247 → #25 | no | .github/workflows/full-suite-report.yml changed OUTSIDE `jobs:` — every job sees that; api/darkpool_aggregator.py is referenced by 10 source file(s); api/main.py is referenced by 946 source file(s) |
...
⛔ **10 of 178 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.

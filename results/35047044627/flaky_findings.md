...
⛔ **6 of 16 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.
...
## `pytest` · `tests.test_theme_index_watchlist`

**test_overlay_today_replaces_stale_and_appends_missing**

- changed state across: #26→#27
- consecutive stable runs since: **4** of 5 needed to leave the set
- ⛔ excluded from NEW while it is here. It is **not fixed**, and it is not re-run: it is a test whose result this suite cannot trust.

## `pytest` · `tests.test_ticker_explain.TestRoute`

**test_requires_auth**

- changed state across: #28→#29
...
- changed state across: #19→#20, #21→#22, #22→#23, #28→#29
- consecutive stable runs since: **2** of 5 needed to leave the set
- ⛔ excluded from NEW while it is here. It is **not fixed**, and it is not re-run: it is a test whose result this suite cannot trust.

## `vitest` · `src/context/AuthContext.test.jsx`

**AuthContext fetchUser — transient vs definitive session-check failures > 503 on a REFETCH with a logged-in user → user and plan state PRESERVED**

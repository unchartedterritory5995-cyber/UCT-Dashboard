...
| #261 → #262 | no | app/src/components/NavBar.module.css is referenced by 2 source file(s); app/src/pages/screener/shell/FilterRail.jsx is referenced by 4 source file(s); app/src/pages/screener/shell/ScannerShell.jsx is referenced by 34 source file(s) |
| #262 → #263 | **yes** | no changed file can reach a test |
| #263 → #27 | **UNREADABLE** | .github/workflows/clock-parity-fixture.yml is UNREADABLE at one of the two commits |
...
⛔ **9 of 189 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.
...
- changed state across: #57→#58, #60→#61
- consecutive stable runs since: **1** of 5 needed to leave the set
- ⛔ excluded from NEW while it is here. It is **not fixed**, and it is not re-run: it is a test whose result this suite cannot trust.

...
| #188 → #189 | no | api/routers/screener.py is referenced by 543 source file(s); api/services/screener/bases.py is referenced by 101 source file(s); api/services/screener/query.py is referenced by 698 source file(s) |
| #189 → #19 | **UNREADABLE** | .github/workflows/clock-parity-fixture.yml is UNREADABLE at one of the two commits |
...
⛔ **12 of 129 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.

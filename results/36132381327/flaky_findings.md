...
| #401 → #403 | no | .github/workflows/full-suite-report.yml changed the suite-running job(s) vitest; .gitignore is referenced by 11 source file(s); api/live_massive_router.py is referenced by 44 source file(s) |
| #403 → #41 | **UNREADABLE** | .github/workflows/clock-parity-fixture.yml is UNREADABLE at one of the two commits |
...
⛔ **10 of 264 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.

...
| #453 → #454 | **yes** | no changed file can reach a test |
| #454 → #46 | **UNREADABLE** | .github/workflows/clock-parity-fixture.yml is UNREADABLE at one of the two commits |
...
⛔ **12 of 291 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.

## `pytest` · `tests.test_discord_close_note`

**test_a_note_that_cannot_be_written_still_posts_the_charts**

- changed state across: #266→#267, #267→#269
...
## `pytest` · `tests.test_flow_worker_watch_coverage`

**test_this_branch_does_not_strand_a_change_flow_worker_runs**

- changed state across: #453→#454
...
- changed state across: #228→#229, #57→#58, #68→#69
...
## `pytest` · `tests.test_nb_foreign_commits`

**test_the_tools_own_self_check_passes[args0-0]**
...
- changed state across: #453→#454, #49→#50
- consecutive stable runs since: **1** of 5 needed to leave the set
- ⛔ excluded from NEW while it is here. It is **not fixed**, and it is not re-run: it is a test whose result this suite cannot trust.

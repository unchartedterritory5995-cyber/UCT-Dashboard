...
| #233 → #234 | **yes** | no changed file can reach a test |
| #234 → #24 | **UNREADABLE** | .github/workflows/clock-parity-fixture.yml is UNREADABLE at one of the two commits |
...
⛔ **11 of 167 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.

## `pytest` · `tests.test_discord_close_note`

**test_a_note_that_cannot_be_written_still_posts_the_charts**

- changed state across: #233→#234, #45→#46
...
- changed state across: #228→#229, #233→#234, #45→#46, #57→#58, #68→#69
...
- changed state across: #233→#234, #25→#26, #26→#27, #57→#58, #60→#61
- consecutive stable runs since: **1** of 5 needed to leave the set
- ⛔ excluded from NEW while it is here. It is **not fixed**, and it is not re-run: it is a test whose result this suite cannot trust.

...
| #434 → #435 | no | .github/workflows/full-suite-report.yml changed the suite-running job(s) vitest; .gitignore is referenced by 11 source file(s); api/routers/journal_two.py is referenced by 487 source file(s) |
| #435 → #45 | **UNREADABLE** | .github/workflows/clock-parity-fixture.yml is UNREADABLE at one of the two commits |
...
⛔ **12 of 281 consecutive pairs were comparable.** A pair that is not comparable contributes NO evidence in either direction — it cannot make a test flaky and it cannot clear one.

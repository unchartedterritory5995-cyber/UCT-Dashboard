...
`{"markdown": tiptap_to_markdown(doc)}` on stdout -- or a JSON LIST of documents, answered as
`{"markdowns": [...]}` in the same order (one spawn for a whole rail file). Used by the wave-6 editor
...
def main() -> int:
    ...
    # ⭐ A LIST is the batch form (wave 7 lane J, fix round 1, I-1): every document a rail
    # file needs, exported in ONE spawn -- one census import (~7 s) instead of one per test,
    # which ran each test against the 15 s vitest timeout. A single document (a dict) is
    # still answered as before.
    if isinstance(payload, list):
        answer = {"markdowns": [tiptap_to_markdown(doc) for doc in payload]}
    else:
        answer = {"markdown": tiptap_to_markdown(payload)}

...
Two rectangles per case, plus the ``rest`` the harness computes:

  * ``price_plot`` — everything above the oscillator stack: the candles, the MA
    and price overlays, the volume pane, and the price axis. **This is the
    region the cutover's design claims reads ABSOLUTE 0.**
  * ``osc_strip``  — the oscillator stack the cutover creates.
  * ``rest``       — computed by mask subtraction, never declarable: the time
    axis and the export footer below the pane stack.
...
def read_manifest_for(page, base, case, token, ready_timeout_ms):
    """Load one case on one base and return its pane manifest.
...
def boxes_for(case, man_a, man_b, gated):
    ...
    time_axis = {"name": "time_axis", "box": [0, chart_height, w, h], **exp}

    if n_b == n_a:
        # No stack was created; nothing to split above the axis.
        return [{"name": "price_plot", "box": [0, 0, w, chart_height], **exp},
    ...
    return [
        {"name": "price_plot", "box": [0, 0, w, osc_top], **exp},
        {"name": "osc_strip", "box": [0, osc_top, w, chart_height], **exp},
        time_axis,
    ], chart_height
        ...
        for case in cases:
            ...
            derived[case["name"]] = {
                ...
                "from": (f"gen_parity_regions.py -- bands {ident_a['id']} panes {ident_b['id']}: "
                         f"chartHeight={chart_height} separatorPx={mans['a']['separatorPx']} "

...
1. **Large Evernote notes block the import UI.** Notes of a few hundred KB convert on the browser main thread; five such notes took about 4 minutes with the tab unresponsive and the progress counter apparently stuck. The import completes correctly and loses nothing, but a member with very large notes will think it has hung. Not fixed: this is a UX risk rather than a correctness defect, and off-main-thread conversion is larger than this certification's remit. **Status update: SUPERSEDED by §15 — this issue is CLOSED, not open.** The large-note responsiveness issue was subsequently fixed and re-certified; see §15 for the measured before/after evidence (~4 minutes frozen at 70/75 → 30 seconds, 75 per-note progress updates, max gap 120 ms). The text above is retained as the historical record of what the original certification found.

...
| **S-D extract** ✅ **BUILT on `wisdom/w1-d-extract`** — `tests/test_wisdom_extract_written_records.py`, 18 tests, green; mutation-proved (the bare given name re-typed into `authors.json` **and** dropped from `ambiguous_speaker_labels` turns it RED by name) | **The drift-#4 end-to-end assertion** (owner ruling, checkpoint 3): a session containing an attendee whose Zoom display name is a CALL author's given name produces **ZERO rows** attributed to that author. Asserted over **written records**, not over the resolver. | S-B's regression test asserts at the layer that *decides* authorship, which is the strongest claim available there and says so in the test. S-D owns the record WRITER, so S-D is the only stream that can assert the property the owner actually asked about. ⛔ Recorded here because a gap that lives only in a checkpoint reply is a gap that gets forgotten (`lesson_a_documented_workaround_is_not_a_recovery_path`). |
| **S-D extract** ✅ **BUILT on `wisdom/w1-d-extract`** — `extract/writer.py` derives the inferred ticker from the record's own quote (no contract-schema change, so `extractor_version` and the frozen golden-v1 gate are untouched), sets the three §8a.4 fields, runs the bar-range pass through a new `core.bars.session_range` seam, and stores a not-a-pass as a MENTION with `entity_id` NULL + a review item. Railed in BOTH directions plus an UNCHECKABLE one; six mutants red. ⚠️ With no bar source installed every verdict is `no_bar_source`, which is NOT a pass — the rule is live, not inert | **F6:** `ticker_inferred` has a column but **no writer binds the bar-range pass to it**. §8a.4 requires an inferred ticker to carry `ticker_inferred=1`, `entity_confidence ≤ 0.5`, `extraction_confidence='low'` AND to pass the bar-range sanity check before storage, else be stored as a MENTION with `entity_id` NULL plus a review item. | The column without the writer is a rule that looks implemented and is not. |
| **S-F publish (F2)** ✅ **BUILT on `wisdom/w1-f-publish`** | **Every publish adapter writes a provenance marker on every write**, and **a CI check fails if any adapter code path can write to a consumer table without it** (§8c.3). Shipped as `adapters/provenance.py` (the marker, standard-library only so the PC-side tools load it by path) + `publish/provenance_check.py` (the AST rail: the consumer-table set is DERIVED by subtracting Wisdom's own `MIGRATIONS` tables from what the publish package and publish tools write — never a typed roster), wired into `.github/workflows/wisdom-rails.yml` with `--self-check` ahead of it, and railed by `tests/test_wisdom_publish_provenance.py`. Runtime half: `validate_export` refuses an unmarked KB row; `brainkb.export_payload` drops one and reports it. | Until then the "did anything reach the member-facing tables?" audit is shape-based — it searches for a marker nobody was *required* to write, so a row published with no marker would not be caught. An empty `wisdom_publish_log` is today's argument, not a proof. |
| **S-F publish (F2)** ⚠️ **MEASURED, integrator ticks it at the merge** | The D16a private-store property test **re-runs un-skipped** at the S-F merge. The Definition-of-Done line for the import-ban rails is **not checked** until it passes with the adapters present (§8b.9). | Its member-facing half had never executed — `api.services.wisdom.publish.adapters` did not exist. **Measured 2026-09-13 on `wisdom/w1-f-publish`: `tests/test_wisdom_core_private.py` = 18 passed, 0 skipped**, and `test_property_no_publish_adapter_output_carries_a_private_value` reports PASSED, not SKIPPED — so neither the `find_spec` guard nor the "nothing to probe" guard fired and the half that covers member-facing OUTPUT has now run. ⛔ That is a measurement on this branch, not the tick: the line is checked by the integrator against the merge tip, because the adapters can change between here and there. |

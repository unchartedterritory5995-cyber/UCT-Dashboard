...
| R-08 | **CLOSED — BLOCKED.** `PUT /api/j2/positions/{id}` is an `api/` file. The Railway `worker` service's live watch list is `['/api/**', …]`, so any `api/` edit redeploys `worker`. Not this increment's to touch. **⛔⛔ RE-EXAMINED UNDER THE WIDENED `api/` AUTHORITY (L3) — STAYS CLOSED, with a narrower and more useful reason.** The waiver covers `api/routers/auth.py` and no other `api/` file; all three holes are in `positions.py` (`:347`, `:344`, `:291`), which it does not name. And it should not — the row's own sentence is the real stop: "⛔ **Not ours to fix** — the hub cannot reach any of them, and a hub-side patch would be a second authority over j2's own validation." There is **no zero-`api/` implementation**: every hole is a server-side acceptance rule, and moving any of them client-side creates exactly the second authority the row refuses. **CLOSED — dependency — owned by Journal 2.0, who owns `positions.py`.** ⭐ Carried forward for whoever takes it: **hole 3 is real and was hit in this charter's own work.** `isinstance(True, int)` is `True`, so a `{"holdMs": true}` passed the numeric gate on `POST /api/auth/preferences` and compared as `1`. The fix is `isinstance(val, bool) or not isinstance(val, (int, float))` — **and it must raise a DIFFERENT sentence than the range check.** With one shared message the guard is unprovable: `True` is `1`, `1` is outside the range, so the range branch raises the identical string and deleting the bool test survives green. That mutation was measured surviving before the messages were split (`60cbe8919`). | `railway status --json`; `60cbe8919` |
| R-09 | **CLOSED — RESOLVED.** `run` and `confirm` are dispatched. | `HubRoot.jsx:171`, `:186` |
| R-11 | **CLOSED — BLOCKED + OUTSIDE OWNER.** `OptionsBoard.jsx` lives under `app/src/pages/journal-2-0/**`, which this build does not edit; the request itself names Journal 2.0 as owner and says "**Blocking:** no". **⛔⛔ RE-EXAMINED UNDER THE WIDENED AUTHORITY (L3) — STAYS CLOSED, UNCHANGED, and the widening does not touch it at all.** What L3 widened was `api/`; the standing zero-edit rule on `app/src/pages/journal-2-0/**` is a different rule and was not widened. Nothing in this row needs `api/`, so there is no re-measure to do on the file list. The request settles the rest itself: "**Owner:** Journal 2.0. **Blocking:** no", and the gap is a COVERAGE gap on rows every `requires:'position'` action is disabled on anyway. **CLOSED — product decision — Journal 2.0's owner takes the one line, `data-hub-pos={String(strategy.id)}`, when they are next in that file.** ⭐ It is recorded in `journalSection.js`'s own header, which is where a future reader actually meets it — not only here. | file path; section header |

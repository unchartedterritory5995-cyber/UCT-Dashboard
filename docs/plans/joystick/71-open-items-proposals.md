...
> ⚰️ **CITE `d899489124`, NOT THE BRANCH SHA.** PR #108 was **squash-merged**, so its branch commit
> `3607b6796` is reachable only from `origin/launch/queued-fixes-pr` and is **not an ancestor of
> master** (`git branch -r --contains`). A ledger citing it would send the next reader to a commit
> `git show` cannot resolve from master — the "citation you cannot quote" defect this programme has a
> standing rule about. The squash commit `d899489124` is the one that carries the work.
...
|---|---|---|---|
| ~~**1**~~ ✅ | ~~**`escalateCue(action)`**~~ — **SHIPPED `d899489124` (PR #108).** `app/src/hub/escalateCue.js` is the one authority and BOTH doors call it — `useJoystick.js:18,280` (gesture) and `HubActionsButton.jsx:10,191` (sheet). `ESCALATE_CUE_MS = 180`, held as a static class and removed on a JS timer. ⭐ The visual fires ONLY when the action escalates AND the haptic returned false: *"a device that CAN buzz has already said the thing."* The non-escalating path lives inside the same function on purpose — otherwise every caller keeps its own `else haptics.impact()` and the two copies simply move one line down. Rail `escalateCue.test.js`. — verified on master by `git show`, 2026-09-13. | | §3 above | The escalate branch exists TWICE today (`useJoystick.js` for the gesture door, `HubActionsButton.jsx` for the sheet door). This is the only row that FIXES something members can feel the absence of: `haptics.warn()` returns false wherever `navigator.vibrate` is missing, which is **every iPhone**, so the commit cue for destructive actions is silently absent on iOS. ⛔ Must be a JS-timed static class, never a CSS animation: `tokens.css` zeroes `animation-duration` and `transition-duration` app-wide under `prefers-reduced-motion`, which would delete the cue for exactly the members most likely to need a non-haptic one. |
| ~~**2**~~ ✅ | ~~**A haptic on `HubActionsButton`'s WCAG 2.5.1 path**~~ — **SHIPPED `d899489124` (PR #108),** and it did ride rank 1: the sheet door is now a CALL SITE, not a second implementation. Its own comment records why this door mattered most — it is the ONLY door a VoiceOver or TalkBack member has (both screen readers eat the two-finger Peek), it once fired nothing at all, and `haptics.warn()` returns false on every iPhone, so an accessible-door member got no escalation whatsoever on the most destructive actions in the hub. — verified on master by `git show`, 2026-09-13. | | this file | Pre-existing, and it rides rank 1 for free: once both doors call one `escalateCue`, the sheet door's missing cue is a call site, not a second implementation. Doing it FIRST would mean writing the third copy of a branch that already exists twice. |
| ~~**3**~~ ✅ | ~~**`scan.flag` commits with no sheet**~~ — **RULED AND SHIPPED `d899489124` (PR #108).** The decision went the documentation way and the exception is now DECLARED rather than undocumented: `registry.js:114-123`. ⭐ The §C2 obligation — *"a way to get back out"* — is delivered by an **Undo on the flag toast** instead of a confirm sheet, so there is no second definition of what unflagging means. ⛔ The Undo is a real control, not a gesture: `screenerSection.js:427-436` renders it into the toast, because *"an undo only a thumb can press is not an undo"* — keyboard and every screen reader reach it. Rendered-text rail `flagUndoAndButtonCue.test.jsx`. — verified on master by `git show`, 2026-09-13. | | this file | ⭐ **Decide before building.** It is either a documentation fix (flag is reversible, one tap, and a sheet would be friction on the most-used action) or a product change (§C2 says a commit gets a sheet). The wrong half of this is cheap to build and expensive to ship — it would put a confirmation in front of a gesture members use constantly. Bring the ruling, not a patch. |
...
## ~~The hub Actions button has no `data-testid`~~ — ✅ DONE 2026-09-13
...
### ✅ DONE 2026-09-13 — and the fallback is DRIVEN, not assumed

`HubActionsButton.jsx` now carries `data-testid="hub-actions"` on the button element itself, and `tools/hub_chip_clearance.py` selects on it.

⭐ **The label match is KEPT as a fallback, and the probe reports which one answered** (`btnBy: 'testid' | 'label' | null`). A silent fallback would let this sweep run green against a bundle that predates the hook while the operator believed the hardening was being exercised — so a production run that falls back prints a ⚠️ naming every mode and width it happened on, and says in the same breath that the geometry is still real.

⛔ **Three rails, and they fail for different reasons** (`hubComponents.test.jsx`): the hook is on the BUTTON and not a wrapper (a hook on a wrapper measures the wrong box); the hook and the accessible name resolve to the SAME element, found two different ways, so an instrument and a screen-reader user can never be talking about different controls; and the glass sweep selects on that exact literal, matched as a CALL SHAPE rather than a bare string — that file explains the hook in its own comments, and a substring search would match its own explanation, which is this repo's most-repeated instrument defect. The third carries a control proving it can fail.

⭐ **The fixture control now drives BOTH selectors**: the same boxes with the hook present (`btnBy=testid`) and with it removed (`btnBy=label`), each asserting which selector answered. A fallback nobody exercises is a fallback nobody knows is broken. Measured: `fixture[clear] btnBy=testid -> PASS` · `fixture[overlapping] btnBy=testid -> FAIL` · `fixture[clear-no-testid] btnBy=label -> PASS`.

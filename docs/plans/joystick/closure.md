...
| id | what | owner |
|---|---|---|
| **G3-18 / D-39** | The chip is partly covered by page-level fixed furniture — Journal's "Log a trade" FAB, on **journal and notebook at 360/375/430**. ⚰️ **The "Breadth span at 360" is WITHDRAWN 2026-09-13:** re-measured on `58c63b806` with the sweep waiting the ~9.3s cinematic intro overlay out, breadth @360 reads clear — the "span" was one of the intro's anonymous capability pills. 7 failing pairs → **6**. Cosmetic-plus, **non-blocking** by ruling. Fix is hub-side, never a `journal-2-0` edit. | joystick |

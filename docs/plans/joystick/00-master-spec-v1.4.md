...
## Master build plan, v1.4 (Wave 0, Wave 0.5 and Phase 1 gate decisions applied)

Supersedes v1.3. Every change is a gate decision or a correction forced by
...
- **Additive by default, with SEVEN named exceptions — this list is the single authority.** No
  ...
  Any change beyond these seven stops and asks. Existing navigation keeps working with the hub disabled.
...
- **Rollout: `hub.enabled` — RESOLVED (Wave 0.5 gate), wording TIGHTENED at the Phase 1 gate.** Exact
  ...
  > `role === 'admin'`; otherwise disabled. Founder rollout flips the unset-default per tier in a
  > later wave.

  There is no founder tier and no tier ladder at all today: one product, one price, so the flag is
  **not** a tier gate — `role === 'admin'` is the only identity this codebase currently distinguishes
  from a member, and the "per tier" language above is reserved for a wave that has not shipped. An

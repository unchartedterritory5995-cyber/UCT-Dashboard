...
| G3-3 | **Peek (two fingers)** | Two fingers down, both up. | The Peek sheet opens exactly once. A single-finger tap still taps and does NOT peek. | ⛔ **RETIRED 2026-09-12** — two-finger Peek was REMOVED by ruling (screen readers consume two-finger tap; two pointers fails WCAG 2.5.1). Obsolete, not failing. |
...
| G3-12 | **The range input (no-drag scrub)** | Open Peek, use the range control instead of dragging. | It moves the same value the drag moves, and reads out the section's own readout. A member who cannot drag can still scrub. | ⛔ **RETIRED AS WRITTEN 2026-09-12** — it names Peek as its door and Peek is gone. The no-drag scrub still exists via the Actions sheet; rewrite the row against that door before running it. |

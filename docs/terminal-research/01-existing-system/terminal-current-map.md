...
| 9 | **Two** headless screenshot consumers of the render route: Morning Wire → Substack (`morning-wire/substack/panelshot.py:43`) **and** `uct-sunday-scan/sunday_scan/panels.py:5` | headless browser → `/r/calendar?w=900&from=today&days=5` | **yes**, via the render route |
...
✅ **RECONCILED WITH D-08 2026-09-25 — this is CP-01's one remaining item, and the two documents now agree.** `CRITICAL_PATH.md` CP-01 carried *"remaining: reconcile the `/r/calendar` consumer note between D-08 and D-09"*. Read side by side, D-08 (`10-roadmap/coexistence-current-mechanisms.md` §4.4) and this section never disagreed on the **negative** claim — both say `/r/calendar` is NOT used by the chart-renderer service, and §4.4 confirms it by grep of `services/chart_renderer/` (which references `/r/chart` only). They disagreed on the **positive** one: §4.4 names **two** screenshot consumers, `morning-wire/substack/panelshot.py:3` and `uct-sunday-scan/sunday_scan/panels.py:5`, while §6.1 row 9 and the sentence below named only morning-wire. The second consumer is now named in both places, so the count is unchanged (9 consumers, 8 of them non-`/calendar` — row 1 IS `/calendar`, which is why those two numbers differ by one and both are correct).

Its consumers are **morning-wire and uct-sunday-scan**, not the chart-renderer service: `morning-wire/substack/panelshot.py:43` navigates a headless browser to `DASHBOARD_URL/r/calendar` with `{w: 900, from: 'today', days: 5}`. **No reference to `/r/calendar` or to the calendar was found anywhere under `services/chart_renderer/`.**

...
* **Git history** — this contract does not name the read-only git exceptions, so no `git log`/`show`/`blame` was run. Commit SHAs quoted anywhere above (`e86ad6d5`, `2908ab227`, `021b4926`, `d26cee0c`, `ed53f9b6`) are transcribed from source comments and `CLAUDE.md`, unverified.

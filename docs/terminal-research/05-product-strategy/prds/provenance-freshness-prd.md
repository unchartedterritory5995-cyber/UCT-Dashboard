...
status: all A-READY-NOW work IMPLEMENTED 2026-09-02 (§0a), including <Cited>'s narrow interim form; S10 and the vendor-side entitlement taxonomy (SPEC-S8 §17a) both formally DEFERRED — neither blocking; S11 IMPLEMENTED 2026-09-03 (a separate system, see product-architecture.md's S11 block), completing S8's own deferred session_stale (§9.6); awaiting owner sign-off on S8's overall completion status
date: 2026-09-02
...
## 0a. S8 readiness review, 2026-09-02

This PRD was re-checked against the now-accepted D1 (Provider Abstraction Layer) and its same-day provenance/freshness hardening pass. Full findings index and technical detail: SPEC-S8 §0a. Product-level summary: S8's boundary held exactly as designed — D1's hardening pass added real backend computation (evidence-based freshness, a vendor-entitlement distinction, observation timestamps) with zero change required to S8's rendering contract, confirming the D6 boundary in practice, not just on paper. The one material item this review found — **§7.2's freshness-class enum is no longer a complete description of what a real producer emits** (D1 now emits a 5th value, `stale`, for a different reason than this PRD's own session-aware `stale` render state, §9.6) — is **RESOLVED by owner decision, 2026-09-02**: see §7.2a. Two further open questions remain, not blocking: whether Entity Master (already shipped) already fulfills what this program calls "S3" (SPEC-S8 §8a — confirmed non-blocking for Step 1), and whether the 7-state taxonomy (§6/§9) needs an 8th state for a vendor-side (not member-side) entitlement gap D1 can now report (SPEC-S8 §17a — owner ruled entitlement stays orthogonal to freshness regardless).

**S8 STEP 1 — IMPLEMENTED 2026-09-02.** Authorized against this review plus the owner's freshness-naming and entitlement-orthogonality rulings. `<CoverageLine>` relocated with a re-export shim; `<FreshnessBadge>` and `<Provenance>`'s degraded state built per §19 Step 1. Full detail: SPEC-S8 §0a/§19.

**S8 STEP 2 (S10-independent subset) — IMPLEMENTED 2026-09-02.** A direct search confirmed S10 (Presentation Primitives) has no PRD/spec and no shipped shared formatter — it is itself not-yet-built infrastructure, not a hidden dependency this review missed — so nothing here waited on it: a small, explicitly-local formatting helper covers the interim, exactly as §19 Step 1 already sanctioned. Delivered: a real, live D1→S8 wire (`GET /api/provenance/quote` + `pages/ProvenanceDemo.jsx` at `/provenance-demo`), the orthogonal AVAILABILITY axis (not-found/entitlement-denied/provider-error, never folded into freshness), and an accessible detail disclosure. The Entity Master/"S3" question (§8a) is now CONFIRMED CLOSED for S8's purposes — `product-architecture.md`'s own S3 system block is titled "Entity Master." Full detail: SPEC-S8 §0a/§19.

**S8 CONTINUATION — ALL A-READY-NOW WORK COMPLETE, 2026-09-02.** S10 formally DEFERRED (not blocking); the §17a entitlement-taxonomy question formally DEFERRED/OPEN (not blocking, no broader taxonomy invented). `<Cited row=…>`'s narrow interim form (§4.5/§6) is built and live-wired against real `bar_provenance.py` data. Remaining requirements are all classified B/S10-dependent, C/taxonomy-dependent (none found), or D/dependent on S11 or D2 or a whole-codebase adoption rollout that is explicitly other owners' follow-on work per §19 Step 5 — none built this pass. One real defect was caught and fixed by inspecting the actual rendered page rather than trusting unit tests alone. Full detail: SPEC-S8 §0a/§19.

**S11 SHIPPED, §9.6 SESSION_STALE NOW REAL — 2026-09-03.** S11 (Session & Market Clock, a separate system per the owner's explicit split — S8 never redefined to absorb it) is implemented: `app/src/lib/marketClock/`, a real NYSE 2026 holiday/early-close calendar behind the exact `sessionState`/`nextBoundary`/`isHalfDay`/`asOfLabel` contract product-architecture.md's S11 block specifies. §9.6's session-aware staleness — the one item this PRD's §12.4 named as S11's hard dependency — is now computed for real (`components/provenance/sessionStale.js`) and wired live into `<FreshnessBadge>` via `ProvenanceDemo.jsx`, independent of D1's source-stale `FreshnessClass`. This closes the last D-classified (S11-dependent) item from the continuation note above; D2-gated `<Cited>` full form and the whole-codebase adoption rollout (§19 Step 5) remain the only items not yet built, both explicitly other systems'/owners' work. Full detail: SPEC-S8 §19 Step 3.
...
**⚠️ 7.2a RESOLVED (owner decision, 2026-09-02).** This 4-value enum is not a complete description of what the accepted D1 program's real adapters emit — D1's hardening pass added a 5th value, `stale`, computed backend-side from the vendor's own reported observation timestamp (a data-quality signal, "this vendor answer looks abandoned"), distinct from this document's own session-aware `stale` render state (§9.6). Decision: D1's backend meaning is preserved exactly, never renamed; S8's own concept is renamed `SESSION_STALE` throughout S8's code, never the bare word "stale" again. Implemented in `freshnessContract.js`'s exhaustive, throw-on-unknown D1→S8 mapping (S8 Step 1, 2026-09-02). Full detail: SPEC-S8 §5.2a.

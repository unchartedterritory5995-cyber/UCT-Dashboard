...
status: reviewed 2026-09-02 against accepted D1 + its provenance/freshness hardening pass — see §0a and SPEC-S8 §0a for the full findings index; one material open item (§7.2a) before Phase 4 begins on FreshnessBadge; awaiting owner approval to leave draft
date: 2026-09-02
...
This PRD was re-checked against the now-accepted D1 (Provider Abstraction Layer) and its same-day provenance/freshness hardening pass. Full findings index and technical detail: SPEC-S8 §0a. Product-level summary: S8's boundary held exactly as designed — D1's hardening pass added real backend computation (evidence-based freshness, a vendor-entitlement distinction, observation timestamps) with zero change required to S8's rendering contract, confirming the D6 boundary in practice, not just on paper. One material, unresolved item carries up to this document: **§7.2's freshness-class enum (`real-time | delayed-15 | end-of-day | historical`) is no longer a complete description of what a real producer emits** — D1 now emits a 5th value, `stale`, for a different reason than this PRD's own session-aware `stale` render state (§9.6). See §7.2a. Two further open questions (not blocking, not resolved): whether Entity Master (already shipped) already fulfills what this program calls "S3" (SPEC-S8 §8a), and whether the 7-state taxonomy (§6/§9) needs an 8th state for a vendor-side (not member-side) entitlement gap D1 can now report (SPEC-S8 §17a).
...
**⚠️ 7.2a MATERIAL FINDING (S8 readiness review, 2026-09-02).** This 4-value enum is no longer a complete description of what the accepted D1 program's real adapters emit. D1's hardening pass (2026-09-02) added a 5th value, `stale`, computed backend-side from the vendor's own reported observation timestamp — a data-quality signal ("this vendor answer looks abandoned") distinct from this document's own session-aware `stale` render state (§9.6, "never a flat wall-clock timeout"). The two share one word for two different facts. Not resolved in this pass — full detail and three candidate resolutions in SPEC-S8 §5.2a; requires either a `data-architecture.md §12.1` decision or a Phase-4 implementation call, both outside this review's authority to make unilaterally.

...
- **Build condition.** **New**, small; the synthesis names NYSE's 2026 early closes (3 July, 27 November, 24 December) as the shape of the dataset (§12.3). ⛔ **CORRECTION (S11 implementation pass, 2026-09-03):** NYSE's actual published 2026 calendar has 3 July as a FULL holiday closure (Independence Day observed — 4 July falls on a Saturday, and NYSE observes the preceding Friday as a whole day off, not a half day); only 27 November and 24 December are real early closes. The shipped dataset (`app/src/lib/marketClock/nyseCalendar.js`) uses the corrected calendar; this line's "3 July" as an early close is the one factual error found and left uncorrected in the prose above, recorded rather than silently fixed in place.
- **Evidence.** C7-02 §0 claim 3, §1.3, §4, §6; C6-02 §5, §9 ("NOT DETERMINED, probably absent"); D-12 §3d open question; synthesis §8.6, §12.3.
- **Implementation status (2026-09-03):** the authorized S11 slice is IMPLEMENTED — `app/src/lib/marketClock/{marketClock,nyseCalendar}.js`, contract exactly as specified above (`sessionState(now)`, `nextBoundary`, `isHalfDay`, `asOfLabel`), covering 2026 only (an uncovered year degrades honestly via `calendarCoverage: false` rather than guessing future holiday dates — small and well-bounded, not a multi-year platform). `useMarketOpen.js` (the existing precedent `MarketClock.jsx`/`ChartMarketClock.jsx` both consume) now sources from this module internally, its returned shape unchanged. `sessionModel.js`'s `nextOpenHint()` is upgraded to skip holidays via `nextBoundary()` rather than guessing "the next weekday." S8's deferred `session_stale` (PRD-S8 §9.6, SPEC-S8 §19 Step 3) is now real — see `provenance-freshness-spec.md`'s own continuation note. Single-venue (NYSE) by design; a second venue is a deliberate future extension, not built (no current Terminal scope needs it). No PRD-S11/SPEC-S11 document was authored — this system's own architecture block above, faithfully implemented with extensive in-code citation back to it, was judged sufficient for a system this size; authoring a parallel formal spec document was judged out of scope for the authorization received.
- ⛔ **THIS RECORD IS DATED 2026-09-03 AND THE FILE HAS MOVED SINCE. READ THE FILE, NOT THIS PARAGRAPH.** On 2026-09-07 the (separate, now closed) UCT Terminal **convergence** program shipped **Seam 7** (`4c4e19ede`) into `app/src/lib/marketClock/nyseCalendar.js`: it added **2027** holiday data — the table carried `COVERED_YEARS=[2026]`, roughly four months of runway, and degraded silently to "every weekday is a full trading day" once that lapsed — and added `tests/test_nyse_calendar_parity.py` so a hand-edit to either calendar table fails CI instead of drifting. It explicitly preserved S11's bundled zero-latency design as deliberate architecture. Seam 7 also found a **third, previously unrecorded NYSE holiday table** in `api/services/voice_temporal_awareness.py` feeding Compass's voice narration, and fixed that file's missing early-close awareness and its naive-DST `_et_now()`. **All of it is a strict improvement to S11 and none of it requires a revision here** — but the sentences above describe the file as it stood on 09-03, and the current file is the authority.
- ⭐ **This is a two-way seam, and the traffic has only gone one way so far.** Convergence follow-ups #1 (`nyseCalendar.js COVERED_YEARS`) and #4 (`_et_now()` naive DST) were **maintenance on Terminal-Next's own output**, performed by another program that did not know this one existed, while this program's control files recorded nothing. ⛔ **Any future convergence-style work on a program-owned path gets logged in `PROGRAM_STATUS.md`'s implementation ledger**, whichever program does it — the ledger is the manifest the re-scoped protection rail checks against, so work that skips it reads as an unrecorded shipment and fails the rail.

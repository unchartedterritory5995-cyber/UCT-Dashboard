...
| # | item | why |
|---|---|---|
| F-I1-1 | ✅ **BUILT 2026-09-11** (`feat/i1-rails`, `be3474241`) — `app/src/pages/research/i1S8Boundary.test.js`. ⛔ **And it found a REAL, LIVE VIOLATION on its first run: `AskAiTab.jsx` renders its own citation list** (`[E#]` marks, source, date, link in local CSS-module classes) instead of composing `<Cited>`/`<Provenance>`. Its sibling tabs on the same page all compose the S8 primitives, so the Ask-AI answer shows a different provenance affordance from every tab beside it. **The Phase-2 double-ownership defect was never closed — only written down as closed.** Recorded as `RECORDED_BOUNDARY_DEBT`; **not fixed**, being member-visible and outside GATE-I1's approved scope. **Needs its own authorization.** | the Phase 2 defect's only regression test was prose, for nine days, while I1 shipped generated prose to members |
...
| F-I1-4 | ✅ **BUILT 2026-09-11** (`be3474241`) — `tests/test_ticker_explain_full_text_completeness.py`. Fields **derived from `EXPLAIN_SCHEMA`** (asserting `additionalProperties: False` at both levels, else the enumeration is only a subset); an unfamiliar schema shape **raises rather than skipping**. Routing proved **behaviourally**: a sentinel ungrounded number and a Buy/Sell/Hold sentence placed in each prose field *alone* must be flagged. Mutation-proved in both directions — deleting `data.get("caveat")` from `_full_answer_text` → 3 failed naming `caveat` as UNGOVERNED PROSE; adding `"risk_note"` to the schema → 1 failed, *"these fields were added to EXPLAIN_SCHEMA and nobody decided how they are governed"*. **Adding a field now breaks the test until it is registered, which was the requirement.** | the convention a new field could silently break |
| — | ⚠️ **Found while building F-I1-4, fixed in the eval harness only:** `checks._full_text` **never read the refusal sentence.** Its docstring claimed it covered `refusal_reason`; the code read only `insufficient_evidence_reason` — the served name, not the raw-payload name. **A fabricated number or a Buy directive inside a refusal passed every mechanical check** (adversarial cases A04 and A14 both passed before the fix). No product path touched. ⭐ A docstring asserting coverage the code did not have — the same shape as this program's other instrument failures | the refusal path was the one place the grounding gate was blind |

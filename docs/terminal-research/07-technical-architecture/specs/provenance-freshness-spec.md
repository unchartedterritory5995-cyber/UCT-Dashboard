...
status: reviewed 2026-09-02 against accepted D1 + its provenance/freshness hardening pass — see §0a; one material open item (§5.2a) before Phase 4 begins on FreshnessBadge; awaiting owner approval to leave draft
date: 2026-09-02
...
2. **§5.2a (new) — MATERIAL, unresolved.** D1's `FreshnessClass` enum now has a 5th value, `stale`, computed backend-side from vendor-timestamp evidence — a different concept from this document's session-aware, frontend-computed `stale` (§9.6), sharing one name. Three resolution options offered; none selected — requires either a `data-architecture.md §12.1` decision or a Phase-4 implementation choice, both outside this review's authority.
3. **§8a (new) — OPEN QUESTION.** Entity Master, already shipped as part of D1, appears to already provide the permanent-identity capability this section attributes to a not-yet-built "S3." Not resolved — flagged for whoever owns the S3 designation.
...
6. **§17a (new) — OPEN QUESTION, not blocking today.** D1 added a vendor-side `entitlement_denied` distinct from this section's member-side `withheld`; the 7-state taxonomy (§5.4/PRD §6) has no state for it. No live surface hits this today, so it is recorded rather than forced into a state that doesn't fit.
...
**Net effect on this review's request** ("is S8 ready for implementation, does it need modification, is it still correct"): the four primitives, the 7-state taxonomy's other six states, the coverage-receipt contract, the sequencing (§19), and the boundary matrix are all still accurate and well-grounded against the newly-hardened D1 contract. One material, unresolved gap exists (finding 2) that should be decided — by whichever mechanism is appropriate (a `data-architecture.md` amendment, or a Phase-4 implementation call) — before `<FreshnessBadge>` is built against real D1 `freshness` values; the rest are open questions worth a deliberate answer but do not block Phase 4's Step 1 (§19, which does not touch freshness-class rendering at all).
...
**⚠️ 5.2a MATERIAL FINDING (S8 readiness review, 2026-09-02) — this enum is no longer accurate against a real, live producer, and this spec does not resolve it.** D1's own freshness enum (`api/services/provider_errors.py::FreshnessClass`) was extended during the "D1 Provenance/Freshness Hardening" pass to a **fifth** value: `real_time | delayed_15 | end_of_day | historical | stale`. This `stale` is computed **backend-side**, in `fmp_client.get_quote`/`massive.get_quote`/`massive.get_batch_quotes`, from the vendor's own reported observation timestamp against a flat, non-session-aware 5-day threshold (`provider_errors.STALE_AFTER_SECONDS`) — a data-quality signal for "this vendor answer looks abandoned/dead" (the live-validation checkpoint's stale/delisted-quote finding), not a UI staleness judgment.

This is a genuinely different concept from §9.6's `stale`, which this document defines as **session-aware** ("never a flat wall-clock timeout," computed from S11's session state against an otherwise-normal freshness class) and is meant to be a *rendering* decision, not a backend-supplied enum value. The two now share one English word for two different facts:
- D1's `stale`: "the vendor's own data for this instrument looks dead" — computed once, backend-side, from real evidence, independent of session state.
- §9.6's `stale`: "this otherwise-normal value hasn't been refreshed recently enough for the current session" — computed per-render, frontend-side, from session state.

**This was not resolved as part of the D1 hardening pass or this review** — the canonical 4-value enum is inherited from `data-architecture.md §12.1 (R-A4-2)`, a document outside S8's own two files and outside this review's stated narrow scope (correcting stale assumptions in *this* PRD/spec, not amending a cross-system architectural decision that D2/D4/S9 may also depend on). **Flagged as a required decision before `<FreshnessBadge>` (§4.3) is implemented against real D1 output**, with three visible options, none selected here:
1. Amend `data-architecture.md §12.1 (R-A4-2)` to make `stale` a legitimate 5th backend-supplied class, and teach `honestStates.js`/`<FreshnessBadge>` to treat it as a stronger, pre-computed instance of §9.6's state 2 (skipping the session-aware computation when the backend has already made the determination).
2. Rename D1's field (e.g. `stale` → `abandoned` or `vendor_stale`) so the two concepts never collide in one enum, and expose it as a **separate** boolean/field on `ProviderResult` alongside `freshness`, leaving the 4-value enum's cited scope intact.
3. Treat D1's `stale` as an internal-only classification `<Provenance>`'s degraded-state logic consumes but never passes through to `<FreshnessBadge>`'s `freshnessClass` prop unmodified — mapping it to whichever of the 4 canonical values it would otherwise have been, plus a separate `dataQualityFlag`.

Whoever finalizes `data-architecture.md §12.1` or begins Phase 4 on `<FreshnessBadge>` must pick one of these (or a better option) before wiring D1's `freshness` field straight into a prop typed for 4 values — passing it through unexamined today would silently reach an unhandled 5th case.

...
status: all A-READY-NOW work IMPLEMENTED 2026-09-02 (§0a/§19), including <Cited>'s narrow interim form; S10 formally DEFERRED — not blocking; the vendor-side entitlement taxonomy (§17a) formally DEFERRED/OPEN — not blocking; remaining B/C/D-classified items (S10 formatter swap, S11-gated session-stale threshold, D2-gated full Cited form, whole-codebase AST enforcement rails) explicitly not built; awaiting owner sign-off on S8's overall completion status
date: 2026-09-02
...
- **D — dependent on another unresolved system, not built:** §19 Step 3 (session-aware staleness threshold — needs S11's calendar model, not fabricated with a flat heuristic); §19 Step 4's *full* D2-gated recursive `<Cited>` form; the PRD §14 AST/acceptance-criteria enforcement rails (criteria 1/2/4/10/11) — these require whole-codebase adoption of `<Provenance>`/`<Cited>` that has not happened yet, explicitly named in §19 Step 5 as other owners' own follow-on migrations, not S8's own build. `honestStates.js` (§4.1) was deliberately NOT built as a separate module — its job is already covered by the combination of `freshnessContract.js` + `availabilityContract.js` + `<Provenance>`'s own conditional logic, and building a "unifying" module now with exactly one real consumer would be the premature abstraction the entitlement-taxonomy deferral explicitly warned against.

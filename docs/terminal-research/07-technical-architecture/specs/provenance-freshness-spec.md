...
status: all A-READY-NOW work IMPLEMENTED 2026-09-02 (§0a/§19), including <Cited>'s narrow interim form; S10 formally DEFERRED — not blocking; the vendor-side entitlement taxonomy (§17a) formally DEFERRED/OPEN — not blocking; S11 IMPLEMENTED 2026-09-03 and the session_stale threshold it gated is now LIVE (see §19 Step 3 note below); remaining B/D-classified items (S10 formatter swap, D2-gated full Cited form, whole-codebase AST enforcement rails) explicitly not built; awaiting owner sign-off on S8's overall completion status
date: 2026-09-02
...
## 0a. S8 readiness review, 2026-09-02 — findings index

This document and PRD-S8 were re-read and cross-checked against the now-accepted D1 (Provider Abstraction Layer) and its 2026-09-02 provenance/freshness hardening pass, per an explicit review authorization ("review/finalize S8 against the completed Entity Master and accepted D1 implementation... correct stale assumptions"). Scope was kept narrow: correct what D1's real, live output now proves or contradicts in *this* document; do not re-derive `product-architecture.md`/`data-architecture.md`/`information-architecture.md` from scratch. Findings, each marked inline at its section:

1. **§5.1a (new).** A second real narrow provenance shape now exists in the codebase (D1's quote adapters), closer to the PRD §7.1 target than `bar_provenance.py` on the timestamp axis. Additive — no contract change.
2. **§5.2a — RESOLVED 2026-09-02 (owner decision).** D1's `FreshnessClass` enum has a 5th value, `stale`, computed backend-side from vendor-timestamp evidence — a different concept from this document's session-aware, frontend-computed `stale` (§9.6). Resolution: D1's meaning is preserved exactly; S8's own concept is renamed `SESSION_STALE` everywhere in S8 code; the distinction is now implemented in `app/src/components/provenance/freshnessContract.js`'s exhaustive, throw-on-unknown `mapD1Freshness()`. No longer blocking.
3. **§8a (new) — OPEN QUESTION.** Entity Master, already shipped as part of D1, appears to already provide the permanent-identity capability this section attributes to a not-yet-built "S3." Not resolved — flagged for whoever owns the S3 designation. Confirmed non-blocking for Step 1 (owner ruling 2026-09-02: S8 consumes canonical identity, never defines it, and Step 1 invented no identity policy).
4. **§14 (extended).** A second live KP/CR/OC/CA evidence-ladder instance now exists (D1's admin/status endpoints), reinforcing §7.3's "one mechanism" requirement with a second precedent. Additive.
5. **§14 (extended) — minor.** A snake_case (D1 Python) vs. camelCase (S8 prop sketches) boundary note for whichever future layer bridges the two. Not yet live, not blocking.
6. **§17a (new) — OPEN QUESTION, not blocking today.** D1 added a vendor-side `entitlement_denied` distinct from this section's member-side `withheld`; the 7-state taxonomy (§5.4/PRD §6) has no state for it. Owner ruling 2026-09-02: entitlement stays a fully separate, orthogonal rendering dimension from freshness — never collapsed into `FreshnessClass` or `honestStates.js` for rendering convenience. No live surface hits this today; recorded, not force-fit.

**Confirmed, no correction needed:** S8's core ownership boundary — S8 renders, D1 (and every other producer) computes/attaches — held exactly as designed through this hardening pass. Every new computation (evidence-based freshness, the entitlement distinction, the observation timestamp) was added entirely inside D1's backend; zero S8 rendering-component contract needed to change for D1's existing capabilities to keep working under it. This is the LOCKED D6 boundary being tested by a real, independent implementation and holding.

**S8 STEP 1 — IMPLEMENTED, 2026-09-02.** Authorized against exactly this findings index plus the owner's freshness/entitlement rulings above. `<CoverageLine>` relocated to `app/src/components/provenance/` with a re-export shim at its old path (both real importers, `ScanResults.jsx`/`EvidenceTab.jsx`, unchanged); `<FreshnessBadge>` built on `sessionModel.js` with the exhaustive `freshnessContract.js` mapping; `<Provenance>`'s degraded state built. 60 new/relocated component tests + the reachability rail + every real consumer's own suite pass (594 tests, 47 files). None of the three components is mounted on a real page yet (Step 2+) — each carries a dated `AWAITING_A_DECISION` entry in `reachable.test.js` naming why, per this codebase's own established idiom for "built, tested, deliberately not yet wired."

**S8 STEP 2 — S10-INDEPENDENT SUBSET IMPLEMENTED, 2026-09-02.** A narrow S10 dependency check (direct search of the planning corpus and codebase, not re-derivation) found: **no PRD/spec for S10 exists under any name**, and **no single authoritative shared formatter exists in code** — `product-architecture.md`'s S10 system block (§19's own citation) tags S10 "consolidate" against ~118 files that each define their own `fmt*` helper (TD-08), meaning S10 is itself not-yet-built infrastructure, not a hidden asset this review missed. Per §19 Step 1's own already-stated sanction ("format via existing narrow helpers... rather than inventing a third formatter"), this did NOT block Step 2: a small, explicitly-local, non-platform helper (`presentationFormat.js`) was written instead, documented as disposable once S10 ships. Delivered: `GET /api/provenance/quote` (the new, minimal live-D1 surface — none existed); `<Provenance>` extended with `loading` (§9.7) and `availability` (§9.3/§16's orthogonal not-found/entitlement-denied/provider-error axis, `availabilityContract.js`, structurally parallel to `freshnessContract.js`); a real accessible detail disclosure replacing Step 1's tooltip stand-in; `pages/ProvenanceDemo.jsx` at `/provenance-demo` — a genuinely reachable, live-wired demonstration (not a mocked test in isolation), which is why Step 1's three `AWAITING_A_DECISION` entries were removed rather than extended. 47+ new tests, full-suite regression clean (3 pre-existing, unrelated failures; one genuine self-inflicted TD-18-shaped defect in the demo's own fetch call was caught by this codebase's own AST census mid-build and fixed by switching to `jsonFetcher`, not routed around). **Still explicitly deferred to Step 2+/S10's eventual arrival:** the full hover/click popover's final visual design, and swapping any of this onto S10's shared formatter once it exists.

**S8 CONTINUATION — REMAINING WORK CLASSIFIED AND A-READY-NOW COMPLETED, 2026-09-02.** Owner decisions: (1) S10 formally **DEFERRED — NOT BLOCKING S8**, not built now, not built merely to complete §19 Step 2's literal formatter-swap line; a future initiative only once real cross-app reuse justifies it. (2) The §17a vendor-side entitlement taxonomy question stays **DEFERRED/OPEN — NON-BLOCKING** — no broader taxonomy invented; `availabilityContract.js`'s existing 5-state contract is used as-is.

Remaining PRD-S8/SPEC-S8 sequence classified:
- **A — READY NOW, completed:** `<Cited row=…>`'s narrow interim form (§4.5, explicitly "buildable now" against `bar_provenance.py`) — built, live-wired via new `GET /api/provenance/bar`, tested (7 component + 3 backend + 2 page-level tests).
- **B — S10-dependent, not built:** the literal formatter swap; S10's own final visual design of `<FreshnessBadge>`.
- **C — entitlement-taxonomy-dependent:** none found — nothing currently ready-now is blocked by the open §17a question.
- ~~**D — dependent on S11, not built:** §19 Step 3 (session-aware staleness threshold).~~ **IMPLEMENTED 2026-09-03** — S11 shipped (`app/src/lib/marketClock/`); see §19 Step 3 below. Moved out of the D bucket.
- **D — dependent on another unresolved system, still not built:** §19 Step 4's *full* D2-gated recursive `<Cited>` form; the PRD §14 AST/acceptance-criteria enforcement rails (criteria 1/2/4/10/11) — these require whole-codebase adoption of `<Provenance>`/`<Cited>` that has not happened yet, explicitly named in §19 Step 5 as other owners' own follow-on migrations, not S8's own build. `honestStates.js` (§4.1) was deliberately NOT built as a separate module — its job is already covered by the combination of `freshnessContract.js` + `availabilityContract.js` + `<Provenance>`'s own conditional logic, and building a "unifying" module now with exactly one real consumer would be the premature abstraction the entitlement-taxonomy deferral explicitly warned against.

**A real defect caught by inspecting the actual rendered UI, not just tests:** `provenance_quote.py`'s own documented `kind: "unknown"` fallback (an untyped exception — e.g. `_MassiveRestClient.__init__`'s pre-existing bare `RuntimeError` when `MASSIVE_API_KEY` is unset — reaching the endpoint's generic `except Exception`) had no case in `availabilityContract.js`, so `mapAvailability`'s own deliberate exhaustiveness guard threw and crashed the page instead of rendering an honest state. Found within minutes of opening `/provenance-demo` in a real browser; no unit test had exercised this exact path (fixtures always supplied a recognized `kind`). Fixed by mapping `"unknown"` to the contract's own pre-existing `UNKNOWN` state — the state already defined for exactly this fact ("absence of any real signal"), not a new one invented for the occasion.
...
**5.1a A second real narrow shape now exists — D1's quote adapters (correction added 2026-09-02, S8 readiness review).** `bar_provenance.py` was this spec's only cited narrow-shape precedent when it was written; the accepted D1 (Provider Abstraction Layer) program has since shipped a second one, in `api/services/provider_errors.py::ProvenanceRecord` as populated by `api/services/fmp_client.py::get_quote` and `api/services/massive.py::get_quote`/`get_batch_quotes`:

| Field (PRD §7.1 target) | D1's `ProvenanceRecord` (fmp_client.py / massive.py, read directly 2026-09-02) | Gap |
|---|---|---|
| source-activity reference | `source_activity` — e.g. `"fmp_client.get_quote"` / `"massive.get_quote"` | None — closer to the target than `bar_provenance.py`'s flat `source` string, since it already names the exact producing function |
| source-entity reference | *absent* | Same gap as `bar_provenance.py` — not tracked at this layer either |
| timestamp | `fetched_at` (request time) **and** `source_observed_at` (the vendor's own reported observation timestamp — FMP's `/stable/quote` `timestamp` field; Massive's snapshot `updated`/`lastTrade.t`, both live-verified 2026-09-02) | **Closer to the PRD §7.1 target than `bar_provenance.py`**: this is a clean two-timestamp split (request time vs. the value's own as-of), not the three-timestamp ambiguity (`bar_time`/`validated_at`/`verified_at`) §5.1's table above describes. `source_observed_at` is `None` when the vendor supplies nothing — never backfilled from `fetched_at` |
| tie-break record | `tie_break: Optional[str]` field exists on the dataclass but is **never populated** by either adapter (confirmed by grep, 2026-09-02) | Same practical gap as `bar_provenance.py` — the field exists in the shape but carries no data yet at this layer |

This does not change §4.2/§4.5's component contracts (both already accept `sourceEntity`/`tieBreak` as optional, so this shape renders correctly under the same props) — it is added here so a future revision citing "the narrow shape S8 must render today" cites both real producers, not only bars.
...
**⚠️ 5.2a RESOLVED (owner decision, 2026-09-02) — supersedes the three-option framing this section originally carried.** D1's own freshness enum (`api/services/provider_errors.py::FreshnessClass`) has a **fifth** value: `real_time | delayed_15 | end_of_day | historical | stale`, computed **backend-side** from the vendor's own reported observation timestamp — a data-quality signal for "this vendor answer looks abandoned/dead" (the live-validation checkpoint's stale/delisted-quote finding). §9.6's own `stale` is a *different* concept: session-aware, frontend-computed, "this otherwise-normal value hasn't been refreshed recently enough for the current session."

**The decision, option 2 of the three originally offered, refined:**
- **D1's backend `stale` is PRESERVED EXACTLY as built and live-validated — never renamed, never reinterpreted.** It stays the literal Python string `"stale"` on `FreshnessClass`, meaning exactly what §5.2a originally described: source/provider data staleness per backend timestamp evidence.
- **S8's own session-derived concept is renamed `SESSION_STALE`** (or the closest S10/S11-consistent naming form once those exist) everywhere in S8's code — never the bare word "stale" again. The two are documented as orthogonal: a value can be source-stale, session-stale, both, or neither.
- **This distinction is now implemented, not merely decided.** `app/src/components/provenance/freshnessContract.js` (S8 Step 1, built 2026-09-02) is the ONE place a raw D1 `freshness` string is interpreted: `mapD1Freshness()` is exhaustive over D1's real 5-value enum, throws on anything unrecognized (no default branch, no silent misrepresentation of a future 6th value), and exports `SOURCE_STALE`/`SESSION_STALE` as the two distinct, permanently non-bare-"stale" constants. `FreshnessBadge.jsx` renders both as independent, separately-`data-testid`'d notes that can co-occur without collapsing into one state — pinned by `FreshnessBadge.test.jsx`'s "source staleness and session staleness never collapse into one render" suite.
- **Entitlement (`entitlement_denied`) stays a fully separate, orthogonal dimension**, per the same owner ruling — never folded into `FreshnessClass` or this mapping. `FreshnessBadge.jsx` renders no entitlement/availability state today; that is a distinct future primitive's job, not retrofitted here for rendering convenience.

**Still open, correctly out of this review's scope:** whether `data-architecture.md §12.1 (R-A4-2)`'s own 4-value citation should be formally amended to acknowledge D1's real 5th value is a cross-system documentation question for that document's owner — S8's own contract no longer depends on that amendment landing first, since `freshnessContract.js` already handles the real enum exhaustively and by name.
...
**⚠️ 8a OPEN QUESTION (S8 readiness review, 2026-09-02) — "S3 does not exist yet" may already be stale.** Entity Master (`api/services/entity_master/api.py`, shipped as part of the accepted D1 program, read directly 2026-09-02) already provides exactly the capability this section describes as S3's not-yet-built job: `resolve(alias, as_of=...)` maps a ticker string to a canonical `entity_id`, and `vendor_symbol(entity_id, vendor, as_of=...)` maps that back to a vendor-native symbol, both with `valid_from`/`valid_to` validity windows — i.e., permanent identity that survives ticker reuse over time, live-validated against a real dual-class-ticker case during D1's own checkpoint. Massive's D1 adapter (`massive.py::_resolve_symbol`) already routes through it today.

Whether Entity Master **is** S3, is a partial down payment on S3, or is a separate, parallel mechanism this spec has not reconciled with S3 is a program-level naming/architecture question — resolving it would require reading `product-architecture.md`'s S3 block directly, which is outside this review's stated narrow scope (correcting *this* PRD/spec, not re-deriving a cross-system architecture decision). **Flagged, not resolved**: if Entity Master is (or should become) S3, this section's "until S3 exists" framing is stale for any capability that already routes through Entity Master, and `<Cited row=…>`'s narrow-form gap (§4.5) may already be smaller than described for those cases — worth a deliberate answer before this section is treated as final.

**Owner ruling, 2026-09-02:** kept explicitly OPEN, and explicitly does NOT block S8 Step 1 — S8 Step 1 built no `<Cited row=…>` at all (that primitive is Step 4, D2-gated per §19) and invented no identity policy of its own. The rule stands: S8 consumes canonical identity; it never defines it. Surface this dependency again, concretely, only if and when a real S8 behavior needs the answer.

**CONFIRMED, 2026-09-02 (S8 Step 2's Entity Master/S3 check).** `product-architecture.md`'s own system block is literally titled **"### S3 — Entity Master"** — S3 *is* Entity Master by the program's own naming, not a separate or parallel system. Its designed primitives (`resolve(alias, asOf) → entity`, `aliases(entity) → dated list`, `vendorSymbol(entity, vendor)`) match the shipped implementation's `resolve()`/`vendor_symbol()` almost exactly (naming-convention casing aside); `aliases()`'s reverse-lookup form was not confirmed to exist and was not chased further (re-deriving Entity Master's full surface is exactly the "reopen Entity Master architecture" this check was told not to do), but nothing in S8 needs it — the specific gap this section names (ticker-reuse misattribution) is closed for any capability routing through `resolve(alias, as_of)`'s bitemporal query, which is exactly what it was built to do. **Closed as satisfied for S8's purposes.** The system block's own "Build condition: New" tag is now stale (Entity Master has since shipped) — a correction for whoever owns that document, not re-litigated here.
...
- **A second confidence/evidence-class precedent now exists (S8 readiness review, 2026-09-02).** PRD §7.3 generalizes "the provider ledger's KP/CR/OC/CA evidence ladder" as the target shape for the confidence field, previously citable only via `provider_coverage_monitor.py`'s per-*field* fill-rate ladder (D12). The accepted D1 program has since shipped a second, live instance at the per-*vendor-adapter* level: `GET /api/admin/fmp-adapter-status` / `GET /api/admin/massive-adapter-status` (`api/routers/{fmp,massive}_adapter_status.py`, no-auth read-only, built 2026-09-02) return an `evidence_ladder` object with exactly `KP`/`CR`/`OC`/`CA` fields — `KP`/`CR`/`OC` auto-derived from real signals (key presence, code reference, `served_total` off zero since process start), `CA` deliberately always `null` with no automated promotion path, matching data-architecture.md §18.2's explicit "not derivable" finding for that field. This reinforces §7.3's "one mechanism, not built separately" requirement with a second concrete, already-tested precedent to cite when `<Provenance>`'s confidence field is eventually built (§4.2) — not a correction, an additional data point.
- **A minor casing-boundary note for whichever future API layer bridges D1 to S8.** D1's Python-side AI-consumable serialization (`ProviderResult.to_dict()`, added 2026-09-02) emits snake_case keys (`source_activity`, `fetched_at`, `source_observed_at`, `tie_break`); this spec's component prop sketches (§4.2 `<Provenance>`, §4.5 `<Cited>`) use camelCase (`sourceActivity`, `sourceEntity`, `tieBreak`). No API route exposing D1's raw shape to a frontend exists yet (§6: "no new API route is required for S8 itself"), so this is not yet a live mismatch — flagged so whichever future boundary layer serializes a D1 `ProviderResult` for a Terminal-Next frontend does the key-casing translation deliberately rather than rediscovering the mismatch mid-build.
...
**⚠️ 17a OPEN QUESTION (S8 readiness review, 2026-09-02) — a second, different kind of "not entitled" now exists, and the 7-state taxonomy (§5.4) has no state for it.** D1's provenance/freshness hardening added `entitlement_denied` (`api/services/provider_errors.py::ProviderAuthError`), derived from a vendor's own HTTP status (403 = valid credential, not entitled to this resource; live-confirmed example: Massive's index-quote endpoint) — this is UCT's own **vendor-side** entitlement, structurally distinct from `entitlements.py`'s **member-side** `withheld` this section describes. A value can be `entitlement_denied` at the vendor layer regardless of which member is asking, and regardless of `entitlements.py`'s answer for that member. None of the 7 states (§5.4) cleanly fit this: it is not state 3 (data does exist, UCT just cannot fetch it under current vendor terms), not state 4 (`withheld` is member-specific; this isn't), and not state 5 (it is a structural/permanent gap, not a transient outage). **Not resolved here** — no live Terminal-Next surface currently attempts to render a value in this state (Massive's index-quote gap has no member-facing consumer today), so this is a real gap flagged for the *next* time D1 grows a capability that does, rather than a blocking defect in the current 7-state taxonomy.

**Owner ruling, 2026-09-02:** entitlement/availability is an orthogonal rendering dimension from freshness, full stop — never an 8th `FreshnessClass` value, never folded into `honestStates.js`'s taxonomy for rendering convenience. S8's eventual state model should support DATA VALUE, FRESHNESS, PROVENANCE, AVAILABILITY/DEGRADED, and ENTITLEMENT/LICENSING as independent axes that can combine (fresh-but-restricted, stale-but-otherwise-valid, not-found, provider-unsupported, provider-failed, etc.), never one collapsed status enum — unless a future, explicitly-approved architecture decision requires otherwise. S8 Step 1 built no entitlement/availability rendering at all (`FreshnessBadge.jsx` renders freshness only); that remains a distinct future primitive's job.
...
- **Step 3 — IMPLEMENTED 2026-09-03, S11 shipped.** `app/src/lib/marketClock/{marketClock,nyseCalendar}.js` is S11's real session/holiday model (2026 NYSE calendar, DST-tested, contract exactly per product-architecture.md's S11 block). Confirmed additive as predicted: `<FreshnessBadge>`'s `sessionState`/`sessionStale` prop shapes did not change. The real computation lives in the new `components/provenance/sessionStale.js` (`computeSessionStale(asOf, now)` — reads S11's `sessionState(now).boundaryAt`, independent of D1's `SOURCE_STALE`) and is wired live into `ProvenanceDemo.jsx`'s `VendorRow`. `useMarketOpen.js`'s naive weekday+fixed-hours guess is replaced internally by S11 (`sessionModel.js`/`MarketClock.jsx`/`ChartMarketClock.jsx` all unchanged at the contract level — this is exactly the "one shared mechanism, two surfaces can never disagree" precedent this section noted, now backed by a real calendar). Regression tests: `marketClock.test.js`, `sessionStale.test.js`, `useMarketOpen.test.js`, `sessionModel.test.js`, plus 3 new `ProvenanceDemo.test.jsx` cases proving source-stale and session-stale render independently.

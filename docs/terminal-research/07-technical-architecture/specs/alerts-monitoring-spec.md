...
| **`scan-membership-change` source data** | ⚰️ **CORRECTED 2026-09-13 (GATE-S7-SCAN-MEMBERSHIP-CHANGE §1, CP1).** `api/services/screener/scan_store.py` — `scan_hits` + `scan_coverage` in **`screener.db`**, written by the 05:00 ET `scan_evaluator.sweep_job`. This cell said the sweep writes to `definition_evaluations`; it does not — that table belongs to `definition_record.py` and lives in `signal_ledger.db`. | Nightly 05:00 ET sweep already computes each saved definition's matching-symbol set per cycle, and **`scan_store.recent_covered_as_ofs(def_hash, tf, limit=2)` already returns exactly the two sessions a diff needs** | ⛔⛔ **THIS IS AN ABSORPTION, NOT A NEW CAPABILITY, AND THIS CELL SAID THE OPPOSITE.** It read *"No code today diffs cycle N against cycle N-1's membership set — this is PRD §6.1 row 3's 'new capability'"*. **Measured 2026-09-12: `api/services/screener/screen_alerts.py` (261 lines) is that diff, and it is COMPLETE, WIRED AND LIVE** — `diff_for` (`:141-154`), driver `run_nightly` (`:171-261`), scheduler entry `screener_screen_alerts` at **05:10 ET** in `api/main.py:1636-1641`, its own `screen_alert_subs` + `screen_alerts_fired` tables in `screener.db`, three `require_paid` endpoints in `api/routers/screener.py:189-213`, and delivery through `watchlist_alert_service.deliver_alert_payload` (in-app + email + Discord). Both gates were read live on `web` 2026-09-12: `SCAN_SWEEP_ENABLED=1`, `SCREEN_ALERTS_ENABLED` **unset** and the code default is ON — so the job is scheduled in production. ⭐ Two consequences: the §4a **absorption default** applies (the legacy path stays live and untouched, the new type runs dark, and the flip plus the legacy switch-off happen in the SAME PR — and for this type that switch-off is member-visible), and **`screen_alerts_fired` is a SEVENTH alert-state table** the completion plan's §0a six-table list does not carry, which makes the consolidation premise stronger than the plan states, not weaker. |
...
| `indicator-condition` | **`{indicator, condition, threshold, tf, params_json, instance_id, scope, def_source}`** — EIGHT keys; the timeframe is its OWN key, and the canonical ADDRESS is assembled from `(indicator, tf)` INTERNALLY for the cadence gate | ⚰️⚰️ **RE-CORRECTED 2026-09-13, MEASURED AGAINST THE MERGED CODE.** The 2026-09-12 text of this cell — ~~`{address: str, condition: …, threshold: float}` — the timeframe rides INSIDE the address~~ — described an INTENT that `ccbab9bcd` did not implement at the params layer. Measured on master: `indicator_condition.PARAMS_SCHEMA` has **8 keys** and **no `address` key at all** (`python -c "from api.services.alert_taxonomy import indicator_condition as ic; print(list(ic.PARAMS_SCHEMA))"`). ⭐ **AND BOTH HALVES WERE PARTLY RIGHT, WHICH IS WHY IT SURVIVED A DAY:** the address model IS in the shipped code — `cadence_for(address)` resolves `uct://<metric>@<entity>/<timeframe>?as_of=<instant>` through D2's book and never a bare metric name — but it is an **internal resolution key built from `(indicator, tf)`**, not the shape a predicate is registered with. ⛔ **THE SPEC IS AMENDED TO THE CODE, not the code to the spec:** the eight-column form is what production carries, `alert_predicates` rows are written against it, and rewriting live predicate params to match a document would be a member-visible migration for zero member benefit. **F-S7-IC-2 (recorded, not scheduled):** collapse `(indicator, tf)` into a single `address` param — sequenced with the first change that already touches predicate registration, never standalone. ⛔⛔ **THE CONSEQUENCE MEASURED AT CP1 STANDS UNCHANGED (F-S7-IC-1):** `indicator_alert_evaluator.all_addresses()` declares **31** addresses, the canonical book **142** metrics, and the intersection is **EMPTY** — reported with its control (intersecting the legacy set against one known legacy address returns 1, so the operator works and the zero is a finding, not a broken instrument). The honest reading of that 0 is **ONE rename** (`close` ↔ `ohlcv.c`) and **THIRTY genuine absences**: the lanes share no vocabulary, legacy speaking indicator (`adx.adx`, `bb.upper`, `atr`) and the book speaking screener-row (`adr_pct`, `above_50sma`). **GATE-D2 CP4** (PRD-D2 §9.5, fingerprint `3257cc319`, merged `404b808c5`) now declares the second axis that closes it. See §5.2.1 and PRD-D2 §9.4. |
...
✅ **D2 HAS NOW PAID IT — PARTLY. `GATE-D2 CP4` (PRD-D2 §9.5, fingerprint `3257cc319`, merged `404b808c5`, 2026-09-13)** declares the **second address form** `<indicator>[.<output>](<params>)@<timeframe>` for the thirty indicator outputs plus the one rename, with a resolver that returns a **computation descriptor and never a value**, flag-OFF behind `CANONICAL_INDICATOR_AXIS_ENABLED`.

⚠️ **WHAT IT DOES NOT CLEAR, STATED PLAINLY:** CP4 does **not** declare a per-timeframe cadence for `ohlcv.*`, because that still means editing `bars_fetch.py` / `bars_sqlite.py` — measured 2026-09-13 as **inside flow-worker's 154-module import closure** and outside its watch list — and the owner's 2026-09-13 ruling is that **the watch list is NOT widened**; any change to those two is BEHAVIOUR-CHANGING and merges after-hours with a marker bump. ⛔ So **every `ohlcv.*` predicate still REFUSES on cadence**, and that remains the correct dark state rather than a gap. CP4 makes the indicator vocabulary *expressible*; it does not make the bars cadence *declared*.
...
| `scan-membership-change` | nightly, **05:10 ET** | ⚰️ **CORRECTED 2026-09-13 (same gate).** This said *"05:00 ET · `scan_evaluator.sweep_job` … reused, not re-scheduled"*. The sweep runs at 05:00 and produces the DATA; the alert job is a **separate scheduler entry** — `api/main.py:1636-1641`, id `screener_screen_alerts`, `CronTrigger(hour=scan_evaluator.SWEEP_HOUR_ET, minute=SWEEP_MINUTE_ET + 10)` = **05:10 ET**, `max_instances=1`, calling `screen_alerts.run_nightly()`. So it IS separately scheduled, ten minutes behind the sweep it depends on, and the flip's legacy switch-off is the removal of that job. |

...
- `grep -n "ErrorBoundary" app/src/pages/charts/ChartsWorkspace.jsx app/src/pages/charts/WidgetHost.jsx` → **no matches**. CONFIRMED.
  ...
  ⚰️ **STALE AS OF 2026-09-21 — CORRECTED HERE 2026-09-25. This finding was true when written and is now false.** `WidgetHost.jsx:107-111` wraps `WidgetBody` in `ErrorBoundary` with a `WidgetErrorFallback` naming the widget type and `key={groupId}` to reset a tripped boundary on a tab swap; `WidgetHeader` renders at `:227`/`:254`, **outside and before** the boundary, so this section's companion claim — that a throwing widget cannot be closed because its header is inside the failing subtree — is also closed. Both landed in `424bf3355` *"S1 CP3: panel registry formalization — registerPanel, TD-02 boundary, mount cap"*, an ancestor of `origin/production`, together with `PANEL_MOUNT_CAP = 3`. ⚠️ What is NOT closed: there is still no `MAX_WIDGETS` board-size cap (weakness #8's second half) — mounts are throttled, board size is bounded only by `FIXED_ROWS`. Found while writing C5-03 `06-ux-and-information-architecture/fixed-modular-hybrid.md`, which had drafted this gap as an unmet precondition of the hybrid decision on the strength of this line.

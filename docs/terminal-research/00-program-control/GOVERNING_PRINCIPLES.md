...
* Model classes (OWNER_SEED_FACTS §5): the strongest available model for synthesis, council review, and red team; a faster class is permitted for leaf research; the choice per role class is recorded in `AGENT_REGISTRY.md`. Models are never downgraded for cost.

...
| **A9** Screening | live surface | ✅ `scan-membership-change` CP3 merged `df937146c` — it fires dark; ⚰️ was *"flag OFF"* — **armed 2026-09-13, in-process `1`** (§4 Flag Ledger). **THE DARK READ HAPPENED 2026-09-21/22** (via `GET /api/admin/alert-taxonomy/dark-report/scan-membership-change`, browser-authenticated): `predicate_count: 0`, zero spans opened. Investigated rather than assumed a bug — wiring, scheduler registration (`api/main.py:6931`), and the feature flag (armed since 2026-09-13) are all correct and symmetric with every sibling alert type. The zero is a **documented, expected "honest zero"**: `docs/feature_flags.json`'s own ledger entry for this flag recorded on 2026-09-13 that the dry run projected N=0 because no member of the s7-dark cohort holds a `screen_alert_subs` row — nobody in the cohort has subscribed to a saved-screen alert, so the comparison loop that would open a span never executes for anyone. This is a **product/cohort-enrollment gap, not a code defect** — the fix, if wanted, is getting a cohort member to subscribe to a saved-screen alert, not a code change. Still not ready for a CP4/FLIP ruling — there is nothing yet to compare | **BLOCKED-OWNER** — read done, shows no comparable data yet; CP4 + FLIP still need real cohort activity first |

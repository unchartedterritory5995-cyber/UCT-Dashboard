...
| **F-D2-3** | D2 CP1 | ⚰️ was *"OPEN — needs re-read"* — **RE-READ 2026-09-22.** The gate packet's own claim that "the other positional readers of that projection are named in the finding" is **false** — traced to source, the finding names only `ticker_returns.py`. A whole-repo enumeration (read-only) found **21+ unprotected call sites across 19 files** with the identical pre-CP2 fragility, including `bars_fetch.py::_fmt_sqlite_bars` (feeds the primary `/api/bars/{ticker}` chart endpoint), the voice assistant, and Journal 2.0's MFE/MAE analytics — none individually protected. Also found a vacuous fixture in `test_signature_router.py` (open==close==95.0, so it cannot actually catch an ordinal swap). **A scoped proposal now exists**: `gates/d2-f-d2-3-ordinal-inventory-scoped-proposal.md` (detection/inventory only — deliberately does NOT propose migrating any of the 21+ sites, several likely flow-worker-reachable), unsigned | a scoped proposal is drafted and ready for the owner's signature; not built. **This is the single most consequential finding from this pass** — a real, live gap in the safety net around the site's primary chart data path, not yet an active defect |

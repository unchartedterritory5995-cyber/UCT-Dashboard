...
date: 2026-09-21
measured_against: "counts and cited SHAs re-derived 2026-09-21 against origin/master @ a392afd11; A12/A10/A14 rows freshly measured 2026-09-21 (code on feat/s7-price-level, not yet merged to master), remaining per-row prose last measured 2026-09-20 (⚰️ this said: origin/master @ f3109e1af · docs @ 8cbd6ec56, dated 2026-09-12)"
---
...
| **BLOCKED-SPEC-READ** | **0** | none. ⚰️ **A14 moved to DONE 2026-09-21**, its CP1 signed and built — the last row that carried this tag |
| **BLOCKED-DEPENDENCY** | **4** | S8 (D2), A1 (D2), A11 (D2, also OWNER), A13 (D2+S5, also OWNER) — ⚰️ **A10 moved to DONE 2026-09-21**, both its named blockers (D3-CP4, D4-CP5) signed and built |
...
**Unique-system arithmetic, so the 32 is checkable, not asserted:** 25 DONE + 1 RULED-HOLD (S7)
+ 1 BLOCKED-DATA (D2) + 1 BLOCKED-OWNER-only (A9) + 0 BLOCKED-SPEC-READ + 2 single-tag
BLOCKED-DEPENDENCY (S8, A1) + 2 dual-tagged OWNER+DEPENDENCY (A11, A13, each counted once
here) = 25+1+1+1+0+2+2 = **32**. The breakdown table's BLOCKED-OWNER (3 = A9 + the 2 dual-tagged)
and BLOCKED-DEPENDENCY (4 = the 2 single-tag + the same 2 dual-tagged) rows therefore sum to 7
tag-occurrences across only 5 unique systems (A9, S8, A1 once each; A11 and A13 twice each),
which is why the table's own rows don't independently sum to 32 — expected, not an error.

⚰️ **RE-DERIVED 2026-09-21, and the arithmetic above HOLDS.** Walked the 32 rows of §1 again from
their own blocker columns: 22 DONE + S7 + D2 + A9 + A11 + A13 + A12 + A14 + S8 + A1 + A10 = 32
was the count as of that morning. Also checked every SHA the DONE rows cite as merged: 38 checked,
**0 unresolved** — 24 are ancestors of `origin/master`, 8 are cherry-picked (the authoring SHA is
not an ancestor, its patch-id IS on master; S6 ×3, D3 ×2, D5 ×3), 6 are `sign_gate` fingerprints
and not commits at all (S2 CP3–CP6, D4 `40caca541` and `23a8da7a9`). ⛔ **Two classes of false
alarm to know before you re-run this check:** an ancestry test reports every cherry-picked
authoring SHA as "not merged" (F-ENVCHECK-1), and a `cat-file` test reports every fingerprint as
"not an object". Use `git cherry` for the first and the approval block for the second.
⚠️ **NOT re-verified:** the numbers inside each proposal (row counts, flow-worker non-strand), any
production data, and S4 CP5–CP7's premises.

✅ **F-AUDIT-3 CLOSED, same day, later.** D3-CP4 and D4-CP5 both signed and built 2026-09-21 —
A10 moved DONE (no dependency remains). **A12-CP1 also signed and built 2026-09-21** — A12 moved
DONE. **A14-CP1 also signed and built 2026-09-21** (fingerprint `417b6b853`, code `f662b84d5` on
`feat/s7-price-level`) — A14 moved DONE, the last BLOCKED-SPEC-READ row. DONE count moved
22 → 23 (A10) → 24 (A12) → **25** (A14), all the same day. See F-AUDIT-3's own row (§3.1c) for
the A10/D3/D4 history; A12's and A14's moves are separate, unrelated signatures.
...
|---|---|---|---|---|
| **S1** Terminal Shell | ✅ **CP1 BUILT AND MERGED** `b7e7541a0` (2026-09-14); ⚰️ was *"CP2 buildable"* — **CP2 SIGNED (owner, 2026-09-19, `e86c92b6f`) AND MERGED to master** (`8baca199b`, confirmed an ancestor of `origin/master`; `app/src/surfaces/pageTitle.js` + `usePageTitle()` in `Layout.jsx`) | product-architecture §5-A.1 — a shell that hosts surface kinds from a manifest | ⚰️ was *"BLOCKED-OWNER — OI-06"*. OI-06 was answered 2026-09-14 (telemetry-derived-defaults.md); CP1 turned out not to need it, CP2 did and is now built. **CP3+** ("surface kinds become real") was gated on OI-06 answered + the A2 registry decision (gate packet §3) — OI-06 is answered and A2 is now DONE too (line 131), so CP3 may already be proposable; not yet checked in detail or proposed. ⚰️ Checked 2026-09-20: a scoped proposal now exists — `12-decisions/gates/s1-cp3-panel-registry-scoped-proposal.md` (`registerPanel`/TD-02 error boundary/staggered-mount cap as MUST, `promote`/`popout` naming as SHOULD, the entity page + DEC-01's final lock explicitly DEFERRED). **SIGNED by the owner 2026-09-21** (fingerprint `fc609961a`) and **BUILT AND MERGED same day**, `feat/s7-price-level` `96c810116`: `registerPanel` validates every entry at registration (`menus.terminal` defaults false, derived count not typed — the proposal's own "21" miscount was corrected to the real 20 before signing); TD-02 boundary wraps `WidgetBody` in `WidgetHost.jsx`; `useStaggeredMount` ported to the main board only (`PANEL_MOUNT_CAP=3`, popped-out boards/popout window/phone app unaffected). SHOULD's `popout`/dock transitions extracted as named pure functions (`popout/popoutState.js`); `promote()` explicitly NOT built this pass — no single call site to wrap, left for a second real consumer per the proposal's own §2 DEFER reasoning. 93 files / 1037 tests green (`npx vitest run src/pages/charts src/widgets`), zero member-visible change. | **DONE** — CP1, CP2, CP3 (MUST + partial SHOULD) all signed and built; `promote()` deferred by design |
...
| **S4** Context Bus | ✅ **CP1–CP3 SIGNED AND BUILT** (`f6df6dca1` CP2, `f4b06a886` CP3) | context-bus-spec §3.1 — one bus, both contexts as thin adapters, every consumer unchanged | CP3 (`HubContext.symbol` now derives from `useAppFocus`) was browser-verified per §7's own requirement — a real `MutationObserver` render-cost measurement across a full nav cycle, given the 2026-09-10 render-freeze precedent on this exact module. ⚰️ was *"CP4 (TickerHubContext), CP5, CP6, CP7 are each their own line"* / *"CP4+ buildable"* — **checked 2026-09-20, and CP4's own one-line table entry does not hold up.** GATE-S4 §4 describes CP4 as "second derivation: `TickerHubContext.sym` + `charts_mobile_sym`... one-file revert, S/M" — framing it as a duplicate of the same symbol-focus authority CP3 just fixed. Direct read of `app/src/components/mobile/TickerHubContext.jsx` shows this is **not the same shape as CP3 at all**: `TickerHubContext.sym` is the mobile ticker-preview-SHEET's own open/closed state (null when no sheet is open), driven by `TickerPopup.jsx:206`'s `if (isTouch) { openTicker(sym); return }` on ANY ticker tap anywhere in the app — a member can have the app's focus on AAPL while briefly previewing TSLA's sheet. Making it "derive from `useAppFocus`" per CP4's stated shape would be a **real regression**: the sheet would always show the app's global focus symbol instead of whichever ticker was actually tapped. Separately, `charts_mobile_sym` (CP4's other half) has exactly two WRITE sites (`TickerHubSheet.jsx:80`, `AiSearchPage.jsx:323`) and **zero read sites anywhere in the repo** — the "`MobileWorkspace` reads it on mount" claim this file's own Charts Hub V2 section made does not correspond to any file that exists (`MobileWorkspace.jsx` was not found; `ChartsWorkspace.jsx` has no reference to this key at all). Likely dead code from an earlier mobile-workspace design, not a live second authority. **CP4 as scoped is not signable — its own premise is wrong on both halves.** Not drafted as a proposal, per this session's own rule against forcing one to hit a round number. CP5 (`setVoicePageHint`)/CP6 (snapshot baseline)/CP7 (timeframe authority) were NOT investigated this pass and may or may not have the same problem — unverified, not assumed fine. | **DONE** for CP1–CP3; CP4 needs a real re-scoping (its stated shape is factually wrong), CP5–CP7 unverified |
...
| **D3** Realtime Streaming | ✅ **CP1/CP2/CP3 BUILT AND MERGED** `302f99e8e`/`af9fe21a6`/`21405e045` | realtime-streaming-spec | ⚰️ was *"UNSIGNED, no CP1 authorized"* — CP1 was owner-signed 2026-09-12. CP4 (the first real consumer, S7's price-level sweep) is the next open checkpoint and is a real architectural bet (§4 marks it the first non-inert one) — not proposed this session. ⭐ **CP4 SIGNED by the owner 2026-09-21** (fingerprint `657002015`) **and BUILT AND MERGED same day**, `feat/s7-price-level` `bb312cc18`: `_d3_prices_for()` runs beside `_prices_for()` (never replacing it), reconciled every tick against `bar_stream`/`bar_broadcaster`, capped at `D3_COHORT_CAP=200` (SHOULD, closing the parent gate's own gap 2); the source-agreement axis (agreed/poll_only/d3_only/not_comparable, never a rate) lands on the SAME comparison-span row the dark-vs-legacy axis already owns, via new `price_level_compare.record_source_outcome`; the heartbeat gained `d3_priced`/`d3_no_price` so a D3-only failure is visible independently of the poll arm. 9 new tests cover every §7 acceptance row; full scoped run 87/87 green. Still DARK — no delivery, no flag change beyond the existing dark-comparison flag. | **DONE** — CP1–CP4 all signed and built |
| **D4** Caching & Serving | ✅ **CP1-CP3 SIGNED AND BUILT** `40caca541` (`f2a2a68a6`/`388cad07c`/`dc5752b16`) · **CP4′ SIGNED AND BUILT** `23a8da7a9` (`3c070bee7`, confirmed on `origin/master`) | caching-and-serving-spec | ⚰️ was *"spec + gate written, UNSIGNED"* — CP1–CP3 were signed and built 2026-09-12/13 (this doc's own §2 build log already had this), and CP4′ (the corrected earnings_table cache-key assertion from F-D4-1) was signed 2026-09-18 and built the same window. ⚰️ was *"The one real remaining item: CP5 (`ticker_logos.py`'s resolution-decision cache) has no approval line at all — it is unscoped, not merely unsigned-with-a-line-written."* **CP5 SIGNED by the owner 2026-09-21** (fingerprint `ce60908e5`) **and BUILT AND MERGED same day**, `feat/s7-price-level` `c269143a7`, MUST scope only per the packet's own recommendation — files **F-D4-2**: SPEC-D4/GATE-D4's own CP5 naming described a different mechanism (an addressed-tier `cache.py` entry) than the disk-only fix actually built (the `.miss`-file provider-skip set that stops the daily 03:25 ET miss-retry from re-walking providers that already answered cleanly) — see the packet's §6; F-D4-2 itself stays open, unproposed, a separate future checkpoint if the owner ever wants the addressed-tier legibility work. `.miss` is now JSON with a legacy-format fallback (no backfill); `_fetch_sources`/`_fetch_sources_with_clearbit` return `(bytes, name)` and take `skip=`; a resolved hit writes a `.source` sidecar. 9 existing tests updated to the new contract + 9 new covering every §8 acceptance row, manual check confirms zero requests to a fully-skipped base chain and exactly one to Clearbit. No schema, no touch to `cache.py`. | **DONE** — CP1–CP5 all signed and built; F-D4-2 open, unproposed, not blocking |
...
| **A9** Screening | live surface | ✅ `scan-membership-change` CP3 merged `df937146c` — it fires dark; ⚰️ was *"flag OFF"* — **armed 2026-09-13, in-process `1`** (§4 Flag Ledger). **THE DARK READ HAPPENED 2026-09-21/22** (via `GET /api/admin/alert-taxonomy/dark-report/scan-membership-change`, browser-authenticated): `predicate_count: 0`, zero spans opened. Investigated rather than assumed a bug — wiring, scheduler registration (`api/main.py:6931`), and the feature flag (armed since 2026-09-13) are all correct and symmetric with every sibling alert type. The zero was a **documented, expected "honest zero"**: `docs/feature_flags.json`'s own ledger entry for this flag recorded on 2026-09-13 that the dry run projected N=0 because no member of the s7-dark cohort (admin/staff/test accounts only, by design — never ordinary members, confirmed against `api/services/rollout.py`) held a `screen_alert_subs` row. **RESOLVED 2026-09-21/22, owner-authorized**: the `smoke@uctintelligence.internal` account (already in the cohort via its admin role) subscribed to its existing "Oops Reversal" saved screen through the real `POST /api/screener/alerts` endpoint — confirmed via `GET /api/screener/alerts` returning `def_hash: sha256:b01b7707...`, `def_id: u_d83f4a3e00f8`, `mode: both`, `created_at: 1790049327`. This is a genuine row through the app's real subscribe path, the same shape the test suite's own canonical fixtures use — not a synthetic/hand-inserted row. It will be picked up by the next scheduled nightly sweep (`api/main.py:6931`, `SWEEP_HOUR_ET` + 20 min); `predicate_count` should move off 0 after that run. Still not ready for a CP4/FLIP ruling on ONE observation — needs several nights before `min_sessions_for_verdict` is met | **BLOCKED-OWNER** — read done, one real comparison now armed for the next nightly sweep; CP4 + FLIP still need several more sessions before a ruling is possible |
| **A10** Options & Flow | live, partner-owned | ✅ **BOTH halves closed 2026-09-21** — D3 CP4 and D4 CP5 both signed and built same day (§1.2). No named blocker remains. ⚰️ Worth restating from the D4-CP5 packet's own §2, since it was never a functional coupling: "nothing in Options & Flow reads a ticker logo" — this was always this programme's roster-completeness bookkeeping on D3/D4-as-systems, not a real dependency A10's members could feel. | **DONE** — no dependency remains |
| **A11** Breadth & Regime | live surface | ✅ `regime-change` CP3 merged `506eeee6d`, ARMED 2026-09-13 — but it compares **dark**. **THE DARK READ HAPPENED 2026-09-21/22**: 58 predicates tracked, up to 6 sessions observed, **zero flips seen in any of them** (`byStatus: {"NO FLIP OBSERVED": 58}`) — `min_flips_for_verdict=1` means none is even verdict-ready yet. Consistent with the harness's own documented cadence note: the regime classifier is a 15-minute TTL cache over the morning-wire breadth push, so the label can move at most about once a day — a real market regime change simply hasn't happened yet during the observed window. Still needs the one-regime ruling, D2 coverage, and this type's CP4 + FLIP | **BLOCKED-OWNER** — the flip, the one-regime ruling — + **BLOCKED-DEPENDENCY** (D2) — and now also **waiting on an actual regime change to occur**, confirmed by the read, not assumed |
| **A12** Watchlists | half-live | ⚰️ was *"S5 + S6"* as a dependency block — both are now DONE (§1.1: S5 CP2–CP4 signed+built; S6 CP2'/CP3/CP4 built, CP5 closed by ruling). A12 inherits no dependency block anymore; matching the A2/A14 correction style, what remains is that A12 **needs its own new checkpoint** proposed against the now-unblocked systems. ✅ **CP1 SIGNED by the owner 2026-09-21** (fingerprint `acaa29de6`) **and BUILT AND MERGED same day**, `feat/s7-price-level` `20f3cacec`: one new test file, `tests/test_a12_s6_consistency_rail.py` — asserts `member_interest.py`'s watchlist/flagged buckets agree with `watchlist_service.py`'s own membership definition, with a mandatory non-vacuity control and two mandatory mutation proofs (both verified failing under an injected break), plus the two named gaps (column-preset persistence, no S5-shaped saved object yet) pinned as falsifiable assertions designed to flip the day CP2 lands. One correction made against the packet's own test-plan prose, by reading source rather than trusting the citation: `_watchlist_syms` carries no `is_flagged_list` filter at all (by design), so the "watchlist" bucket is the union of every list including flagged — the agreement test is built on a user with no flagged activity, where the two definitions genuinely coincide; the flagged-mixed case is covered by the (weaker, true) non-vacuity assertion instead. Zero product code, zero schema, zero new route. 7 new tests, 102 total including every sibling suite, zero regressions. No A12 PRD/spec exists still — this checkpoint does not substitute for one (flagged, not fixed). | **DONE** — CP1 signed and built |
| **A13** Journal | live (528 files) | ✅ `position-risk` CP3 merged `6a67a4b5d`, ARMED 2026-09-13 — but it compares **dark**. **THE DARK READ HAPPENED 2026-09-21/22**: 8 predicates, 4 past their session minimum (`verdict_ready`), but every one of those 4's actual observations landed as `not_comparable` (agreed/new_only/legacy_only all 0) — no position has come close enough to its stop yet in the observed window to generate one real side-by-side comparison. Still needs D2, S5, and this type's CP4 + FLIP | **BLOCKED-OWNER** — the flip — + **BLOCKED-DEPENDENCY** (D2, S5) — and now also **waiting on a real stop-hit/stop-proximity event to occur**, confirmed by the read, not assumed |
| **A14** Portfolio & Risk | ✅ **CP1 SIGNED by the owner 2026-09-21** (fingerprint `417b6b853`) **and BUILT AND MERGED same day**, `feat/s7-price-level` `f662b84d5`: `api/routers/portfolio_heat.py` (`GET /api/portfolio/heat`, per-router `require_paid`, a byte-identical pass-through of `portfolio_heat.portfolio_heat()`'s own return dict) + `app/src/pages/PortfolioHeat.jsx` (new page — risk-heat/caps, notional exposure, regime, concentration flags, per-position table with placeholder-stop positions rendered flagged not hidden, by-sector) + a nav entry ("Portfolio Risk", Journal group) gated the ordinary way (`FREE_PAGES` untouched — `['/morning-wire']` only, per G1's "no free/paid split, only paid"). **This is the first of the five 2026-09-21 checkpoints that is genuinely member-visible** — a member can now navigate to `/portfolio-heat` and see the numbers `portfolio_heat.py` already computed. 4/4 tests named in the proposal's §5 pass, plus 2 non-vacuity controls; zero regressions across 11 backend router+AST-rail tests, 21 frontend nav-taxonomy tests, 16 backend nav-resolution tests. Corp-actions display remains explicitly DEFERRED (named in the proposal's §2, not this checkpoint's scope). | DEC-08 (⚰️ was labeled "D8"; see DEC-08 row) + **G1, a NEW owner question (OWNER_INPUTS.md), not F-09** — both answered, see history below | ⚰️ **DEC-08 CONFIRMED NEEDED 2026-09-20** (owner, direct answer) — unblocked the *deferral*, not the whole system. ⚰️ was *"inherits F-09's real provider gap"* — **wrong pairing, corrected 2026-09-20**: F-09's class-G provider gap (no M&A/spinoff/rights/buyback/ticker-change data source) belongs to D5's corp-actions EVENT calendar, a separate system; `portfolio_heat.py`'s risk-heat computation needs no corporate-actions data at all and has no provider gap. The real remaining blocker, per the original 2026-09-12 bucket-sort research (`10-roadmap/2026-09-12-a-series-bucket-sort.md:357`, "owner-bound twice over"): **who may see aggregate risk** — never actually answered (S9's OI-03/OI-12 settled VENDOR licensing tiers, a different question from MEMBER subscription-tier gating). **G1 ANSWERED 2026-09-20**: *"we no longer have a free and paid tier, only paid"* (verified against `app/src/constants/freePages.js`, `FREE_PAGES = ['/morning-wire']`) — closed as: gate the new page as an ordinary paid-member feature, no elevated tier, not admin-only. **The scoped CP1 proposal this closed against**: `docs/terminal-research/12-decisions/gates/a14-portfolio-heat-cp1-scoped-proposal.md` — one read-only route + one page + one nav entry, all reading `portfolio_heat.py`'s existing output verbatim, gated with the same `require_paid` idiom other paid routes already use. | **DONE** — CP1 signed and built |
...
|---|---|---|---|
| **F-D2-1** | D2 CP2 — fundamentals has no declaration to ratify | ⚰️ was *"OPEN — write the declaration if writable, else EXCLUDE"* — **CLOSED 2026-09-22.** ✅ **SIGNED by the owner** (fingerprint `454e11421`) **and BUILT AND MERGED same day**, `feat/s7-price-level` `1a15a752a`: `tools/build_canonical_address_book.py` gained `earnings_table_store()`, AST-derived from `earnings_table.py::_build()`'s `_sanitize({...})` dict literal, `_build_quarterly()`'s reported/forward branches (correctly excluding `_sort` via detecting its own `.pop()` call), and `annual_financials.py`'s two `row = {...}` literals (confirmed identical, not merely assumed). `api/data/canonical_address_book.json` gained a new `earnings_table` store entry, authority `"derived"` — deliberately **zero** `metrics` entries, since a snapshot is two lists of rows with no single as-of column, and flat `metric.column` entries the way `ohlcv.c` is declared would misrepresent that shape. 5 new tests, each independently re-deriving the shape from source rather than trusting the builder's own function; the mutation proof was verified for real (corrupted the checked-in JSON, watched the comparison go red, restored it clean). Also fixed, per the proposal's own MUST-BUILD: `test_signature_router.py`'s vacuous open==close==95.0 fixture. 152/152 tests pass across every touched/adjacent suite. ✅ **CHERRY-PICKED TO PRODUCTION `master` as `771d195a0`, 2026-09-22** (bundled with F-D2-3/D2-CP5 and a `bars_ordinal_census.json` environmental-drift regen). Railway `web` confirmed `SUCCESS` on `771d195a0` specifically (polled by commit SHA, not status alone); verified live via `/api/health` returning `uptime_seconds: 80`, a fresh boot | **DONE** — signed, built, and deployed |
...
| **F-D2-3** | D2 CP1 | ⚰️ was *"OPEN — needs re-read"* — **CLOSED 2026-09-22.** ✅ **SIGNED by the owner as D2 CP5** (fingerprint `06ddbe081`) **and BUILT AND MERGED same day**, `feat/s7-price-level` `8d3688213`: `tools/bars_ordinal_census.py`, a whole-repo AST sweep classifying every module that calls `bars_sqlite.get_bars`/`get_bars_before`/`get_bars_since` as `named_access` (goes through `address_book.row_position`) or `positional` (a fixed-ordinal tuple/list unpack, an int-literal subscript in 0-5, or delegation to a known positional formatter — `bars_fetch._fmt_sqlite_bars`, independently confirmed positional by source read, covers the cross-function case where a module fetches bars and hands the raw rows to that helper rather than unpacking them itself). **Measured, not the pre-signature estimate**: 1,380 modules scanned, **31 positional, 1 named-access** (`ticker_returns.py`) — the AST sweep found more than the earlier hand-enumeration (the "21+ across 19 files" figure above), which is exactly the discrepancy this checkpoint's own tool exists to replace with a derived count. Refuses rather than guesses on an unrecognized call site (verified real, not just documented, via a dedicated test). Caught and fixed one real bug while building it: `ast.walk()` traverses in tree-structural order, not source-line order, so the naive "first hit wins" design cited an irrelevant line on `api/main.py` — fixed to take the earliest-by-line-number hit among all matches, with a regression test. Also re-applied, per this checkpoint's own MUST-BUILD list: `test_signature_router.py`'s vacuous open==close==95.0 fixture, now o=92.0/c=95.0 so an ordinal swap is actually catchable. 9 new tests (non-vacuity, independent re-parse, checked-in-matches-fresh comparison, mutation-proved CAN-FAIL, refusal path + negative control, evidence-ordering regression); 127 passed across every touched/adjacent suite. Checked-in inventory: `api/data/bars_ordinal_census.json` (gitignored, force-tracked). ✅ **CHERRY-PICKED TO PRODUCTION `master` as `771d195a0`, 2026-09-22** (bundled with F-D2-1 and a census regen made necessary purely by other, unrelated commits shifting `modules_scanned` 1,380→1,423 with zero findings added or removed — verified by diff before regenerating) | **DONE** — signed, built, and deployed. Detection-only: no reader in the census was migrated, no production behavior changed |
...
| **F-S7-EP-1** | event-proximity | ⚰️ was *"OPEN"* — **THE DARK READ HAPPENED 2026-09-21/22**: 26 predicates, 41 total agreed observations across them (the most real signal of any of the four blocked alert types), but **no single predicate individually crossed its own verdict-ready bar yet** — the agreement is spread thin across predicates rather than concentrated in enough of any one. Closest of the four to a ruling, not there yet. Still OPEN | the dark read is now done; still needs individual predicates to accumulate more sessions before a ruling is possible |
...
### 3.1c ⛔ FINDINGS OPENED BY THE 2026-09-21 RE-VERIFICATION

Registered with evidence. **Not fixed**: the pass that found them was a status re-derivation, and
the first one is a guard, which is not edited without its own review.

| id | finding | evidence | closes when |
|---|---|---|---|
| **F-ENVCHECK-1** | **`tools/terminal_next_env_check.py`'s check 1 tests ANCESTRY, and this programme's code reaches master by CHERRY-PICK (`merge-run`, RESUME §0), so every authoring SHA reads "unpublished" forever.** It prints `FAIL ... 10 unpublished commits` + `STOP — a measured failure` on a clean tree, which would stop the weekly autonomous run on every clean Saturday. It is the mirror of the 2026-09-14 defect the same tool was built to fix (that one over-counted a stale `origin/<branch>`; this one under-counts a rewritten history). | run from `s7-price-level` on 2026-09-21: tool exit **1**; `git cherry origin/master HEAD` → **77 `-` (already on master under other SHAs), 2 `+`**; 9 of the 10 commits the tool names are `-`, the tenth (`87b5735f4`) is a genuine `+` | check 1 compares by **patch-id** (`git cherry`), keeps the UNREADABLE-vs-FAIL split, and carries a control proving a genuinely unmerged commit still FAILS — `87b5735f4` is a live positive today. ⛔ Ancestry stays as the fast path; only the fall-through changes |
| **F-AUDIT-3** | ✅ **CLOSED BY BUILD, NOT BY RULING — 2026-09-21.** Originally: §0 counted D3 and D4 **DONE** while A10 was **BLOCKED-DEPENDENCY on exactly D3-CP4 + D4-CP5** (§1.3) — a checkpoint another system waits on is not non-blocking, so the classification contradicted itself. **Both D3-CP4 and D4-CP5 are now signed and built** (§1.2), so the contradiction is gone by construction rather than by a ruling: A10 no longer waits on either system-shaped checkpoint. The classification question this finding raised (does an "additional, non-blocking checkpoint" stay non-blocking when another system genuinely waits on it) never needed answering here, because the wait itself resolved first. It remains a live question for any FUTURE case of the same shape (a still-open example: D5's CP2, currently unsigned and read by nothing, per D5's own §1.2 row) — not re-opened as a numbered finding unless a new instance surfaces. | §1.2 D3/D4 rows vs §1.3 A10 row, both now current | resolved: both halves built. A10's remaining gap is D4's own F-D4-2 (unproposed, not blocking) — see the D4/A10 rows directly |

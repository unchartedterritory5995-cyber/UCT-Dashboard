...
|---|---|---|---|
| **F-D2-1** | D2 CP2 — fundamentals has no declaration to ratify | ⚰️ was *"OPEN — write the declaration if writable, else EXCLUDE"* — **INVESTIGATED 2026-09-22, confirmed WRITABLE**: `earnings_table.py`'s payload is a single-function-built, internally shape-versioned, stable 6-key shape (no ticker/source-dependent key variation within the kind) — the cross-module "ten metric spellings" CP2.1 measured is vocabulary divergence between files, not shape instability inside one persisted snapshot, and only the second would make a declaration dishonest. **A scoped proposal now exists**: `gates/d2-f-d2-1-fundamentals-declaration-scoped-proposal.md`, unsigned | a scoped proposal is drafted and ready for the owner's signature; not built |

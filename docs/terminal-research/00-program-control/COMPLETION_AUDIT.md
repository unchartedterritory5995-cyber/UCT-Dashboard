...
**Unique-system arithmetic, so the 32 is checkable, not asserted:** 23 DONE + 1 RULED-HOLD (S7)
+ 1 BLOCKED-DATA (D2) + 1 BLOCKED-OWNER-only (A9) + 2 BLOCKED-SPEC-READ (A12, A14) + 2 single-tag
BLOCKED-DEPENDENCY (S8, A1) + 2 dual-tagged OWNER+DEPENDENCY (A11, A13, each counted once
here) = 23+1+1+1+2+2+2 = **32**. The breakdown table's BLOCKED-OWNER (3 = A9 + the 2 dual-tagged)
...
A10 moved DONE (no dependency remains), and the DONE count moved to **23**. See F-AUDIT-3's own
row (§3.1c) for the full history.

...
| **F-AUDIT-3** | **The DONE rule and A10's blocker contradict each other.** §0 counts D3 and D4 **DONE** because their new CP4/CP5 are "additional, non-blocking checkpoints" (the D5-CP2 precedent) — yet A10 is **BLOCKED-DEPENDENCY on exactly D3-CP4 + D4-CP5** (§1.3). A checkpoint that another system waits on is not non-blocking. Either A10's blocker is misnamed, or D3/D4 are not DONE. S1-CP3 has no dependent, so it is unaffected. | §1.2 D3/D4 rows vs §1.3 A10 row, read side by side | **an owner/classification ruling**, not a build: (a) A10 is blocked on *proposals awaiting signature* — a state of its own, not a dependency on unfinished systems, and D3/D4 stay DONE; or (b) D3/D4 move out of DONE. **Counts under (b): DONE 20, and D3/D4 join BLOCKED-SPEC-READ (4).** The headline count below is left at 22 under (a) until ruled |

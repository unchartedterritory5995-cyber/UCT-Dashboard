...
| **F-S7-RC-3** | path B has no suppression | ⚰️ was *"CONFIRMED, OPEN"* — **FIXED 2026-09-20**, `feat/s7-price-level` `87b5735f4`: `_regime_shift_already_told()` reads the most recent `regime_shift` insight's own headline and suppresses a repeat for an unchanged summary; 42-test regime-change suite + 238 total across every `voice_proactive_service` caller, 0 failures. ⚰️ **ALSO NOT "shipped" — corrected 2026-09-21:** `87b5735f4` is on the branch only. `git cherry origin/master HEAD` (patch-id, run from the code tree) marks it `+`, and `_regime_shift_already_told` has **0 hits** in `api/services/voice_proactive_service.py` on `origin/master` @ `a392afd11`. Production is running WITHOUT it. What closes the gap is the owner-gated cherry-pick through `merge-run` + a deploy authorization | ✅ fixed on the branch · ⛔ **NOT deployed** — awaiting the cherry-pick |

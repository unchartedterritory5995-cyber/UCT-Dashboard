...
their own blocker columns: 22 DONE + S7 + D2 + A9 + A11 + A13 + A12 + A14 + S8 + A1 + A10 = 32. Also
checked every SHA the DONE rows cite as merged: 38 checked, **0 unresolved** — 24 are ancestors of
`origin/master`, 8 are cherry-picked (the authoring SHA is not an ancestor, its patch-id IS on master;
S6 ×3, D3 ×2, D5 ×3), 6 are `sign_gate` fingerprints and not commits at all (S2 CP3–CP6, D4 `40caca541`
and `23a8da7a9`). ⛔ **Two classes of false alarm to know before you re-run this check:** an ancestry test
reports every cherry-picked authoring SHA as "not merged" (F-ENVCHECK-1), and a `cat-file` test reports
every fingerprint as "not an object". Use `git cherry` for the first and the approval block for the second.
⚠️ **NOT re-verified:** the numbers inside each proposal (row counts, flow-worker non-strand), any
production data, and S4 CP5–CP7's premises. ⚠️ **One classification conflict is open — F-AUDIT-3**
(§3.1c): D3/D4 are counted DONE while A10 waits on exactly their new checkpoints.
...
| **A10** Options & Flow | live, partner-owned | ⚰️ **D3's half is now closed** — D3 CP4 signed + built 2026-09-21 (§1.2). The gap narrows further, to **D4's CP5 alone** (ticker_logos) — proposal drafted, awaiting the owner's signature (§1) | **BLOCKED-DEPENDENCY** — narrower than a whole-system wait, and narrower than it was |
...
| **F-AUDIT-3** | ⚰️ **HALF CLOSED BY BUILD, NOT BY RULING — narrowed, 2026-09-21.** Originally: §0 counted D3 and D4 **DONE** while A10 was **BLOCKED-DEPENDENCY on exactly D3-CP4 + D4-CP5** (§1.3) — a checkpoint another system waits on is not non-blocking, so the classification contradicted itself. **D3-CP4 is now signed and built** (§1.2), so that half of the contradiction is gone by construction, not by the ruling below — A10 no longer waits on anything D3-shaped. **D4-CP5 is unsigned and still open**, so the identical question remains for D4 alone: D4 is counted DONE while A10 (§1.3) still names D4-CP5 as a live blocker. | §1.2 D4 row vs §1.3 A10 row, read side by side | **the same owner/classification ruling, narrower now**: (a) A10's D4 wait is a *proposal awaiting signature*, a state of its own, and D4 stays DONE; or (b) D4 moves out of DONE until CP5 ships. **Counts under (b): DONE 21** (D3 already counts DONE either way — it is fully built now), **D4 joins BLOCKED-SPEC-READ (4).** The headline count is left at 22 under (a) until ruled |

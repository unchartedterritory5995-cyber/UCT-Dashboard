...
| Joystick hub absent on 6 non-section routes (touch smoke FAILED 2026-09-24) | `hub_nav_smoke.py --auth --touch` on production: hub root not in the DOM on `/model-book`, `/formulas/reference`, `/desk`, `/journal`, `/portfolio-heat`, `/community`; present on the other 13. The joystick programme's 9/12 touch record had it SHOWING on four of those. Not caused by this program's deploy (docs-only, no runtime reader — verified). **Settled which half is right:** `app/src/hub/HubContext.jsx:185-189` states the design — a route hubRoutes doesn't recognize "leaves `mode` exactly as it was … the hub keeps showing whatever section was last active" — so the absence is a regression against the hub's own stated design (sometime after the 9/12 pass; W4 glass on 9/18 is the obvious suspect, not proven), not a stale tool expectation. Record: `docs/plans/joystick/smoke-runs/2026-09-24T23-17Z-touch.md` | Joystick owner — a real regression to find; no rollback lever fits (the docs deploy didn't cause it; the kill switch would remove the hub from the 13 routes where it works) |

...
|---|---|---|
| ⭐ **S7 cross-type inventory — NEW 2026-09-25, and it moves the next decision** (`tools/s7_arming_inventory.py`, evidence `2026-09-25-s7-arming-inventory/`) | The completion plan's §1 ordering table is from 09-11 and marks 5 of 7 types as work to do; **all seven already have a `*_compare.py` AND a `*_projection.py`, and six are observing real spans on production.** Measured: 7 types · 181 predicates · agreed 144 · **new_only 0** · legacy_only 2344 · not_comparable 63. ⭐ **`catalyst-match` is the cleanest evidence in the taxonomy — 58 predicates, 58 verdict-ready, 80 agreed, zero of all three other outcomes**, which is what a flip-ready absorption looks like on the evidence axis. ⛔ NOT the same as "may flip": §2's filing-watch parity precondition and §2a's checklist (pinned shapes, a named call site with its rail, a liveness stamp) are unmeasured by a counter. `indicator-condition`'s 0 predicates is its CORRECT state (§2b sequences it behind D2). ⚠️ `event-proximity` has 59 agreed against 59 not_comparable and 0 ready; `position-risk` is 8/8 ready with `agreed 0`, i.e. no tick ever had both sides fire — neither is a defect on this evidence, neither is agreement either | **Owner's, and it is the biggest S7 call available:** `catalyst-match` is the strongest candidate for the next increment, and the whole taxonomy's risk is one predicate (price-level `08d68edb`, the only disagreement in 181 — `new_only` is 0 everywhere, so no type would send an EXTRA alert on flip). Re-run the tool any time; `--self-check` proves its verdicts can fail |
...
| `/options-flow` takes >45 s to reach DOMContentLoaded in the first minute after a **FRONTEND-TOUCHING** deploy (found 2026-09-24; the word "ANY" struck 2026-09-25 on measurement) | Reproduced on two consecutive fresh boots, on two different builds, by the same smoke at +30 s; gone by +5 min. **The two variables were conflated and are now separated.** Measured 2026-09-25 19:41Z on `5bad63301`, whose diff against the last frontend deploy (`9a8c7163c`) touches **zero `app/**` files**, so every hashed chunk name is unchanged: on a **34-second-old pod** `/options-flow` reached DOMContentLoaded in **3.11 s**, with **30 of 30 assets Cloudflare `HIT`** and the slowest response at 6.13 s. Warm (724 s): 0.33 s. ⚰️⚰️ **THE NARROWING WAS TESTED THE SAME NIGHT AND IT FAILED. WITHDRAWN.** The claim was *"what costs the 45 s is new hashed chunk names = a genuine edge-cache MISS on the heaviest route, which only a frontend change produces"*, and it shipped with its own falsifier: *the next `app/**` deploy should reproduce the slow fresh load.* That deploy was `bfac72d61` (the layout version stamp, 00:52Z), whose chunk hashes genuinely changed — `indicatorRegistry-BJfN7QgD` → `-BRjs8kBX`, `GlobalVoiceLayer-EEIBWt2L` → `-rum9a_pV`. **Measured at pod age 49 s: DOMContentLoaded 3.69 s, and 27 of 28 assets came back Cloudflare `HIT`.** The slow load did not reproduce. ⛔ **So the cause is UNEXPLAINED and both narrowings are unsupported** — neither pod age (3.11 s at 34 s) nor chunk freshness (3.69 s at 49 s with new hashes) reproduces it. Four fast cold-pod measurements now stand against the two original >45 s failures. ⭐ The surviving confounds, neither of which this probe controlled: **market session** (both original failures were during RTH; three of the four fast reads were not, which is the very reason `MARKET_HOURS_FLOOR_S = 720` exists) and **the instrument** (`hub_nav_smoke`'s 45 s `goto` budget vs this probe's own 150 s one — a different question asked of the same route). ⚠️ A new chunk URL coming back `HIT` at 49 s also means **something warmed the edge before the probe did**, so this test cannot separate "chunks are cheap" from "somebody else paid for them" — which is the next thing to control for, not a conclusion to draw. The member-facing statement stands unchanged and unexplained: two measured runs saw `/options-flow` miss a 45 s budget on a fresh pod during market hours. A member opening Options Flow right after a frontend deploy still waits. The post-deploy smoke must still run on a warm pod (≥5 min) | Owner / platform — deploy-time cost, not a code defect in any one commit |
...
2. ⛔ **Before trusting `s7-alerts-completion-plan.md` §1's ordering table, run `python tools/s7_arming_inventory.py`** — that table is from 2026-09-11 and marks five types as work to do which are substantially built; the tool derives the live state and `2026-09-25-s7-arming-inventory/results.md` holds the reading.

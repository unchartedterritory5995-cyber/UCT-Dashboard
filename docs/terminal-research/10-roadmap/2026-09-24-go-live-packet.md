...
| A10 Options & Flow | CP1 built (`FlowExplainButton.jsx`, tested). **CP2 prepared 2026-09-25 as draft PR #194** (`6ea3d40aa`): 6 inserted lines in `OptionsFlow.jsx`, 0 modified, 0 deleted, plus a 7-case structural guard; every rail green except `reachable.test.js`, which correctly demands its now-stale "held unmounted" entry be deleted (owed in the PR, refused by the authoring session's tool policy). The gate lets the PR open before the ack and forbids merge without it | **2026-09-25, later:** precondition released by the owner; landed, H15-rolled-back on a fresh-load smoke timeout, exonerated by the controlled comparison (the reverted build fails the same way), **re-landed by the owner's push and LIVE as `7878374fb`** — `RESUME-HERE.md` §3/§5/§7 carry the full trail | Done |

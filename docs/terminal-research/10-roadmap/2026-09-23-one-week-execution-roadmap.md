...
| 3 | ~~S7 Alerts — real remaining question~~ | A9's own S7 dependency is a *different* trigger type (`scan-membership-change`), unaffected either way — time-gated on several more nights of real data before a CP4/FLIP ruling is even possible, not a decision or build task. | ✅ **RE-AFFIRMED 2026-09-23 — owner chose C, keep the HOLD**, given the real reasoning from `DECISION_CARDS_2026-09-18.md` CARD 6 (unmade persistence semantics, thin/partly-synthetic n=10 sample, zero trendline coverage). No code or config change; legacy `watchlist_alerts` keeps firing. Recorded directly in CARD 6 itself, not only here. |
...
| 7 | ~~OI-17 — should these five endpoints require authentication?~~ | — | ✅ **CLOSED AND SHIPPED LIVE, 2026-09-23.** Owner delegated ("idk your call"); gated the four non-partner routes (`/api/live-prices`, `/api/snapshot`+`/{ticker}`, `/api/movers`+`/api/extended-movers`, `/api/gex/data`) behind `Depends(get_current_user)`, verified zero legitimate anonymous callers first (every real frontend caller confirmed inside `AuthGuard`, no external/Discord-bot HTTP caller found). 73 tests re-run fresh (13 new/updated + 60-test regression check), hygiene clean. Pushed `caebdab16`, confirmed `SUCCESS`, live on `origin/production`, and **the actual behavior change verified directly**: anonymous `curl` to both `/api/live-prices` and `/api/movers` now returns `401` (was `200`). The fifth endpoint (`/api/live/massive/ticker-flow`) stays out of scope — partner file, part of the owner's own Ravi conversation. ~~One small stale-comment follow-up found, not fixed (out of scope): `tests/test_provenance_quote.py`'s docstring cites the old no-auth behavior as its own precedent — no functional impact, worth a look someday.~~ **Closed:** the docstring was corrected in `6e7b10407` (2026-09-24) and the endpoint itself joined OI-17's class in `7e0ab34c2` (2026-09-25) — anonymous → 401, proved live. |
| 8 | ~~Approve a narrow scope for registering A11's breadth/exposure metrics into D2~~ | — | ✅ **CLOSED AND SHIPPED LIVE, 2026-09-23.** Owner delegated ("not sure, your call"); approved, recorded honestly as delegated (matching Packet W's precedent) rather than claimed as direct owner authorship. The agent's own investigation found the original framing wrong in an important way — breadth's metrics are NOT shaped like `fund_snapshots` (which was correctly left unaddressed); `breadth_snapshot_numeric` has a genuine single-column as-of and a flat metric dict, and breadth's metric names are already a governed in-repo catalogue — so real registration was the right call, not a declared non-addressing. 51 breadth metrics registered (193 total in the address book now), 49 tests re-run fresh, hygiene clean. Pushed `985a3761a`, confirmed `SUCCESS` and live on `origin/production`. |

...
10. **The product's thesis, its falsifiers, or its non-goals.** ⚰️ **RE-POINTED 2026-09-26 — this cell was stale in TWO places, one of them caught by the gate-item-18 author on delivery.** (a) `product-architecture.md` no longer owns the thesis and non-goals alone: checklist row 18's pair — `05-product-strategy/product-vision.md` and `non-goals.md` — now own them, and the vision is the owner's own sentence verbatim (CARD 25). The Phase-2 file is cited by them, not replaced. (b) `10-roadmap/mvp.md` is **no longer NOT STARTED** — it is DRAFT COMPLETE (823 lines, 2026-09-26) and does own which workflow and what onboarding means. §3.4 still cites and restates nothing, which is the part that was always right.

...
role: >
  ...
  glossary: it proposes twenty metrics, each with a stated instrument, and it says of each
...
⛔ **The arithmetic, because it is the actual argument.** At n = 26, one account is **3.85 percentage points**; at n = 29, **3.45**. OBS-1 rejected n = 40 for a latency percentile *because* its nearest-rank resolution is 2.5 points and preferred n ≥ 60 at ~1.7. **An engagement rate over this population has worse resolution than a percentile this programme already ruled too coarse to compare.** Item 37's RB-1 reaches the same verdict from the rollout side and rules percentage staging out on it: *"a fraction over 26 accounts yields a cohort nobody can reason about"* (`rollout-rollback.md:104`), noting that percentage buckets **browsers, not users**.

⚠️ **And the denominator is not one number.** `users` = 26 on 2026-09-12; the preference-bearing cohort = 29 on 2026-09-25. Two tables, two dates, no contradiction — and no stable denominator either. ⛔ **A rate needs one denominator with one owner.** Until a member population exists, this programme does not have that, which is a second-authority problem about the one quantity every business metric divides by.
...
### 3.3 The product's own goals, cited so this document does not invent a second set
...
```
grep -c '^#### \(MQ\|PH\|EQ\|CO\)-' docs/terminal-research/10-roadmap/success-metrics.md
grep -o '^#### \(MQ\|PH\|EQ\|CO\)-[0-9]*' … | sort | uniq -d     # duplicate-id check
grep -c '^- \*\*Computable today\.\*\*' …                          # field-2 coverage
```

**Result: 20 metrics, 0 duplicate ids, 20 of 20 carrying all four fields.** Distribution: member-visible quality **7**, product health **4**, engineering quality **6**, cost **3**.
...
| **NO** — no instrument exists; the sibling owning the gap is named | **8** | MQ-5, MQ-6, PH-3, EQ-2, EQ-3, EQ-6, CO-2, CO-3 |
...
| metrics with a target labelled **DERIVED** (a contract, or a number already chosen and shipped by a named authority) | **8** | MQ-1†, MQ-7, PH-2, PH-3, EQ-1, EQ-2, EQ-6, CO-1 |
...
† **MQ-1's threshold is DERIVED (CARD 16 chose 250 ms) while its statistic has never been computed.** That combination is the whole point of this document and it is why the two are separate columns.
...
| **Which surfaces members use** (route breadth, panel-open counts) | a member cohort | `page_views` holds **4,949** rows and OI-21 calls it an **existence-check, not a rate**. ⛔ And an aggregate route-breadth read was **attempted twice and refused both times** under production-read permissions (`DECISION_CARDS_2026-09-26.md:440-441`) — so this is blocked on a grant *and* on a population. |
...
| 5 | **Any tier metric — upgrade rate, tier mix, ARPU-by-tier, a locked-behind-a-higher-tier funnel** | **CARD 17, owner ruling 2026-09-26: *"there is one paid tier only that is it."*** With one paid tier there is nothing to compare. The entitlement axis is **binary**. TEST-8 and item 36 §6's header already bind the same way; item 37 forecloses tier staging. |
...
| **SM-9** | Cost gating at v1 | **The only cost bar in force is cap coverage per member-facing AI lane (CO-1). No cost-as-a-share-of-revenue bar is adopted** | R-18's own mitigation, which names A-02, and E-06's conclusion that *"the economic risk is … a population without a ceiling."* ⛔ A revenue-share bar flips pass/fail on an undecided price: the same feature set reads 1.4–3.6 % or 9–24 %, and the high case 78 % | The owner naming a monthly AI ceiling (OI-10) — which makes a ceiling-ratio computable immediately and does not need a price at all |
...
10. **The product's thesis, its falsifiers, or its non-goals.** `product-architecture.md` owns them; §3.3 cites three of its tests and restates none.
...
13. **That any metric here can fire.** ⛔ Nothing was run. §2.4 states the obligation and names the four proof methods; discharging them is engineering work, and until it is done every bar below is a claim about an artifact and not about the world.

---

## GAPS — what this document could not reach

1. ⛔⛔ **Not one measurement is mine, and the strongest baselines here are `n = 1`.** MQ-2 (31.1 MB), MQ-4 (the 16-cell figures) and EQ-3 (+7.9 MB/min) each rest on a single run, each taken after the close, and SM-8 exists because of it. ⚠️ MQ-2's is worse than n = 1: its own evidence file says *"No true cold pass"* (45 of 107 resources from browser cache) and *"Market closed"*, **so the RTH magnitude of the cold-board cost is unestablished** and three of my four member-visible quality baselines are after-hours readings of a market product.
...
4. ⚠️ **The 26-vs-29 denominator is unreconciled and I chose not to reconcile it.** Two tables, two dates, no contradiction — but nothing in the programme says which is the population a rate would divide by, and a metrics document should not settle a denominator by picking the one that suits it.
...
11. ⚠️ **Eleven of the twenty metrics are, in some part, pointers to gate items 25 and 36.** That is deliberate — the alternative is a second authority — but it means this document's usefulness is bounded by theirs, and if either is re-cut, §4 must be re-read rather than assumed to have travelled.
12. ⛔ **The third subagent input I dispatched (program-control facts) had not returned when this was written**, so `CRITICAL_PATH.md`, `OWNER_INPUTS_REQUESTED.md` and the risk register were read directly by me instead, by targeted grep rather than in full. ⚠️ **A full read of `OPEN_QUESTIONS.md` (OQ-01…OQ-16) was not done**, so an open question bearing on a metric here may be unrepresented.
...
- `00-program-control/GOVERNING_PRINCIPLES.md:78` and `00-program-control/charter/OWNER_SEED_FACTS.md:61` — the $7 weekly promo and the one-paid-tier seed.

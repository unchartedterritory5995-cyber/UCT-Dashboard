...
| **CP2** | **The ledger, INERT.** `corp_actions.db` + its one self-declaring `CREATE TABLE` literal + ~~the D2 builder extension that derives its five metrics by AST~~ **the D2 builder extension that derives its metrics by AST from the store's own `CREATE TABLE` literal** *(NEEDS-REWORD applied 2026-09-15. ⛔ **THE STATED REASON WAS WRONG AND IS RETRACTED 2026-09-15:** it claimed the count "was never enumerated anywhere for `corp_actions`". It is — **`reference-corp-actions-spec.md` §4.1 enumerates exactly five metric addresses**: `numerator`, `denominator`, `cash_amount`, `effective_date`, `state`. The audit searched the PACKET and not the SPEC, and reported an absence it had not looked for. **The original "five" was CORRECT.** ⭐ The reword still stands, on the standing rule alone — an assertion carries no derived number, and a count typed beside a clause that says the builder derives by AST is the enumeration-beside-its-source defect even when the number happens to be right.)*. Written by nothing, read by nothing; the derivation rail and the axis report extended to the new store. | no — inert data, and the new module is outside flow-worker's closure if it adds no import to `api/services/{bars_*,entity_master}` | **S/M** |
...
| **CP4** | **The split list, DARK.** `bars_sanitize._fetch_meta`'s FMP call is dual-computed against D5's ledger: both computed on every call in test and on a sampled fraction in production, log-only, never raising, **serving the legacy value**. ~~Four outcomes, never a rate.~~ **The outcomes the dual-compute enumerates, never a rate.** *(NEEDS-REWORD applied 2026-09-15: same rule — the assertion names the kind of thing, the build produces the count.)* | ⚠️ **YES — `bars_sanitize.py` is an INERT STRAND** (flow-worker RUNS it, does not WATCH it). Must be classified live-or-incidental BEFORE merge; if live, a marker bump and an after-hours window | **M** |

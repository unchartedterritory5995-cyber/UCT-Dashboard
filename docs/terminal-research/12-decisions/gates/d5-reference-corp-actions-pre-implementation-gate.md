...
| **CP2** | **The ledger, INERT.** `corp_actions.db` + its one self-declaring `CREATE TABLE` literal + ~~the D2 builder extension that derives its five metrics by AST~~ **the D2 builder extension that derives its metrics by AST from the store's own `CREATE TABLE` literal** *(NEEDS-REWORD applied 2026-09-15: the count was never enumerated anywhere for `corp_actions` — D2's "five" are the `ohlcv` BARS metrics. A clause that says the builder derives by AST must not also assert how many it will find.)*. Written by nothing, read by nothing; the derivation rail and the axis report extended to the new store. | no — inert data, and the new module is outside flow-worker's closure if it adds no import to `api/services/{bars_*,entity_master}` | **S/M** |

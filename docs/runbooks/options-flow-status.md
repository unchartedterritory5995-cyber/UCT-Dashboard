...
| Job | Status | Evidence |
|---|---|---|
| Python failure baseline | **DONE-VERIFIED** | 71/71 batches, **23,849 tests, 66 failures, 0 errors** (2 since fixed → **64 open**), 1 UNRUNNABLE group (10 files, left unrunnable). Never near the 3 GB floor (min free **11.31 GB**), 106.9 min. The **"9 pre-existing failures" figure is NO LONGER TRUE** — it is 66 across 21 files, 39 of them in four single-cause clusters, 3 environment artefacts (net 63). Rail mutation-proved four ways. **It also surfaced a live production defect, now fixed — `9111438d0`.** `docs/test-baseline/python-failures.md` + `.json`. |

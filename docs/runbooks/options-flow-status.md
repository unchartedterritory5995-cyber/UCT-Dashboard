...
| Job | Status | Evidence |
|---|---|---|
| Python failure baseline | **DONE-VERIFIED** | 71/71 batches, **23,849 tests, 66 failures, 0 errors** (2 since fixed → **64 open**), 1 UNRUNNABLE group (10 files, left unrunnable). Never near the 3 GB floor (min free **11.31 GB**), 106.9 min. The **"9 pre-existing failures" figure is NO LONGER TRUE** — it is 66 across 21 files, 39 of them in four single-cause clusters, 3 environment artefacts (net 63). Rail mutation-proved four ways. **It also surfaced a live production defect, now fixed — `614036147`.** `docs/test-baseline/python-failures.md` + `.json`. |
| **Python baseline — the 4 clusters + batch 37.2** | **DEFERRED — owner ruling, dedicated session. DO NOT START.** | The 64 open failures are not 64 defects. Four single-cause clusters hold 39: `test_implied_backfill` (15, fake-provider signature drift), `test_screener_wave2_analyst_store` (9, `fetch_one` returns `None`), `test_web_capture_coverage` (8, `KeyError: 'text_origin'`), `test_ticker_meta` (5 remaining — 3 stale patch targets + 2 mapping mismatches). Plus the **batch 37.2 UNRUNNABLE group**: 10 files whose results are LOST, not red — `--timeout=120` fired inside a real `sleep()` in `discord_index_close.wait_until_warm` and killed the process before the JUnit XML was written. Re-running it needs a per-test timeout raise or a fake clock, not another sweep. Detail + the full open table: `docs/test-baseline/python-failures.md`. |
...
| 4 | ✅ **FIXED — `614036147`.** Was a LIVE `NameError` on master: `api/services/ticker_meta.py:145` called `_log.warning(...)` where every other logging call in the module uses `_logger`. Shipped 12:01 2026-09-12 in `553f6b68b` (D1 G1 tranche 1), **inside the `except` handler that commit added to keep** the docstring promise *"Never raises"*. Contained — `_base_meta` caught it and fell back to Finnhub, nothing 500'd — but **every FMP profile failure was logged as `name '_log' is not defined`, destroying the real provider error**. One-line fix, mutation-proved (2 green, 0 newly red), ride-along marker bump #6 (flow-worker **BUILT on `c4c77d385`, not SKIPPED**). ✅ **VERIFIED ON PRODUCTION 23:17 UTC:** `GET /api/ticker-meta/ZZZZQTEST` → **200**, and the web log carries `WARNING api.services.ticker_meta: FMP profile failed for ZZZZQTEST: FMP /stable/profile: no data for this request` — the REAL provider error from the line-145 handler, not the NameError. That line could not have existed before the fix: the NameError was raised while evaluating that very log call, so `_base_meta` logged `name '_log' is not defined` instead. Control: `BRK-B` → 200 with no warning, so the handler only fires on a genuine FMP failure. ⚰️ **It moved 2 ids, not the 5 first attributed to it**: the NameError was MASKING three stale-patch-target failures from the same introducing commit, which stay open as test-fake drift. A loud error hides quiet ones. | Fixed on the owner's decision after the sweep surfaced it. The 3 it unmasked belong to the cluster session. |

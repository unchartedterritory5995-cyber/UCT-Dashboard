...
| 14 | `2026-09-15-session14-autonomous.md` | 2026-09-15 | ✅ tracked, here | **$0.00.** The 42 pairs GRADED — 42 Y / 0 over-merges, LENS_STRICT_06 confirmed, after fixing a sheet whose `jaccard` column was a boolean wearing a similarity's name. Three findings that move the programme: Railway's Wait-for-CI is OFF so a red gate still deploys; `reconcile`'s run root is CWD-relative and ephemeral on the pod, which blocks the N-pass build; and `run_weekly_audit` does not exist. **The merge stopped at the repo's own burst clause, which wants an owner attestation — not a tool** |
| 15 | `2026-09-15-session15-window-merge.md` | 2026-09-15 | ✅ tracked, here | **$0.00.** R56 moves the persisted-runs root onto the VOLUME (it resolved to an ephemeral image layer, which is why N-pass could never have completed); R52 stops `force` bypassing the one switch that spends; R53 lands the $25/night budget knob; R57 fixes a chain step that named a function which does not exist — **and the rail then found three more, so four of seven weekly steps have never run**. The merge window OPENED on the first probe; **GitHub refuses to create the PR**, on an unmodified form, with no rulesets on master |
| 16 | `2026-09-15-session16-landing.md` | 2026-09-15 | ✅ tracked, here | **$0.00.** R57 deletes three weekly steps that had NEVER RUN (their targets do not exist; `not_available` is a skip, so nothing ever paged), intent kept as W2-A/B/C. **R58's control settles three sessions of ambiguity: PR creation fails REPO-WIDE** — a one-commit, one-file branch gets the identical error, so it was never branch size. The landing is verified master-first and fully gated (1,305 passed, 0 destructive SQL, `app/` untouched) and **waits on the guard's burst clause**; no attestation, no bypass. N-pass DESIGNED, not built — it is not on the path to INGEST while EXTRACT is dark |
| 17 | `2026-09-16-session17-landed-and-lit.md` | 2026-09-16 | ✅ tracked, here | **$0.00. LANDED and LIT.** The branch was already on master (`c0c950fbb`, merged by another workstream's batch lander at 00:38) — and `land_master_first.py` reported that as *UNRECOGNISED PARENTS*, because `git merge` on an already-merged branch exits 0 without moving HEAD, so the tool read **master's own parents**. Fixed and mutation-proved. Landing verified whole (84/85 files byte-identical, 0 of our `CLAUDE.md` lines lost, merged-tree gate **1328 passed / 0 failed**). **`WISDOM_INGEST_ENABLED` lit on `web`** with all 24 other gates still False and zero catch-up slots in grace. The first dark-check probe had a **failing control** and was rebuilt to enumerate by prefix. R50 says there are **three** member-visible doors, not one — all shut |
| 18 | `2026-09-17-session18-extract.md` | 2026-09-17 | ✅ tracked, here | **$0.00. EXTRACT HELD; no Railway write made.** All three things the brief asked to confirm came back different. **ONE** INGEST night, not two (Tuesday's 18:47 slot preceded the flip; proved by `count=1` run row against `beats=3`, and a skipped slot writes a heartbeat but no run row). The **entity master was never empty** — production holds **32,651 entities** seeded 09-07; every "empty" reading was the conftest sandbox (R42 one level up), so session 9's demoted-CALL headline is a harness artifact. And EXTRACT is blocked by things no readiness list named: production's corpus is **empty** (`wisdom_sources 0`), and the **golden gate** would refuse it anyway (`wisdom_eval_runs 0` → `blocked_by_gate`). Adversarial verification refuted 2 of my 3 claims **on mechanism while their conclusions held**, and found a real defect: the **per-night budget is not per night** — it clamps the whole programme cumulatively, so a $75 night budget buys ONE night then silently stops |
| 19 | `2026-09-17-session19-force-never-spends.md` | 2026-09-17 | ✅ tracked, here | **$0.00. Four guards, and two were wrong in the same direction as their tests.** **R64**: a forced run can NEVER spend — R52 guarded the wrong half (it required a literal while the switch was OFF; the hazard is force WITH it on, where `spend_allowed` short-circuited on the flag before looking at `force`). Its old test asserted the hazard AS A REQUIREMENT. **R65**: the per-night budget rationed nothing — it clamped the programme, so $75/night bought ONE night (measured 0 of 10 allowed vs a control's 9 of 10). **R66**: `_normalize` read only the top level, and the walk found a SECOND live instance (`evals`) plus a third (a ticket number recorded as the reason nothing happened). **R67**: the gate verdict crosses as an aggregates-only manifest, whitelist-classified both ways. **R68**: SOURCES lit — and the flag turned out to have a SECOND consumer, arming Sunday's weekly step too. Landed on attempt 17 after ~25 guard refusals; no override |

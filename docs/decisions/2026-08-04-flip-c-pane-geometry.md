...
| `ast_user_formula_sma20` | Phase D **Task 16** (2026-08-07) | **Same reason, one lane further out — and it was PROMOTED from a placeholder rather than added.** It is an engine rehearsal on ONE build (`--same-build`): side A draws nothing, side B draws a USER's `ast` definition, `sma(close, 20)`, from `instancesB` against a catalogue `?userdefs=` installs on both sides. There is no `bands`-vs-`panes` pair to measure, so this record carries no cutover number for it. Its own number is **140,925 px, 5/5 runs, zero variance** on build `9b7a6dfc8d8a` (`price_plot:28719 mid_panes:61883 osc_strip:50323`, summing to the headline), and its fail-proof MOVES: `--perturb-b-instances '{"color": "#ff00ff", "lineWidth": 4}'` → **141,551, 5/5**, with all 626 px of the difference inside `osc_strip`. ⚠️ **Task 11 measured this same case at 0 with its fail-proof ALSO at 0** and honestly refused to report it as a pass — nothing installed a user definition into the registry the binder resolves through. `nativeRegistry.installUserDefinitions` is what changed. Both zeros are still reachable on demand (delete `userDefs`; or flip the stored `meta.repaint` to a stale `repaints`) and both probes are recorded in the case's own `why` and in `.superpowers/sdd/2026-08-06-phase-d-builder/task-16-report.md`. |

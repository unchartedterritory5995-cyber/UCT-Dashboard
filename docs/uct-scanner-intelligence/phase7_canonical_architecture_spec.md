...
- **bull_flag**: `trendline_pair`, 4 anchors = the flag channel's two lines. The pole itself (the move that defines the pattern's name) is never anchored — only its timestamps (`pivot_ts`) and scalar `extras.pole_pct` survive. A renderer wanting to draw the pole needs `pivot_ts[0]`/`[1]` plus a price it doesn't have today.
- **high_tight_flag**: **correction (Package 8D, verified by a direct re-read of `high_tight_flag.py::_build_detection`, not the Phase-7 recon this line originally inherited from — that recon's finding was specifically about `bull_flag`, a different file, and this line wrongly generalized it to HTF too).** HTF's `trendline_pair` anchors ARE `[pole_base, pole_top, flag_low, flag_high]` — a genuine pole segment (`anchors[0]→[1]`) plus a flag-range diagonal (`anchors[2]→[3]`), both real geometry, not a placeholder. Package 8D labeled these with `anchor_roles=["pole_base","pole_top","flag_low","flag_high"]` and `semantic_subtype="pole_and_flag"` — no new anchors needed, only the labels this section's own earlier draft anticipated.
...
|---|---|---|---|---|---|---|---|---|
| High Tight Flag | `trendline_pair`, **pole IS anchored** (corrected Package 8D — anchors[0]→[1] is a real pole_base→pole_top segment, mislabeled here originally) | `status` const "ready" | none in-file (shared 7d only) | none | none (base_catalog sibling has it) | ✓ (Package 8D: `anchor_roles` labels line the pole/flag correctly) | shape supported; toggle dead, extras unread | **high** (was; Package 8D lowered geometry-labeling to done for this family) |

...
**Date:** 2026-08-03 (rev 2 — review-panel amendments applied)
**Status:** Approved design; amended per 5-lens review (CEO · designer · engineer · 12-persona user panel); engineer findings pending final pass
...
### 4.4 Behavior
- **URL state:** opening pushes `?earnings=SYM` **once** (pushState, so browser Back closes the modal); **arrow-key stepping and rail-section changes use replaceState** — Back always closes, never walks through step history. Rail section serializes as `&esection=`; deep links restore both. Applies to all three mount points (Calendar, MyStocksHub, CatalystFlow).
- **Arrow keys** ← → step through the day's reporters (disabled while focus is in an input/textarea). Per-section scroll positions are retained in a state map while the modal is open; symbol switch resets them.
...
| Need | Source | Notes |
|---|---|---|
| Expected move (implied) | **NEW: in-house from Massive chains** via existing `api/services/polygon_options.py` (ATM straddle mid ± IV cross-check, expiry bracketing the report date) | **Pulled into the launch slice** — the Setup Grade must not ship on the delayed yfinance straddle. Cache 15min + serve-stale. Horizon stated in payload. |
| Implied-at-the-time per past quarter | **Nightly implied store — starts immediately** (UI-independent), snapshotting T-1 close implied for symbols reporting within 14 days (~40–80/night, bounded) | Best-effort IV-history backfill validated for the ~500 most-watched reporters pre-launch; cold-start UI state per §4.3.1a. Honest label: "tracking since 2026-08". |
| Unified earnings history (est/actual/surprise/next-day reaction/gap/drift per quarter) | **NEW endpoint** `GET /api/research/earnings-history/{sym}` composing FMP `stable/earnings` (already live for ModelBook) + internal bars store | Fixes the dead Overview card; feeds lollipop, reaction bars, modal + page. Reaction day = next session for AMC, same session for BMO (holiday-aware). 30d cache for closed quarters. |
...
- Modal keeps AbortController on symbol switch; arrow-key stepping reuses the mounted shell (no remount flash) and renders Brief cached-only (§4.3.3).
...
- **BE (pytest):** earnings-history composition (est/actual/reaction math, AMC/BMO reaction-day alignment, gap vs drift), implied-move computation vs fixture chain incl. expiry selection, FMP-probe gating logic, nightly-store bounding (14-day window), grade-snapshot persistence, cache/serve-stale behavior. Weekday-clock injected per house rule (no weekend-only time bombs).
...
5. **P5 — Polish:** mobile audit pass, tablet tier, empty-state copy, cross-surface QA, dead-code deletion (`ComingSoonTab.jsx`, legacy duplicated panels), full A&O/Filings modal sections if usage data asks.
...
Owner-selected on screens: Glass Premium register (S1) · two-pane rail modal with tab-like switching (S2) · banner header (S3) · implied-vs-realized + dollar strip (S4) · lollipop + side table (S5/5b) · Brief-contained AI + post-report banner flip (S7) · page left-rail architecture (S9). Delegated to Claude: reaction bars + dots + bracket (S6) · twin Analyst|Ownership panels (S8 — now a post-launch modal section per CEO review; ships on the page in P3) · rail composition, footer, behavior, typography, data plan, phasing (S10, approved wholesale). **Rev 2 amendments** from the 5-lens review: launch slice + scope freeze, trust posture (§12), grade scope-labels + partial-input rule, horizon-honest implied math + T-1 pinning, §4.5 state machine, replaceState stepping, signed/shape-coded encodings, contrast floors, gold-restraint rules, shell components, ReactionBars → Earnings History, learnability layer, cold-start states, cost bounds. Mockup screens: `.superpowers/brainstorm/345223-1785804114/content/` (hexes illustrative; tokens normative).

...
- **Per-tick admission control (in the provider instance):** a request counter with a hard ceiling `MSGRAPH_ONENOTE_MAX_REQUESTS_PER_TICK` (default 120) — if a tick would exceed it, the provider stops enumerating/fetching, publishes the cursor at the last fully-imported page, and returns what it has; the next tick resumes. Plus an `AsyncRateLimiter` at a low rps to smooth bursts.
...
- Tunables (all defaulted, provider-read live): `MSGRAPH_ONENOTE_PAGES_PER_TICK` (K, default 40), `MSGRAPH_ONENOTE_MAX_REQUESTS_PER_TICK` (default 120), `MSGRAPH_ONEDRIVE_PAGES_PER_TICK` (delta-page budget, default 200), `MSGRAPH_HTTP_TIMEOUT_SECONDS` (default 30).

...
**🔴 SUPERSEDED 2026-09-05 — use `OWNER_VENDOR_CAPTURE_PACKET_V3_1.md`. Do not
run this V3 script.** `medianCandLower` below (`math.max(math.min(raw,raw[1]),
math.min(raw[2],raw[3]))`) happens to give the right answer at the one planted
probe row, but is NOT a general lower-middle-of-four formula — a real
permutation/duplicate/random property test (which this version's own
self-check did not run) finds it wrong roughly 30% of the time under a
different pairing of the same four numbers. V3.1 replaces it with a formula
proven order-independent by that same property test, run and committed before
V3.1 was written. Everything else below — the rising/percentrank/bbw
candidates, the packet flow, Tranche 1B, thinkorswim notes — is correct and
carries over unchanged; kept here verbatim as the historical record.

...
| Track | Scope | Status |
|---|---|---|
| A — Vendor Parity / Semantic Truth | Populate vendor-observation store; initial priority list; real vendor-runtime captures; resolve known Pine ambiguities; vendor-parity as a distinct coverage dimension; CI fixtures; reword benchmark language everywhere | **All non-manual-capture prep DONE, 2026-09-04.** `VENDOR_CAPTURE_PLAN.md` now holds: the full 64-function prioritized list (4 tiers, cross-referenced against every `divergences.json` row — plus a structural finding that the `stateful` shape has ZERO applicable functions today, not just zero observations, since `supertrend`-class latching state is a deliberately excluded grammar limitation); `vendor_truth.py --check`/`--coverage`/`--selfcheck` and `test_vendor_truth.py` all verified working correctly against the still-empty store (selfcheck genuinely discriminates a planted defect); confirmed no CI wiring invokes `vendor_truth.py` anywhere in the repo (reported, not fixed — no general test CI exists here to attach it to). `OWNER_VENDOR_CAPTURE_PACKET.md` is the dead-simple, standalone execution packet for Tranche 1 (1 chart, 2 scripts, ~20-30 min). **Blocked only on the owner's TradingView login** (hard capability wall: adding even a built-in indicator, or opening the Pine Editor, requires an account — verified directly; account creation is prohibited for this agent). Resumes the moment the owner sends back the packet's captured values. |
...
| C — Product Telemetry | 5-event minimum, correlation IDs, using `landing_events`/`signature/ledger.py` | **DONE, 2026-09-04.** All five events wired end-to-end (`import_submitted`, `compile_finished`, `import_accepted`, `delivery_configured`, `execution_finished`), extending `landing_events` — no new analytics platform. Shared `import_id` threads client → server for full-journey reconstruction. De-dup guard verified non-vacuous. Never logs raw source/screenshot content. 38 new tests (28 backend + 10 frontend), all green; one real pre-existing test broken by this work (`ImportBox.thinkscript.test.jsx`'s naive POST-matching) found via full regression sweep and fixed. See `RISK_REGISTER.md` RISK-023 and `TRACK_C_TELEMETRY.md` for full detail. |
...
| F — Imported Parameter Contract ADR | Design doc for Pine input → adjustable UCT input mapping | **ADR complete, 2026-09-04, PROPOSED for owner review** — `2026-09-04-pine-input-parameter-fidelity-adr.md`. Recommendation: adjustable parameters as a translation-time override map (parameter source-name → value) that the existing fold-to-default mechanism consults instead of always folding to the script's declared default; the canonical tree stays exactly as closed and literal as it is today (no ninth node type), so `maxLookback`/domain checks/repaint linting all re-run unmodified on the new tree a parameter change produces, exactly as they would for a hand-edited source change. v1 scope is `input.int`/`input.float` used as plain numeric literals only — `input.bool`/`input.source`/switch-driving `input.string` are explicitly deferred (they select subtrees, not literals) and disclosed, not silently frozen without explanation. No broad implementation until this ADR is accepted. |

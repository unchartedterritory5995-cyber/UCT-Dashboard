...
| Track | Scope | Status |
|---|---|---|
| A — Vendor Parity / Semantic Truth | Populate vendor-observation store; initial priority list; real vendor-runtime captures; resolve known Pine ambiguities; vendor-parity as a distinct coverage dimension; CI fixtures; reword benchmark language everywhere | **Capture plan prepared 2026-09-04** (`VENDOR_CAPTURE_PLAN.md`) — bounded TradingView Tranche 1 (2 Pine scripts, 1 chart, 8 functions across the known-ambiguity/core-indicator/prior-incident categories) designed and ready; thinkorswim/TC2000 oracle formulas + capture protocol prepared, tracked VENDOR-UNVERIFIED. **Blocked on the owner's TradingView login** (hard capability wall: adding even a built-in indicator, or opening the Pine Editor, requires an account — verified directly, not assumed; account creation is prohibited for this agent). Per the owner's hybrid-capture ruling, execution resumes the moment the owner confirms they are logged in. |

...
| MP-007 | Reproduce/audit the 4 cited benchmark numbers (38/38, 43/58, 21/48, 28/48); denominators, classification, fixtures | testing-validation | MUST | Live re-run of the 5 corpus test files + the Python conformance tool (commands in PROGRESS.md) | `app/src/components/chart/engine/ast/{doorScorecard,pine.corpus,pine.blindCorpus,pine.screenerCorpus,thinkscript.corpus}.test.js`, `tools/ast_conformance.py` | — | **corpora located + numbers reconciled 2026-09-04 (all 5 cited figures confirmed stale/superseded, none reproducible as quoted); actual test execution still not run** |
...
| MP-014C | Door C — TC2000/PCF: same coverage audit | architecture | MUST | Capability matrix for TC2000 | **CORRECTED 2026-09-04:** `app/src/components/chart/engine/ast/pcf.js` (`parsePcf`) + `tests/fixtures/ast/pcf_corpus.json` (accepted/offset_dependent/refused) exist — not greenfield, coverage/depth unmeasured | MP-008 | not started — needs the same benchmark-reproduction treatment as Pine/thinkScript |

...
| MP-007 | Reproduce/audit the 4 cited benchmark numbers (38/38, 43/58, 21/48, 28/48); denominators, classification, fixtures | testing-validation | MUST | Written benchmark-reproduction report | Pine/thinkScript benchmark suite (location TBD) | Pine/TS archaeology | **in progress** |
...
| MP-014C | Door C — TC2000/PCF: same coverage audit | architecture | MUST | Capability matrix for TC2000 | unknown — no TC2000 work found yet, may be greenfield | MP-008 | not started |
...
| MP-066 | Plan documentation derived from the same capability metadata the engine uses, to avoid drift | documentation | SHOULD | NOTE: "Segment G6: the member's grammar reference, generated from the manifest" commit suggests this may partly already exist — verify before building fresh | TBD — dispatched agent investigating | MP-033 | **in progress** |

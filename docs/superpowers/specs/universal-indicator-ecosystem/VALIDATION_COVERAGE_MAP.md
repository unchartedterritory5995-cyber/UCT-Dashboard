...
| Vendor parity (translation layer vs. real TradingView/thinkorswim/TC2000 output) | **0 parity-comparable (unchanged) — 4 real vendor-semantics-only observations now held (2026-09-05), ~5/64 functions at Unit via spec-probe only** | `RISK_REGISTER.md` RISK-018a: `tests/fixtures/vendor/observations/` holds 4 real TradingView captures (`ta.rising`, `ta.median` even-length, `ta.percentrank`, `ta.bbw`), confirmed live (`vendor_truth.py --check`/`--coverage` → "0 parity-comparable, 4 vendor-semantics-only"). **Still 0 parity-comparable** — these four have no UCT engine implementation, so they cannot be VENDOR-PARITY VERIFIED; they are VENDOR SEMANTICS CAPTURED only. **Do not read dual-kernel conformance or golden-fixture pass rates as vendor-parity evidence** — they are a different, weaker claim (self-consistency), and this exact conflation risk is the reason this row exists separately from the row above |

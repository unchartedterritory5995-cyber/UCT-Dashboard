...
| Vendor parity (translation layer vs. real TradingView/thinkorswim/TC2000 output) | **0 — Exists (unverified), ~5/64 functions at Unit via spec-probe only** | `TEST_CREDIBILITY_FINDINGS.md` (RISK-018): `tests/fixtures/vendor/observations/` does not exist — zero real vendor observations held, confirmed live (`vendor_truth.py --check` exits 2). **Do not read dual-kernel conformance or golden-fixture pass rates as vendor-parity evidence** — they are a different, weaker claim (self-consistency), and this exact conflation risk is the reason this row exists separately from the row above |

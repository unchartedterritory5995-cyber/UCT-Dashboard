...
| **`VWAP_SESSION_ANCHOR`** — session VWAP buckets by UTC DAY (B3 A7, 2026-08-02) | ⏳ **OPEN — the owner's call; the default is UNCHANGED and UTC-day bucketing ships.** `computeVWAP` restarts its accumulator on a UTC calendar day, not an ET session. Regular hours never notice (09:30-16:00 ET is one UTC day), which is why nothing caught it; on EXTENDED hours — the only hours VWAP renders on — it wipes mid-session at 20:00 ET under EDT and **19:00 ET under EST**, and because Monday evening has already opened the next UTC day, **Tuesday's 04:00 ET open never resets at all**. **Correctness cost of KEEPING it: the chart opens that session at 93.9178 where a session anchor reads 108.3633 — $14.45 wrong — and stays >$0.50 wrong for 120 of its 193 bars; 207 of 579 bars differ by >$0.01.** **Pixel cost of CORRECTING it, measured: 2,590 changed pixels (0.348118%)**, 20/20 runs, zero variance, `--cases vwap_only`; builds `d64c84c2ebf7` (UTC-day, ships) → `8bbbb44e1110` (ET-session). The SAME 2,590 with `--instances-side both`, because both lanes read one function. Each build independently deterministic (`--same-build --repeat 5`: 0 px) | Unlike the head-mask this is the MATHS, so applying it is a **`compute.rev` bump** (not `version`) and force-migrates every binding pinned to `vwap@rev 1` — which is why it is a trading decision and not a migration's. Deliberately NOT bundled into the VWAP Flip A commit: both lanes read `computeVWAP`, so correcting it moves A and B together and `engine_vwap_vs_legacy` would still report 0 — the migration would look verified while the picture changed underneath it. Record: `docs/decisions/2026-08-02-vwap-utc-day-bucketing.md`; pinned by `engine/__tests__/vwapUtcBucketing.test.js` (which asserts the record still exists, still says OPEN, and still carries the number) and by `goldenFixtures.test.js`'s three session cases |

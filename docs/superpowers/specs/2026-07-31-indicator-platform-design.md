...
1. **Shared golden fixtures** (bars→expected columns JSON) run by vitest AND pytest (later the AST interpreter); one written semantic spec (alignment, warmup, NaN, seeding); rel-tol 1e-9; mandatory session fixtures: extended-hours day crossing UTC midnight + a DST transition (the JS VWAP UTC-day bucketing bug class). `indicator_compute.py` stays plain loops (numpy changes summation order).
   ⚠️ **ONE named exception to the rel-tol rule exists, and only at the RENDER boundary:** `MACD_HEAD_MASK` (§11). The raw natives agree at 1e-9 as required; the engine's `macd` COLUMN and the array `StockChart` draws from carry NaN on the first `signalPeriod - 1` bars of the MACD line where Python has values (8 bars at 12/26/9). It is measured, bounded and OPEN — `docs/decisions/2026-08-02-macd-head-mask.md`. Any OTHER lane divergence is a bug, not a precedent.
...
| **`MACD_HEAD_MASK`** — the MACD head-mask (B3 A5, 2026-08-02) | **OPEN — the OWNER's call; default unchanged (`true`).** The chart holds the MACD line's head back to the signal's first bar — 8 bars at 12/26/9 (bars 25-32). Measured cost of dropping it: **88 changed pixels (0.011828%)**, 20/20 runs, builds `9f566cd22874` (mask on) vs `9045bb69fc56` (mask off), `--cases macd_headmask`. **Named §9.1 exception:** JS↔Python agree at 1e-9 for the RAW natives; the engine's `macd` COLUMN and the array `StockChart` draws from carry 8 NaNs where Python has values. Bounded to one indicator, one column, `signalPeriod - 1` head bars | The maths is Python's and the fixtures'; the picture is what shipped, and moving it is visible at the start of every MACD chart. Parked behind ONE constant that both lanes read (`nativeRegistry.MACD_HEAD_MASK`) so applying the decision is one edit in its own commit, never a line inside a migration. Record + measurement: `docs/decisions/2026-08-02-macd-head-mask.md`; pinned by `nativeRegistry.test.js` and `engine/__tests__/macdHeadMaskRendered.test.jsx`, both of which go red the moment it moves |

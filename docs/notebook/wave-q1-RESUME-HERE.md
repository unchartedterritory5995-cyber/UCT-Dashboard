...
| **Branch tip** | `51c22bd32` **plus this docs-only commit stamping the table**. `master` is at `7ed6b2ce5`; all three deploys (`cd674ef56`, `eedb58ac8`, `7ed6b2ce5`) are ancestors of it, and the branch is ahead of master only by these docs commits. ⛔ A doc cannot name its own SHA; that is why this row says what each commit IS rather than pretending to a single "the commit". Read the tip with `git log --oneline -1`, always. |

...
| **Collaboration** | 2 | One real, substantial capability exists dark: a token-based, sanitized, flag-gated-OFF public share-link mechanism (`J2_SHARE_LINKS_ENABLED=0`) — more built-out than either research phase credited. Zero comments/mentions/multiplayer/team-workspace concept; no account/team-boundary primitive exists anywhere in UCT's auth system, which is the real prerequisite before any deeper collaboration work, not a UI increment. |

...
| 22 | Note-sync connectors (Obsidian, Notion, …) | **NOT APPLICABLE** | ⚠️ **This row said `NOTE_SYNC_ENABLED` is unset. It is not.** Measured on the production web pod at release: `NOTE_SYNC_ENABLED=1` and `NOTE_SYNC_OBSIDIAN_ENABLED=1` — the connectors workstream armed both, and this audit asserted a flag state from a code default instead of reading it (`project_feature_flag_ledger`: OFF-and-unset is indistinguishable from off-on-purpose, and *neither* is the same as ON). **The conclusion survives the correction, and for a stronger reason than the one it was resting on:** a review lives in `j2_thesis_reviews` and is never written into a note body (§2), so there is nothing in a synced note for a connector to carry out. Sync is *structurally* unable to reach a review — not merely switched off. |

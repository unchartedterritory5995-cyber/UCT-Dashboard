...
⚰️ **CORRECTION (2026-09-05, Wave 1 Slice 2 implementation-time verification) — the "Remaining build" bullets above understate the actual gap; preserved verbatim above, not rewritten.** "Persist SUGGESTED mentions... written whenever a note is saved" reads as a small addition to an existing detection pass. There is no existing detection pass to add persistence to: `enrichment.scan_notes_for_tickers()` (the only cashtag-in-prose scanner) has exactly one caller — the one-time post-migration import wizard — and `create_note`/`update_note` never invoke it. The SUGGESTED tier described two paragraphs up ("the mention scanner's output, offered, never auto-committed") produces no persisted artifact today under ordinary note-writing; it only exists as a one-shot import-time offer. The two OTHER tiers (CONFIRMED = `j2_notes.ticker`, STORED = `j2_note_embeds.symbol`) are real and correctly described. The class-share bug is also narrower than stated: only the hyphen form (`$BRK-B`) breaks; the dot form (`$BRK.B`) already works. Full evidence trail: `primary-platform-decision-log.md`, "P0-3 scope correction" (2026-09-05).

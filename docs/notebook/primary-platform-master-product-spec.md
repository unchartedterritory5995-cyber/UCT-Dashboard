...
> L, M, N, O, O6 and P closed in production afterwards, so **most of the P0/P1
> build list below has shipped** and its "Problem:" clauses describe a product
> that no longer exists. Items the code has overtaken carry an inline ⚰️
...
> ⛔ **A "Problem:" clause here is a 2026-09-05 measurement, not a standing fact.**
> Several now read as open gaps for capabilities that are live in production;
> that is the single most expensive way to mis-plan from this file.
> ⛔ **The absence of a marker means nobody has re-checked that line, not that it
> is true.**
> ⚠️ **This file already carries a ⚰️ SUPERSEDED note of its own (§P0-3) that is
> ITSELF now out of date** — see the marker stacked beneath it. A correction ages
> exactly like the claim it corrected.
...
11. ⚰️ **SUPERSEDED (2026-09-11): all three "will match" bars AND version history are shipped and production-verified** — search (Wave A: date filters, query-aware snippets, opt-in BM25, entity-anchored retrieval), trash + 30-day retention purge (Wave 0), export (round-trip verified, plus single-note export in Wave C), version history (Wave C — manifest §7 #2). The sequencing call below was made, honoured, and then completed; read it as a record of a decision, not an open one. **What we will match:** reliable search, a trash can, real export. Version history is P1, not P0 — a deliberate sequencing call (search/trash are the two failure modes members hit *first*; version history matters more once real usage accrues), not an oversight; stated explicitly here so it is a decision, not a silent gap.
...
⚰️⚰️ **AND THIS CORRECTION IS ITSELF SUPERSEDED (2026-09-11).** The three bullets
immediately below were true on 2026-09-05 and are false today. Manifest §7 #3,
`[m]`: **`_sync_note_mentions` runs on note create AND update**, persisting the
prose-mention sidecar to `j2_note_mentions` (manifest §1 "Knowledge"). The
sentence *"There is no ongoing detection pass at all"* no longer describes the
code. ⛔ **Both layers stay on the page.** The research-time claim ("~75% already
shipped") was wrong one way, this correction is now wrong the other way, and only
the code was ever right — the manifest logs this as a case where the measurement
overrules **both** documents. A reader who sees only the newest text learns a
status; a reader who sees the stack learns that this particular claim has been
stated confidently and wrongly twice.
...
⚰️ **SUPERSEDED (2026-09-11) — SHIPPED in Wave C, merged `928380241`, production-verified.** Manifest §7 #2, `[m]`: `j2_note_versions` with coalescing capture, a diff view, `GET /api/j2/notes/{id}/versions`, and a restore that force-captures a pre-restore checkpoint so restoring is never destructive — exactly this item's "smallest valuable version". ⭐ **Its stated failure condition was avoided**: the version table IS covered by `account_purge`. ⚠️ **Its success condition is met and a different one opened**: nothing prunes `j2_note_versions`, and the retention question this spec left to the architecture doc is contradicted between two documents and owned by nobody (manifest **S-16**). ⛔ **The "Why P1 not P0" reasoning below asked for an explicit owner decision rather than a silent gap — that was the right instinct and it is the sentence worth keeping.** Original text preserved:

...
**Not yet done (this slice, explicitly, not silently):** dedicated interaction tests exist for `CaptureMenu` + the two trade-context doors, but the remaining 7 widget-level "choose destination" buttons are verified only by confirming their existing tests still pass, not by a new test proving each widget's specific button/picker interaction. Real browser E2E (create a capture, use the picker, add a comment, verify it lands correctly for each of the 4 destinations) has not been run against a live server. Commit not yet made as of this writing.

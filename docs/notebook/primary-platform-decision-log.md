...
**Not yet started (this slice's remaining scope + the rest of Wave 0):** frontend for the trash view (`'__trash__'` sentinel, `FolderSidebar.jsx` "Trash" entry, restore action, `deleted=true` wired into `useJ2Notes`/`buildNotesUrl`); frontend for the P0-2 folder-count fix (new `useJ2NoteFolderCounts`/`useJ2NotesByFolders` hooks, replacing `FolderSidebar.jsx`'s capped-page-derived `notesByFolder`); local draft safety net (P1-10, `NoteEditorPage.jsx`, not yet read); the search read-latency benchmark at 100/1k/10k/50k+ notes; real member-facing E2E (incl. adversarial trust-recovery: double-delete, restore-twice, folder-deletion interaction, multi-user isolation — through the actual UI, not just the service layer); discoverability verification (two-rail: route/AST reachability + unmocked mount); deploy; production verification; the final Wave 0 certification report.

...
with sync_playwright() as pw:
    ...
    try:
        ...
        dismiss_intro(page)
        new_btn = page.get_by_role("button", name="+ New note", exact=True)
        new_btn.wait_for(state="visible", timeout=45000)
        s_empty = shot(page, "T1-0-empty-notebook")
        t_start = time.time()
        new_btn.click()
        page.get_by_placeholder("Title").wait_for(state="visible", timeout=45000)
        ...
        page.get_by_placeholder("Title").fill("Plan for tomorrow's session")
        ...
        title_after = page.get_by_placeholder("Title").input_value()

...
def main() -> int:
    ...
    step("sandbox identity answers", bool(ident.get("nonce")), identity=ident)
    with sync_playwright() as pw:
        ...
        def leaks(text):
            return [s for s in (SECRET_TITLE, ASK_Q, ASK_A, main_note["id"], sibling["id"], "secret-tag-8b",
                                "ZZ8B", "trade-secret-8b", EMAIL, "Share Eightb") if s in text]

        o = open_public(share_url, "shared-note", "10-stranger-share-link")
        step("stranger reads the share link", "The public thesis" in o["text"] and not leaks(o["text"])
             and NEUTRAL in o["text"] and "linked note" in o["text"],
        ...
        pubs = req.get(f"{BASE}/api/j2/publish").json()["publications"]
        slug = next(p["slug"] for p in pubs if p["kind"] == "note")

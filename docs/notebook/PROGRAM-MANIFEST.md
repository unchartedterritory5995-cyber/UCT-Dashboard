...
| Q1-16 | Local purge on revocation, driven by the auth answer | `[m]` 🔴 **NOT partial — the purge does not exist in any form, and the codebase forbids it ON PURPOSE.** The *driver* half is real and genuinely server-driven: `outboxDrain.js:36` `isTransient = (e) => !e?.status \|\| e.status >= 500`, so a 401/403 off the live response classifies as permanent → `settleBlocked` (`:101`) sets `permanent: true` and **KEEPS the entry**, "retired from retrying … and still holding every word". Nothing anywhere in `app/src` calls `indexedDB.deleteDatabase`; the only hit is the prohibition at `notebookDb.js:27` — *"`deleteDatabase()` IS NOT A RECOVERY STRATEGY … clearing it is data loss wearing the word 'repair.'"* Logout (`AuthContext.jsx:156-162`) touches no IDB. ⛔⛔ **This row should be re-adjudicated as DO-NOT-BUILD (§8), not as unfinished work**: it asks for exactly the deletion the owner's standing Wave-Q1 invariant forbids — *a queued entry is never removed unless the server body is proven to contain its content*. Building it as written would destroy unsynced member words on a 401. **An owner ruling, not an implementation, is what this row needs** | wave-q0 §11 |

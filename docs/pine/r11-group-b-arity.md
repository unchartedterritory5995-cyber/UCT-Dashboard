...
|---|---|
| `ta.highest` / `ta.lowest` | 1-arg form: does `ta.highest(20)` default the source to `high` (and `ta.lowest` to `low`), or to `close` for both? The asymmetry is the whole risk — guessing `close` would be silently wrong on 81 sites. |

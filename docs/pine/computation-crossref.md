...
| `ta.valuewhen` | 6.9% | 100 | 711 | **RUNTIME LANE: served (`01ef1fd76`, 2026-09-22)** · host lane: still `refused` `pine:function` | `pine:function` | — | 100 | The DELIBERATE RULING stands and is now RESOLVED BY A TWIN rather than by a refusal. Pine's 3rd arg counts OCCURRENCES, the host table's counts BARS; they line up positionally and answer different numbers, so the runtime lane got Pine's semantics (`CARRIED2`/`OP.CARRIED2`) and the host lane keeps the screener's bar-window function and its frozen columns. Semantics measured, not chosen: `r11-valuewhen-spy-2026-09-11.json` — occurrence 0 INCLUSIVE (122/122 discriminating bars), occurrences count FIRINGS, never-fired is `na`. A host-lane Pine spelling is still refused, and still correctly. |

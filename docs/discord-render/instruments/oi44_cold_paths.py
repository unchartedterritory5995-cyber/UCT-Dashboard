...
def measure(modules: list[str], *, timeout: float = 120.0) -> list[dict]:
    ...
    ⛔⛔ And every child pins the shared data root FIRST (see `_PRELUDE`): importing `api.**`
    standalone on this box otherwise writes to the owner's live `C:\\data`."""
    out = []
    prelude = _PRELUDE % {"root": str(ROOT)}
    for mod in modules:
        code = (prelude +
...
def self_check() -> int:
    ...
    m = measure(["xml.dom.minidom", "definitely_not_a_real_module_zzz"], timeout=30)
    m_ok = (m[0]["cold_ms"] is not None and m[0]["cold_ms"] > 0.0
            and m[1]["cold_ms"] is None)
    print(f"  {'ok  ' if m_ok else 'FAIL'} the measurer times a real module and reports an "
          f"unimportable one as None: {m[0]['cold_ms']} / {m[1]['cold_ms']}")
    bad += 0 if m_ok else 1

    print(f"TOTALS oi44_cold_paths --self-check {'PASS' if not bad else 'FAIL'} "
          f"declared={len(cases) + 1} failed={bad}")

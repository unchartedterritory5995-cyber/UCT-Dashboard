...
|---|---|---|---|---|
| **C-01** | A `web` restart kills in-flight replies and blanks the page the renderer screenshots | 1,077 deploys, median 8.4 min; 238 × 422; SPCX 09-08 flow-worker restart | 2.1 durable jobs + resume · 2.3 swap-signature retry | ✅ **CLOSED** — `test_C01_resume_*` (2.1, mutation-proved) |
| **C-02** | `web` saturation → Discord acks miss 3 s and page loads miss 21 s | 23 × 10015 (22 RTH), 37× co-occurrence with 142 × 502 | 2.1 off-loop ack + dedicated workers · 2.3 background renderer lane | ✅ **CLOSED, owner ruling 2026-09-19 — the V2-attributable half is CLOSED ON EVIDENCE; the shared-event-loop half is explicitly out of this programme's scope** (D-04, `instruments/c09_c02_probe.py`), and a class whose in-scope half is done and whose remainder is a separate, already-tracked upstream programme counts as closed for the "every forensics class closed with a commit" gate criterion. `test_v2_chart_is_offered_and_deferred_without_a_background_task` (2.1b, mutation-proved) closed the structural ack half. Re-verified live 2026-09-19 (not re-trusted from the original writeup): `c09_c02_probe.py` run 5 consecutive times, **150/150 offers queued every run, `over_3s: 0` every run**, sub-millisecond acks throughout (p50 0.0022 ms, max 0.019–0.025 ms across the 5 runs), self-check 9/9 PASS every time — alongside the `--real` S1 results (0 over 3 s at 30-concurrent, at 30/s and at the design burst). ⭐ The structural reason is asserted, not trusted: `_enqueue` returns the defer *before any render begins* (`commands.py:200-205`, confirmed live) and `record_ack` posts to a writer **queue** (`runtime.py:255-256`, confirmed live), so the ack path does no store I/O. ⛔ **WHAT WOULD FALSIFY IT:** any I/O added to the ack path — a synchronous store write, a network call, a lock a worker holds. ⛔ **WHAT THIS DOES NOT COVER, STATED RATHER THAN IMPLIED:** saturation of the **shared uvicorn event loop** by unrelated web traffic (the 524 class). That is upstream of V2 entirely, cannot be closed from inside this path, and already has its own detector (`observe._loop_alerts`, confirmed present live). The renderer choice is irrelevant to the claim — the ack is decided before any renderer is selected. |
| **C-03** | Discord rejects the whole component tree (invalid emoji, duplicate id, >100 chars) | 33 × COMPONENT_INVALID_EMOJI | 2.6 pre-flight validation | ✅ **CLOSED** — components test forbids U+25B2 (2.1a, mutation-proved) · `delivery.validate_components` (2.6) |
...
| **C-05** | A FastAPI route function called in-process leaks a `Query()` default | 178 autocomplete failures, 6 days | 2.4 service functions only + import rail | ✅ **CLOSED** — autocomplete fake bound to the real signature (2.1a, mutation-proved) |
...
| **C-08** | `/flow` failures misreported, and no time budget | 19 failures: 18 timeouts + 1 refused, all "reconnecting"; 9/11 flow-worker OI fallback >60 s | 2.1a per-class contract · 2.4 10 s budget + labelled cached card | ✅ **CLOSED** — `test_C08_*` (2.1a, mutation-proved) |
| **C-09** | The warm cycle competes with members for the renderer | 4,785 over-budget cycles; ~3,300 renders/day | 2.1 background lane · 2.3 renderer background slots | ✅ **CLOSED 2026-09-17** (W4/D-16, re-measured on the CURRENT tip `77dad414d`). ⚰️ It was 🔴 OPEN on a 50/50 result measured **pre-rebase**, and `D09-PREFLIGHT-DELTA-2026-09-15.md:148-149` listed the re-run as outstanding step 6; the instrument was not in the tree and no artifact existed, so on 2026-09-17 the closure was REFUSED under R-CITE and the row left open. It is closed now because the measurement exists: `instruments/c09_real_path_races.py` (recovered from `b12920907`), artifact `evidence/c09-races/2026-09-17-races-on-77dad414d.txt`. **RenderGate wired: the member wins 50 of 50 real-path races** (`{'member': 50, 'background': 0, 'nobody': 0}`); **ClasslessValve control: 0 of 50** (`{'member': 0, 'background': 50}`) — the control fires hard, so the result is not vacuous. Contenders are the PRODUCT: `produce_chart` vs `warm_hot_charts`, winner observed at first entry into `bars_fn`, png cache cleared every round. Mutation-proved 2/2 RED: M1 the warm cycle relabelled MEMBER → RED; M2 the comparator inverted → RED (`TOTALS mutation_c09_real_path PASS declared=2 failed=0`). RenderGate is live in-process at `api/services/discord_interactions.py:77`. |
| **C-10** | Cold bars storms hold the bars gate for 30 s | 42 `warm gate timed out` (per-deployment pass) | 2.4 per-hop timeouts + jitter | ✅ **CLOSED** — `test_c10_the_bars_hop_is_bounded_and_its_retry_is_jittered_not_a_fixed_wait`, plus the per-attempt budget made structural in `_call.py` |
| **C-11** | A delivery failure or crash ends with nothing said | 23 × 10015 + 23 × ATT finals produced no member message; crash path edits nothing | 2.1 runtime terminal-state guarantee | ✅ **CLOSED** — `test_C11_*` (2.1, mutation-proved) |
| **C-12** | Failures cannot be tied to a command, symbol, member or request | Finding #1 | 2.2 correlation ids, structured events, jobs table | ✅ **CLOSED** — 2.2 built (`2509cc0de`, merged `6d779dd47`): `test_discord_render_observe.py` · `test_discord_render_health_command.py` · `test_discord_render_health_endpoint.py` — 18 mutations red |
| **C-13** | A credential in logs | 142 renderer call logs with the render token | 2.3 renderer log hygiene · OI-13 rotation | ✅ **CLOSED 2026-09-18.** Log-hygiene half MEASURED (D-03, `instruments/c13_token_sweep.py`): eleven text shapes the renderer formats a URL or a credential into are swept for the token AND the secret, each in plaintext, two url-encoded forms and base64; a **control with redaction disabled finds 33 leaks**, so the clean result is an instrument that could have seen a presence. It found **two real gaps, both fixed**: `\b(secret)=` could not match `CHART_RENDERER_SECRET=` (the preceding `_` is a word character), and `scrub` knew only `name=value` — so **`X-Render-Token: <token>`, the shape the token actually travels in, survived untouched**. ⚰️ And the sweep's own first version merged `{**forms(token), **forms(secret)}`, whose keys collide, so it **never looked for the token at all** and reported a clean result about the secret alone. **OI-13 rotation completed 2026-09-18, owner-directed** (`OI-13-ROTATION-RUNBOOK.md`, all 7 steps): both `CHART_RENDER_TOKEN_PREVIOUS`/`VITE_CHART_RENDER_TOKEN_PREVIOUS` cleared after dual-sender functional proof (a live `/chart` render in `#render-smoke`, Morning Wire's `.env` fingerprint-matched to CURRENT). Step 7's control re-run against 22.5 MB of REAL captured production log content (`evidence/logs/`, both `web` and `flow-worker`, spanning the rotation) found **zero occurrences** of either the old or current token value in any of 4 encodings, with its own non-vacuity control (a planted needle was found by the same scan). Full account: `D14-LOG.md`, entry `2026-09-18 08:52 ET`. |
| **C-14** | `/flow` queries the wrong partition: ETFs report no flow | SPY 0 / 182, QQQ 0 / 136, SMH 0 / 83 | 2.4 resolver picks the partition (OI-16) | ✅ **CLOSED on the V2 path** — 2.4a built (`0e331168a`): `test_the_flow_partition_follows_flow_ingestions_classifier_then_the_etf_list` · `test_the_v2_flow_handler_passes_the_resolved_partition` · `test_the_flow_job_reads_the_partition_it_is_given_and_defaults_to_stocks`. Production class table: SPY/QQQ/SMH/IWM/SPX/NDX → `etfs`, NVDA/AAPL → `stocks` |

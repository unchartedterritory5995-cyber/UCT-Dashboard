...
### Reading as of 2026-09-14 17:32 ET — **organic members exposed to V2: 0**

⛔ **GENERATED BY THE TOOL, NOT TYPED.** Pasted from `flip_preconditions.py --json`, counts computed from the rows. A hand-typed tally beside the table it describes is the defect this programme has re-committed four times — the previous version of this section said "Four rows NOT MET" over a table holding three.

| Precondition | State | Evidence |
|---|---|---|
| zero xfails in the forensics suite | ✅ MET | 23 test(s), 0 xfail decorator(s) in test_discord_render_forensics.py |
| every forensics class closed with a commit | 🔴 **NOT MET** | 11/14 closed; still open: **C-02**, **C-09**, **C-13** |
| the artifact cache is wired to the hot path | ✅ MET | bindings imports artifact_cache |
| soak clean for >= 24 h | ⚪ NOT MEASURABLE | 66 clean tick(s); 90 needed for 24 h |
| /chart is shadowed (structural) | ✅ MET | 3 test(s) driving /api/discord/interactions with a real Ed25519 signature over a chart payload |

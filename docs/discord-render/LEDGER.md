...
| OI-29 | **2.6 hardens the failure MESSAGE, not the path the forensics evidence came from** (Lane C, CCR-2). `JobContext.edit` → `runtime.edit_fn` → `di.edit_original`, so the chart IMAGE PATCH never passes through `delivery.py`. The 23 × `10015` and 23 × `ATTACHMENT_NOT_FOUND` in `01-failure-forensics.md` were image/follow-up PATCHes. | Lane C's AST walk over the callers, reported rather than assumed. | Recorded rather than left implicit. Two honest options: route `edit_fn` through a new multipart `delivery.edit_image` (where the §3.4 size guard belongs), or state in §3.4 that 2.6 covers text deliveries only. ⛔ Not decided by an agent — the owner's call, because it changes what "2.6 is done" means. |

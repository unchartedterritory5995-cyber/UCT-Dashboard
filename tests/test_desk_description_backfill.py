"""desk_description_backfill — retro-apply the new YouTube description format
(evergreen links+disclaimer, plus real per-session chapters where already
stored) to the back catalog.

Contract: eligibility is REGISTRY-derived (edu_categories.kind == 'show'),
every description is built from the VIDEO'S OWN stored chapters (real
transcript-derived, never invented); the ledger makes the run idempotent and
resumable; one bad video never stops the sweep; today's videos belong to the
live pipeline, not the backfill. Mirrors desk_cover_backfill.py's mechanics.
"""
import contextlib
import json
import os
...
from api.services import desk_description_backfill as bf
from api.services import education_service as edu

ET = ZoneInfo("America/New_York")
NOW = datetime(2026, 8, 28, 1, 0, tzinfo=ET)          # "today" = Aug 28
...
def _seed(edu_db, *, youtube_id="YT1", title="Evening Update — August 15, 2026",
          category="Evening Update", kind="show", created_at=OLD_TS, chapters=None):
    ...
    with contextlib.closing(edu._connect()) as c:
        c.execute("UPDATE edu_videos SET created_at=? WHERE id=?", (created_at, row["id"]))
        c.commit()
    if chapters is not None:
        edu_db.set_video_insights(row["id"], chapters=chapters)
    return row
...
def test_eligible_skips_blank_ids_todays_and_unparseable(edu_db, ledger):
    ...
    _seed(edu_db, youtube_id="TODAY1", created_at=int(NOW.timestamp()),
          title="Evening Update — August 28, 2026")
...
# ---------------------------------------------------------------------------
# backfill_one — the video's OWN stored chapters, links-only when there are none
# ---------------------------------------------------------------------------

def test_backfill_one_composes_links_only_when_no_chapters_stored(edu_db, ledger):
    v = dict(_seed(edu_db))
    v.update(edu_db.get_video(v["id"]))
    yt = _YT()
    assert bf.backfill_one(v, youtube=yt, ledger_path=ledger) == "done"
    assert yt.calls[0][0] == "YT1"
    desc = yt.calls[0][1]
...
def test_backfill_one_composes_with_real_chapters_when_stored(edu_db, ledger):
    v = dict(_seed(edu_db, chapters=[{"t": 0, "title": "Open"},
                                     {"t": 300, "title": "MU entry at the 21"}]))
    v.update(edu_db.get_video(v["id"]))
    yt = _YT()
    assert bf.backfill_one(v, youtube=yt, ledger_path=ledger) == "done"
    desc = yt.calls[0][1]
    assert "⏱️ Chapters:" in desc
    assert "0:00 Open" in desc
    assert "5:00 MU entry at the 21" in desc


def test_backfill_one_update_description_error_is_recorded_not_raised(edu_db, ledger):
    v = dict(_seed(edu_db)); v.update(edu_db.get_video(v["id"]))
    class _BoomYT:
        def update_description(self, *a):
...
def test_backfill_one_skips_an_unparseable_title(edu_db, ledger):
    v = dict(_seed(edu_db, title="no structural separator"))
    v.update(edu_db.get_video(v["id"]))
    yt = _YT()
    assert bf.backfill_one(v, youtube=yt, ledger_path=ledger) == "skipped"
    assert yt.calls == []
...
def test_run_backfill_processes_all_and_survives_one_bad_video(edu_db, ledger):
    ...
    class _FlakyYT(_YT):
        def update_description(self, video_id, description):
            ...
            super().update_description(video_id, description)
...
def test_run_backfill_respects_limit_and_flag(edu_db, ledger, monkeypatch):
    ...
    monkeypatch.setenv("DESK_SESSION_DESCRIPTION_CHAPTERS", "0")
    out = bf.run_backfill(youtube=_YT(), ledger_path=ledger, now=NOW, sleep_secs=0)
    assert out.get("skipped") == "DESK_SESSION_DESCRIPTION_CHAPTERS is off"
    monkeypatch.setenv("DESK_SESSION_DESCRIPTION_CHAPTERS", "1")
    _seed(edu_db, youtube_id="B1", title="Evening Update — August 14, 2026")
...
def test_limit_zero_is_a_dry_run_not_the_full_sweep(edu_db, ledger):
    _seed(edu_db, youtube_id="A1")
    yt = _YT()
    out = bf.run_backfill(youtube=yt, ledger_path=ledger, now=NOW, sleep_secs=0, limit=0)
    assert out["eligible"] == 1 and out["done"] == 0 and yt.calls == []
...
def test_flipping_the_flag_off_stops_a_sweep_in_flight(edu_db, ledger, monkeypatch):
    ...
    class _KillSwitchYT(_YT):
        def update_description(self, video_id, description):
            os.environ["DESK_SESSION_DESCRIPTION_CHAPTERS"] = "0"  # operator kills it mid-run
            super().update_description(video_id, description)

    out = bf.run_backfill(youtube=_KillSwitchYT(), ledger_path=ledger, now=NOW, sleep_secs=0)
...
def test_backfill_endpoints_require_the_bearer(client):
    assert client.post("/api/desk/description-backfill").status_code == 401
    assert client.get("/api/desk/description-backfill").status_code == 401


def test_backfill_post_fires_background_and_get_reports(client, ledger, monkeypatch):
    monkeypatch.setenv("DESK_DESCRIPTION_BACKFILL_LEDGER", ledger)
    started = {}
    monkeypatch.setattr(bf, "start_background", lambda **kw: started.update(kw) or True)
    r = client.post("/api/desk/description-backfill?limit=5",
                    headers={"Authorization": "Bearer ppp"})
    assert r.status_code == 200 and r.json()["started"] is True
    assert started["limit"] == 5
    bf._ledger_write(ledger, "YTX", "done")
    g = client.get("/api/desk/description-backfill",
                   headers={"Authorization": "Bearer ppp"})
...
def test_status_reports_the_gate_and_endpoints_are_sync_defs(edu_db, ledger, monkeypatch):
    ...
    monkeypatch.setenv("DESK_DESCRIPTION_BACKFILL_LEDGER", ledger)
    s = bf.status()
    assert s["chapters_enabled"] is False
    import asyncio
    from api.routers import desk_zoom_webhook as wh
    ...
    assert not asyncio.iscoroutinefunction(wh.description_backfill_start)
    assert not asyncio.iscoroutinefunction(wh.description_backfill_status)

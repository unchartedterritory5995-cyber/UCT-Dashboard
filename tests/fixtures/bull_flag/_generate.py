...
def _build_bars(base_price, base_bars, pole_pct, pole_bars,
    ...
    """Build a synthetic OHLCV series with a clean base-pole-flag structure.

    `flag_retrace_pct` is interpreted in detector terms: the fraction of the pole's
    height the flag's deepest low retraces. flag_low = pole_top - retrace * pole_height.
    The generator anchors the flag channel around this exact flag_low so the detector's
    measured retrace matches the input target.
    """
    ...
    # 1. Base consolidation (deliberately tight so its lows do NOT undercut the pole base)
    ...
    # detector retrace = (pole_top_price - flag_low) / (pole_top_price - pole_base_price)
    # pole_base_price ~= pole_start_price (the lowest point right before the pole)
    pole_height = pole_top_price - pole_start_price
    if pole_height > 0:
        target_flag_low = pole_top_price - flag_retrace_pct * pole_height
    else:
        # No pole or negative pole: just slide down a fixed amount
        target_flag_low = pole_top_price * (1.0 - flag_retrace_pct * 0.1)
    flag_high = pole_top_price
    ...
    channel_center_at_top = (flag_high + target_flag_low) / 2.0

    for i in range(flag_bars):
        # i-th bar center shifted by slope. We want flag_low at *some* bar to actually hit the target.
        slope_offset = flag_high * flag_slope_pct_per_bar * (i - mid_i)
        upper = flag_high + slope_offset

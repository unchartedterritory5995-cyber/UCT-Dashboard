"""Battery test for the holy_grail (Raschke) detector."""
import pytest

from api.services.pattern_engine.detectors.uct.holy_grail import detect_holy_grail
from api.services.pattern_engine.primitives.context import build_context
from tests.pattern_engine.detectors.fixture_loader import load_all_fixtures


FIXTURES = load_all_fixtures("holy_grail", include_internal=False)


@pytest.mark.parametrize("fixture", FIXTURES, ids=lambda f: f.name)
def test_holy_grail_fixture(fixture):
    ctx = fixture.context if fixture.context is not None else build_context(fixture.bars, sym="TEST")
    detections = detect_holy_grail(fixture.bars, ctx)
...
def test_geometry_extras_richness():
    ...
    for key in ("adx_approx", "di_plus", "di_minus", "pullback_bar_idx",
                "pullback_low", "20ema_at_pullback", "ema_slope_pct_per_bar",
                "vol_contraction_ratio", "atr14", "swing_high"):
        assert key in extras, f"missing geometry.extras.{key}"
    assert extras["adx_approx"] >= 30.0
    assert extras["di_plus"] > extras["di_minus"]
    assert extras["ema_slope_pct_per_bar"] > 0
    assert extras["vol_contraction_ratio"] < 1.0

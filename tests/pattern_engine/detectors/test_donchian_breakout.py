"""Battery test for the Donchian Breakout (Turtle) detector."""
import pytest

from api.services.pattern_engine.detectors.classical.donchian_breakout import detect_donchian_breakout
from api.services.pattern_engine.primitives.context import build_context
from tests.pattern_engine.detectors.fixture_loader import load_all_fixtures


FIXTURES = load_all_fixtures("donchian_breakout", include_internal=False)


@pytest.mark.parametrize("fixture", FIXTURES, ids=lambda f: f.name)
def test_donchian_breakout_fixture(fixture):
    ctx = fixture.context if fixture.context is not None else build_context(fixture.bars, sym="TEST")
    detections = detect_donchian_breakout(fixture.bars, ctx)
...
def test_geometry_extras_richness():
    ...
    for key in ("donchian_period", "donchian_high_or_low", "breakout_close",
                "atr_20", "volume_ratio", "direction"):
        assert key in extras, f"missing geometry.extras.{key}"
    assert extras["donchian_period"] in (20, 55)
    assert extras["volume_ratio"] >= 1.0

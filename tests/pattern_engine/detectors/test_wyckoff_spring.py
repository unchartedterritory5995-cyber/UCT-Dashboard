"""Battery test for the wyckoff_spring detector."""
import pytest

from api.services.pattern_engine.detectors.uct.wyckoff_spring import detect_wyckoff_spring
from api.services.pattern_engine.primitives.context import build_context
from tests.pattern_engine.detectors.fixture_loader import load_all_fixtures


FIXTURES = load_all_fixtures("wyckoff_spring", include_internal=False)


@pytest.mark.parametrize("fixture", FIXTURES, ids=lambda f: f.name)
def test_wyckoff_spring_fixture(fixture):
    ctx = fixture.context if fixture.context is not None else build_context(fixture.bars, sym="TEST")
    detections = detect_wyckoff_spring(fixture.bars, ctx)
...
def test_geometry_extras_richness():
    ...
    for key in ("range_bars", "range_high", "range_low", "range_height_pct",
                "spring_low", "spring_bars_below", "spring_depth_pct",
                "reclaim_bar_idx", "reclaim_volume_ratio",
                "accumulation_volume_signature"):
        assert key in extras, f"missing geometry.extras.{key}"
    assert extras["range_bars"] >= 15
    assert extras["spring_low"] < extras["range_low"]
    assert extras["reclaim_volume_ratio"] >= 1.2  # _RECLAIM_VOL_MIN

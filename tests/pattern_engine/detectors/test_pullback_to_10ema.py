"""Battery test for the pullback_to_10ema detector."""
import pytest

from api.services.pattern_engine.detectors.uct.pullback_to_10ema import detect_pullback_to_10ema
from api.services.pattern_engine.primitives.context import build_context
from tests.pattern_engine.detectors.fixture_loader import load_all_fixtures


FIXTURES = load_all_fixtures("pullback_to_10ema", include_internal=False)


@pytest.mark.parametrize("fixture", FIXTURES, ids=lambda f: f.name)
def test_pullback_to_10ema_fixture(fixture):
    ctx = fixture.context if fixture.context is not None else build_context(fixture.bars, sym="TEST")
    detections = detect_pullback_to_10ema(fixture.bars, ctx)
...
def test_geometry_extras_richness():
    ...
    for key in ("ema_10_value", "pullback_bar_low", "pullback_bar_close",
                "distance_to_ema_pct", "ema_slope_pct_per_bar",
                "pullback_volume_ratio", "atr14", "swing_high"):
        assert key in extras, f"missing geometry.extras.{key}"
    assert extras["ema_slope_pct_per_bar"] > 0
    assert extras["pullback_volume_ratio"] < 1.5

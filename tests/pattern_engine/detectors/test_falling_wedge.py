"""Battery test for the falling_wedge detector. Runs every fixture in
tests/fixtures/falling_wedge/ and asserts the expected outcome.

A falling wedge is a bullish reversal/continuation: both upper and lower
trendlines slope down, with the lower line declining less steeply, so the
lines converge downward toward an apex.
"""
import pytest

from api.services.pattern_engine.detectors.classical.falling_wedge import detect_falling_wedge
from api.services.pattern_engine.primitives.context import build_context
from tests.pattern_engine.detectors.fixture_loader import load_all_fixtures


FIXTURES = load_all_fixtures("falling_wedge", include_internal=False)


@pytest.mark.parametrize("fixture", FIXTURES, ids=lambda f: f.name)
def test_falling_wedge_fixture(fixture):
    ctx = fixture.context if fixture.context is not None else build_context(fixture.bars, sym="TEST")
    detections = detect_falling_wedge(fixture.bars, ctx)
    ...
    if fixture.expected_fires:
        ...
        assert d["pattern_id"] == "falling_wedge"

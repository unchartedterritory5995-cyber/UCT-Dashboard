"""Battery test for the VSA No Demand detector."""
import pytest

from api.services.pattern_engine.detectors.classical.vsa_no_demand import detect_vsa_no_demand
from api.services.pattern_engine.primitives.context import build_context
from tests.pattern_engine.detectors.fixture_loader import load_all_fixtures


FIXTURES = load_all_fixtures("vsa_no_demand", include_internal=False)


@pytest.mark.parametrize("fixture", FIXTURES, ids=lambda f: f.name)
def test_vsa_no_demand_fixture(fixture):
    ctx = fixture.context if fixture.context is not None else build_context(fixture.bars, sym="TEST")
    detections = detect_vsa_no_demand(fixture.bars, ctx)
...
def test_geometry_extras_richness():
    ...
    for key in ("bar_range", "avg_range", "bar_volume", "prior_volume",
                "avg_volume", "is_up_bar", "range_ratio",
                "volume_ratio_to_avg"):
        assert key in extras, f"missing geometry.extras.{key}"
    assert extras["is_up_bar"] is True

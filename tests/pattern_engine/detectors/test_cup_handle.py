"""Battery test for the cup_handle detector. Runs every fixture in
tests/fixtures/cup_handle/ and asserts the expected outcome.

Cup-and-handle is O'Neil's classic bullish continuation: a rounded U-shaped
consolidation (cup) followed by a tight pullback (handle). Breakout above
the right rim on volume confirms.
"""
import pytest

from api.services.pattern_engine.detectors.classical.cup_handle import detect_cup_handle
from api.services.pattern_engine.primitives.context import build_context
from tests.pattern_engine.detectors.fixture_loader import load_all_fixtures


FIXTURES = load_all_fixtures("cup_handle", include_internal=False)


@pytest.mark.parametrize("fixture", FIXTURES, ids=lambda f: f.name)
def test_cup_handle_fixture(fixture):
    ctx = fixture.context if fixture.context is not None else build_context(fixture.bars, sym="TEST")
    detections = detect_cup_handle(fixture.bars, ctx)
    ...
    if fixture.expected_fires:
        ...
        assert d["pattern_id"] == "cup_handle"

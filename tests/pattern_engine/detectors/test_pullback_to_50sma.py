"""Battery test for the pullback_to_50sma (O'Neil 2nd Buy Point) detector."""
import pytest
...
def test_pullback_to_50sma_fixture(fixture):
...
def test_geometry_extras_richness():
    ...
    for key in ("sma_50_value", "prior_advance_pct", "prior_advance_bars",
                "test_bar_low", "reclaim_bar_idx", "test_volume_ratio",
                "reclaim_volume_ratio"):
        assert key in extras, f"missing geometry.extras.{key}"
    assert extras["prior_advance_pct"] >= 30.0

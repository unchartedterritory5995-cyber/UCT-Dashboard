"""Battery test for the MACD Bearish Crossover detector."""
import pytest

from api.services.pattern_engine.detectors.classical.macd_bearish_cross import detect_macd_bearish_cross
from api.services.pattern_engine.primitives.context import build_context
from tests.pattern_engine.detectors.fixture_loader import load_all_fixtures


FIXTURES = load_all_fixtures("macd_bearish_cross", include_internal=False)


@pytest.mark.parametrize("fixture", FIXTURES, ids=lambda f: f.name)
def test_macd_bearish_cross_fixture(fixture):
    ctx = fixture.context if fixture.context is not None else build_context(fixture.bars, sym="TEST")
    detections = detect_macd_bearish_cross(fixture.bars, ctx)
...
def test_geometry_extras_richness():
    ...
    for key in ("macd_value", "signal_value", "histogram", "crossover_bar_idx",
                "was_above_zero", "prior_histogram", "dcr"):
        assert key in extras, f"missing geometry.extras.{key}"
    assert extras["histogram"] < 0  # bear cross: histogram negative

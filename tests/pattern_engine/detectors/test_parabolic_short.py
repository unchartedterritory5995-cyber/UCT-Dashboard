"""Battery test for the parabolic_short detector."""
import pytest

from api.services.pattern_engine.detectors.uct.parabolic_short import detect_parabolic_short
from api.services.pattern_engine.primitives.context import build_context
from tests.pattern_engine.detectors.fixture_loader import load_all_fixtures


FIXTURES = load_all_fixtures("parabolic_short", include_internal=False)


@pytest.mark.parametrize("fixture", FIXTURES, ids=lambda f: f.name)
def test_parabolic_short_fixture(fixture):
    ctx = fixture.context if fixture.context is not None else build_context(fixture.bars, sym="TEST")
    detections = detect_parabolic_short(fixture.bars, ctx)
...
def test_geometry_extras_richness():
    ...
    for key in ("30bar_advance_pct", "climax_range_ratio", "climax_dcr",
                "climax_vol_ratio", "gap_up_pct", "gap_failed",
                "climax_high", "climax_low", "atr20", "target_low"):
        assert key in extras, f"missing geometry.extras.{key}"
    assert extras["climax_range_ratio"] >= 3.0
    assert extras["climax_dcr"] <= 0.30
    assert extras["climax_vol_ratio"] >= 3.0
    assert extras["gap_failed"] is True

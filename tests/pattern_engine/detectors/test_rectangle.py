"""Battery test for the rectangle (trading range) detector."""
import pytest

from api.services.pattern_engine.detectors.classical.rectangle import (
    detect_rectangle,
)
...
FIXTURES = load_all_fixtures("rectangle", include_internal=False)


@pytest.mark.parametrize("fixture", FIXTURES, ids=lambda f: f.name)
def test_rectangle_fixture(fixture):
    ctx = fixture.context if fixture.context is not None else build_context(fixture.bars, sym="TEST")
    detections = detect_rectangle(fixture.bars, ctx)
...
def test_confidence_formula_pin():
    """Pins the canonical 4-weight confidence formula for rectangle.

    Formula: round(0.40*geometry_score + 0.25*volume_score
                   + 0.20*context_score + 0.15*historical_score, 2)

    Uses the 'clean_bullish_continuation' fixture (deterministic positive case).
    Any future formula drift causes this test to fail loudly.
    """
    fixtures = load_all_fixtures("rectangle", include_internal=False)
    fixture = next((f for f in fixtures if f.name == "clean_bullish_continuation"), None)
    assert fixture is not None, "missing clean_bullish_continuation fixture"
    ...
    detections = detect_rectangle(fixture.bars, ctx)
    assert detections, "clean_bullish_continuation must fire for the formula-pin test"
    d = detections[0]
    ...
    assert d["confidence"] == expected, (
        ...
        "Check formula weights in detect_rectangle."
    )

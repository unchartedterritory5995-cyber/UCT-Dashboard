"""Battery test for the VCP detector. Runs every fixture in
tests/fixtures/vcp/ and asserts the expected outcome."""
import pytest
...
FIXTURES = load_all_fixtures("vcp", include_internal=False)


@pytest.mark.parametrize("fixture", FIXTURES, ids=lambda f: f.name)
def test_vcp_fixture(fixture):
    ctx = fixture.context if fixture.context is not None else build_context(fixture.bars, sym="TEST")
    detections = detect_vcp(fixture.bars, ctx)
...
def test_narrative_richness():
    ...
    # Each narrative field should be substantial (>200 chars = paragraph length)
...
def test_geometry_extras_richness():
    ...
    for key in ("n_contractions", "contraction_pcts", "pivot_price",
                "last_contraction_low", "final_contraction_volume_ratio",
                "pattern_bars", "final_contraction_bars"):
        assert key in extras, f"missing geometry.extras.{key}"

    assert isinstance(extras["contraction_pcts"], list)
    assert len(extras["contraction_pcts"]) == extras["n_contractions"]
    assert extras["n_contractions"] >= 2

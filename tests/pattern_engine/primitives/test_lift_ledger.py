...
def test_a_NEUTRAL_structure_is_graded_BOTH_ways():
    """⛔⛔ GRADING A NEUTRAL STRUCTURE LONG IS A DIRECTIONAL CLAIM MADE ON ITS
    BEHALF.

    `darvas-box` and `square-box` both published on the long metric while
    declaring `bias="neutral"`. A box is a range -- Darvas's own words describe
    a frame, not a forecast -- so defaulting it to long is not neutral at all.
    It is an undeclared bet, and invisible precisely because the number looks
    like every other number in the table.

    Measured both ways the row says something one number cannot: positive on
    ONE side marks direction; positive on BOTH marks VOLATILITY, since price
    left the range either way.
    """
    import importlib.util
    import os
    ...
    root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.dirname(os.path.abspath(__file__)))))
    spec = importlib.util.spec_from_file_location(
        "rll2", os.path.join(root, "tools", "run_lift_ledger.py"))
    rll = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(rll)

    from api.services.screener import base_catalog as bc

    neutral = [s for s in bc.RELATIONS if s.bias == "neutral"]
    assert neutral, "fixture: there must be a neutral structure to check"
    for st in neutral:
        assert sorted(rll._directions_of(st)) == ["long", "short"], st.key

    # The control: a structure that DOES claim a direction is graded once.
    for st in bc.RELATIONS:
        if st.bias == "bullish":
            assert rll._directions_of(st) == ["long"], st.key
        elif st.bias == "bearish":
            assert rll._directions_of(st) == ["short"], st.key


def test_a_published_neutral_row_keeps_the_side_it_did_not_headline():
    """A neutral row headlines the stronger side; the other must remain
    visible, or the choice of which to show is unauditable.
    """
    from api.services.screener import base_catalog as bc
    ...
    for key, row in entries.items():
        st = bc.by_key(key)
        if st is None or st.bias != "neutral":
            continue
        sides = row.get("by_direction")
        if not sides:
            continue          # not yet re-measured under the both-ways rule
        assert set(sides) == {"long", "short"}, (
            "%s records only %s -- the unshown side is missing"
            % (key, sorted(sides)))
        head = row.get("direction")
        assert head in sides, "%s headlines %r, which it does not hold" % (
            key, head)

...
# Replay ships DARK (review Block A1): the capture side is risk-free and must
# collect from day one, but the replay side has known Block-B work (intraday
# detect_windows cap, edge honesty, CPU bounds) before it may touch flow.db
# autonomously. Flip FLOW_TAPE_REPLAY_ENABLED=1 only after Block B lands.
REPLAY_ENABLED = os.environ.get("FLOW_TAPE_REPLAY_ENABLED", "0").lower() in ("1", "true", "yes")
...
def _replay_window(target: date, gap_start_min: int, gap_end_min: int) -> dict:
    ...
    # ER copy-forward (review A2) — same contract as the other two builders.
    try:
        from api.massive_ws_worker import _load_er_flags
        er_map = _load_er_flags(list({e.root for e in events}))
    except Exception:
        er_map = {}
...
def replay_gaps(target: date = None) -> dict:
    ...
    try:
        ...
        # isn't a gap, it's the present). NOTE (Block B1, replay still dark):
        # detect_windows counts future session minutes as empty intraday, so a
        # same-day full_session collapse is possible — refuse it here; the
        # capped-detection rework lands before the replay flag flips.
        now_min = now_et.hour * 60 + now_et.minute
        if target == now_et.date():
            if mode == "full_session":
                return {"status": "deferred_full_session",
                        "note": "intraday full_session detection not trusted "
                                "until Block B1 (capped detect_windows) lands"}
        ...
        for s, e in windows:
            if not REPLAY_ENABLED:      # kill switch honored mid-run (A5)
                break
            results.append(_replay_window(target, s, e))

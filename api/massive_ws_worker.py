...
async def _run_session(ws):
    ...
    async def oi_fetch_manager():
        ...
        while not stop_event.is_set():
            ...
            # Stage 4 fallback (added 6/30 evening): try Massive's snapshot
            # endpoint for any contracts Schwab couldn't resolve. Schwab
            # has known coverage gaps on deep-OTM strikes, small/mid-cap
            # names, and freshly-listed weeklies. Massive's chain endpoint
            # often has them since it's sourced from the same OPRA feed
            # as the trade stream.
            #
            # Cost is modest: one chain call per unique underlying among
            # the unresolved contracts. For a batch where Schwab missed
            # 30 contracts spread across 5 tickers, that's 5 Massive calls
            # not 30.
            #
            # Kill switch: set MASSIVE_OI_FALLBACK_DISABLED=1 to skip
            # Massive entirely without redeploying.
            schwab_unresolved_keys = {
                ck for ck, oi in results if oi is None or oi == 0
            }
            massive_disabled = os.environ.get(
                "MASSIVE_OI_FALLBACK_DISABLED", "0"
            ).lower() in ("1", "true", "yes")
            if schwab_unresolved_keys and not massive_disabled:
                # Reconstruct the (sym, cp, strike, exp_mdy) tuples for the
                # unresolved contracts so we can pass them to Massive
                # fetcher (same input format as Schwab fetcher).
                ck_to_tuple = {}
                for entry in batch:
                    sym, cp_letter, strike, exp_mdy = entry
                    ck = f"{sym}|{cp_letter}|{float(strike)}|{exp_mdy}"
                    ck_to_tuple[ck] = entry
                massive_batch = [
                    ck_to_tuple[ck] for ck in schwab_unresolved_keys
                    if ck in ck_to_tuple
                ]
                try:
                    from api.massive_oi_snapshots import (
                        _fetch_oi_all_async as _fetch_oi_massive
                    )
                    massive_results = await _fetch_oi_massive(massive_batch)
                except Exception as e:
                    logger.warning(
                        "[massive-ws] Massive OI fallback failed: %s", e
                    )
                    massive_results = []

                # Merge Massive results into the main results list, but
                # only overwrite Schwab "none/zero" entries — never
                # downgrade a known Schwab result.
                massive_by_ck = {
                ...
                if massive_by_ck:
                    new_results = []
                    massive_filled = 0
                    for ck, oi in results:
                        if (oi is None or oi == 0) and ck in massive_by_ck:
                            new_results.append((ck, massive_by_ck[ck]))
                            massive_filled += 1
                        else:
                            new_results.append((ck, oi))
                    results = new_results
                    logger.info(
                        "[massive-ws] Massive OI fallback resolved %d of "
                        "%d Schwab misses (%d underlyings queried)",
                        massive_filled, len(schwab_unresolved_keys),
                        len(set(t[0] for t in massive_batch))
                    )
                    _state["oi_fetch_massive_resolved"] = (
                        _state.get("oi_fetch_massive_resolved", 0)
                        + massive_filled
                    )

            # Build source tag — items resolved by Massive get a different
            # tag so we can track per-source stats later.
            resolved = []
            for ck, oi in results:
                if oi is None or oi <= 0:
                    continue
                # We can't easily tell here which source resolved it
                # without re-checking — use a unified tag for now.
                # (oi_fetch_massive_resolved counter above tracks the split.)
                resolved.append((ck, oi, "ondemand"))

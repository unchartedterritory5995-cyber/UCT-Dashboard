...
#     Count how many of these signals an alert hits:
#       1. Premium meets tier-and-cap-band threshold
#       2. V/OI >= stack vOI threshold
#       3. Hit count (same-contract repeats) >= stack hit threshold
#       4. Grade >= stack grade threshold
#     Show only if signals >= min_signals (default 2 of 4).
#
...
DEFAULT_THRESHOLDS = {
    "stack": {
        "min_signals": 2,         # min stacked signals to qualify
        "vOI": 3.0,
        "hit_count": 3,
        "grade": "B",
    },
...
def _qualifies_curated(alert: dict, thresholds: dict) -> bool:
    ...
    # Algo: always excluded — non-directional multi-leg trades have no
    # actionable directional read.
    if tier == "algo":
        return False
    ...
    # Unusual: its own path. The V/OI anomaly + small premium IS the signal.
    if tier == "unusual":
        u = thresholds.get("unusual", {})
        return (prem >= u.get("min_premium", 100_000) and
                v_oi >= u.get("vOI", 5.0))

    # Other tracked tiers: apply stacking.
    ...
    signals = 0
    # Signal 1: premium meets tier-and-cap threshold
    ...
    return signals >= stack.get("min_signals", 2)

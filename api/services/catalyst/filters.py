...
def is_real_catalyst(c: dict) -> tuple[bool, Optional[str]]:
    ...
    action, or an earnings print that actually MOVED the stock). A dead-flat
    stock on normal volume with only vague prose about it is NOT a catalyst
    — that's the PAR / MEDP / FTRE failure mode, and it's also every sleepy
    regional-bank earnings print that used to flood the list in July.
    ...
    # independent hard signal (scanner/analyst). Report-day volume is
    # ALWAYS elevated, so we deliberately do NOT fall through to the
    # volume-surge rescue below — volume alone must not resurrect a flat

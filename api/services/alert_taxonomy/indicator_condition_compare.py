...
BLIND_SPOTS = (
    ...
    "intersection is EMPTY. So the cadence gate refuses every predicate the "
    "legacy lane can express, and for thirty of the thirty-one the reason is "
    "ADDRESS_UNRESOLVED rather than an undeclared cadence — the book declares "
    "no join for them, and none is supplied here: that would be a second "
    "authority over metric identity, the exact thing the plan's §2b ruling "
    "killed, so it is REPORTED, not built. `close` is the exception, and it "
    "changes the REASON, never the OUTCOME: D2 CP4 (signed after this harness) "
    "declared the ONE join `close` -> `ohlcv.c`, this gate now asks it, and "
    "`close` refuses REFUSAL_CADENCE_UNDECLARED — the SAME reason `ohlcv.c` "
    "itself refuses on, because the bars store still declares no cadence "
    "(F-D2-2). `close` and `ohlcv.c` are the same quantity under two names, "
    "the one name D2 CP4 ever joined, and both still cannot fire dark.",

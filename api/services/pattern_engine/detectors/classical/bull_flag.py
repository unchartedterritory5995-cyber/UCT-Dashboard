...
def _try_extract_pattern(bars, pivots, pole_top_idx: int, pole_top) -> Optional[dict]:
    ...
    # Find the absolute bar index of the flag low (for higher-low + MA-pullback checks).
...
def _score_geometry(c: dict) -> float:
    ...
    base = (0.30 * pole_score + 0.30 * retrace_score
            + 0.25 * parallel_pts + 0.15 * duration_score)
    base += structure_geom_boost(c)  # higher-low (+8) + rising-MA-pullback (+10/15/18/22)
...
def _build_detection(bars, c, confidence, context,
    ...
    what_it_is = (
        ...
        f"Tom Bulkowski's Encyclopedia of Chart Patterns measures the flag as "
        f"one of the weakest patterns in his data, not a high-reliability one: "
        f"a 44-45% break-even failure rate, an average move of only 8-9%, and "
        f"'Not ranked' among his overall performance tiers. His own site frames "
        f"this bluntly -- flags are the most commonly traded continuation "
        f"pattern and among the weakest in his measured numbers. What his data "
        f"DOES support here: volume declining into the flag body happens on "
        f"74-77% of his sample, which is exactly the {flag_vol_pct:.0f}%-of-pole "
        f"contraction this candidate shows. The flag is "
    ...
    why_it_matters = (
        ...
        f"intent. The {retrace_pct_pct:.0f}% retrace sits inside this detector's "
        f"own 15-50% band (Bulkowski publishes no retrace-depth figure for the "
        f"regular flag), and the {flag_count}-bar consolidation duration is "
        f"well inside Bulkowski's own 3-week ceiling for the pattern. Volume "
        f"contraction into the flag ({flag_vol_pct:.0f}% of pole average) is a "
        f"real, measured signature (see above) rather than a guaranteed "
        f"confirmation -- Bulkowski's own numbers put the break-even failure "
        f"rate at 44-45% even when a flag forms cleanly, so this setup should "
        f"be sized and stopped like the moderate-edge continuation play the "
        f"data shows, not a near-certain one. "

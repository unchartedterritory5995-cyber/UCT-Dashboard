...
def _build_detection(
    ...
    what_it_is = (
        ...
        f"was the lowest in that same window. The fractal pivot method used here detects "
        f"these reversal points without any lag, and assigns each pivot a strength score "
        f"(0-100) reflecting how dominantly it stood out from its neighbors and by what "
        f"margin (a wider, larger-margin pivot scores higher). Here {pivot_count} "
        f"significant pivots have formed over the last {window_bars} bars: "
        f"{pivot_high_count} swing-highs and {pivot_low_count} swing-lows. The pivot "
        f"density of {pivot_density_per_10_bars} pivots per 10 bars indicates "
        f"{density_desc} (low density = trending price action; high density = chop or "
        f"consolidation). The strongest pivot in the window - at "

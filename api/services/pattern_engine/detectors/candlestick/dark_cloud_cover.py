...
Context (critical — bearish reversal gate):
  - Dark Cloud Cover is a REVERSAL pattern and is meaningless without a
    ...
    following is true (anchor = bar N, the completion bar at index i):
...
def detect_dark_cloud_cover(bars: List[Bar], context: dict) -> List[Detection]:
    ...
    for i in range(start, len(bars)):
        ...
        # and detection builder. Anchor = bar N (the completion bar at index i).
        ...
        # Dark Cloud Cover is a REVERSAL pattern: it is meaningless without
        ...
        if not has_reversal_context:
            continue  # anti-pattern: dark cloud cover in non-topping location
...
def _score_context(
    ...
    *,
    ...
    """Score topping context at bar N index (completion bar).

...
def _strip_media(node: dict[str, Any], parent: str) -> dict[str, Any] | None:
    """Replace every image with a text marker (inline) or a paragraph holding
    one (block), and drop `import-link://` link marks (they name documents of
    an import that does not exist here). Returns a new node."""
    if not isinstance(node, dict):
        return None
    if node.get("type") == "image":
        text = {"type": "text", "text": _image_marker(node)}
        return {"type": "paragraph", "content": [text]} if parent in _BLOCK_CONTAINERS else text
    out = dict(node)
    marks = node.get("marks")
    if isinstance(marks, list):
        kept = [m for m in marks
                if not (isinstance(m, dict) and m.get("type") == "link"
                        and str((m.get("attrs") or {}).get("href") or "").startswith(_LINK_PREFIX))]
        if kept:
            out["marks"] = kept
        else:
            out.pop("marks", None)

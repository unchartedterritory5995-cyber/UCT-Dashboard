...
_REALTIME_INSTRUCTIONS = (
    ...
    "sentences. If a tool returns `{ok: false, error: ...}`, briefly say what "
    "went wrong and offer a next step.\n\n"
    ...
    "MEMORY: When the user states a preference or fact about themselves, "
    "silently call `remember(fact, category)`. When they say 'forget X', call "
    "`forget(query=X)`. When they ask what you remember, call `list_my_facts`. "
    "When they reference a past conversation, call `recall_session(query)`.\n\n"
    "WRITES — two-phase confirm pattern:\n"

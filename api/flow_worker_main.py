        ...
        try:
            ...
            if os.getenv("OI_MORNING_ENABLED", "0") == "1":
                ...
                sched.add_job(_oim.run_oi_morning,
                              ...
                              coalesce=True, replace_existing=True)
                n += 1
                log.info("[startup] Morning OI cron registered (08:00 ET weekdays) [single-name v2]")
            else:
                log.info("[startup] Morning OI push disabled (set OI_MORNING_ENABLED=1) [single-name v2]")
        except Exception as e:  # noqa: BLE001
